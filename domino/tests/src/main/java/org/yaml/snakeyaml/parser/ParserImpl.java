...
public final class ParserImpl implements Parser {
		...
		public Event produce() {
			...
			if (!scanner.checkToken(Token.ID.StreamEnd)) {
				...
				if (!scanner.checkToken(Token.ID.DocumentStart)) {
					throw new ParserException(null, null, "expected '<document start>', but found " + scanner.peekToken().getTokenId(),
	...
	private VersionTagsTuple processDirectives() {
		...
		while (scanner.checkToken(Token.ID.Directive)) {
			...
			if (token.getName().equals("YAML")) {
				...
				if (major != 1) {
					throw new ParserException(null, null, "found incompatible YAML document (version 1.* is required)",
							token.getStartMark());
				}
		...
		if (scanner.checkToken(Token.ID.Alias)) {
			...
			if (indentlessSequence && scanner.checkToken(Token.ID.BlockEntry)) {
				...
				if (scanner.checkToken(Token.ID.Scalar)) {
					...
					Token token = scanner.peekToken();
					throw new ParserException("while parsing a " + node + " node", startMark, "expected the node content, but found "
	...
	private class ParseBlockSequenceEntry implements Production {
		...
		public Event produce() {
			...
			if (!scanner.checkToken(Token.ID.BlockEnd)) {
				Token token = scanner.peekToken();
				throw new ParserException("while parsing a block collection", marks.pop(), "expected <block end>, but found "
						+ token.getTokenId(), token.getStartMark());
			}
			Token token = scanner.getToken();
	...
	private class ParseBlockMappingKey implements Production {
		...
		public Event produce() {
			...
			if (!scanner.checkToken(Token.ID.BlockEnd)) {
				Token token = scanner.peekToken();
				throw new ParserException("while parsing a block mapping", marks.pop(), "expected <block end>, but found "
						+ token.getTokenId(), token.getStartMark());
			}
			Token token = scanner.getToken();
			...
			if (!scanner.checkToken(Token.ID.FlowSequenceEnd)) {
				if (!first) {
					if (scanner.checkToken(Token.ID.FlowEntry)) {
						...
						Token token = scanner.peekToken();
						throw new ParserException("while parsing a flow sequence", marks.pop(), "expected ',' or ']', but got "
								+ token.getTokenId(), token.getStartMark());
					}
				}
				if (scanner.checkToken(Token.ID.Key)) {
			...
			if (!scanner.checkToken(Token.ID.FlowMappingEnd)) {
				if (!first) {
					if (scanner.checkToken(Token.ID.FlowEntry)) {
						...
						Token token = scanner.peekToken();
						throw new ParserException("while parsing a flow mapping", marks.pop(), "expected ',' or '}', but got "
								+ token.getTokenId(), token.getStartMark());
					}
				}
				if (scanner.checkToken(Token.ID.Key)) {
