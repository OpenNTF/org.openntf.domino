...
public class EdgeLabelWeightTransformer implements Transformer<Edge, Number> {
	...
	public EdgeLabelWeightTransformer(final Set<String> labels, final Boolean filterLabels, final String weightKey,
			final Boolean normalizeWeights, final Boolean invertWeights) {
		this.labels = labels;
			...
			if (filterLabels) {
				...
				if (object instanceof Number) {
					...
					if (this.normalizeWeights) {
						return weight
								/ JungHelper.totalWeight(JungHelper.filterEdgeLabels(edge.getVertex(Direction.OUT).getEdges(Direction.OUT),
										this.labels, filterLabels), this.weightKey);
					} else {
						return weight;
					}
				} else {
