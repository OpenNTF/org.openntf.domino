...
public class TypeRegistry {
	 ...
	 *         class. (Either the class itself, or a base class if the class was
	 *         registered).
	 */
	public Class<?> getTypeHoldingTypeField(Class<?> type) {
		if (type.getAnnotation(TypeField.class) != null)
			return type;
		Class<?> result = typeFields.get(type);
		return result;
	}

	/**
	 * @param typeHoldingTypeField
	 *            The type that has the {@link TypeField} annotation for the
	 *            Proxy class to be constructed.
	 * @param typeValue
	 *            The actual persisted value for a type field on an
	 *            {@link Element} in the graph
	 * @return The type that needs to be constructed, or null if there is no
	...
	public TypeRegistry add(Class<?> type) {
		...
		if (!typeFields.containsKey(type)) {
			...
			Validate.assertArgument(typeHoldingTypeField != null,
					"The type and its supertypes don't have a @TypeField annotation: %s", type.getName());
