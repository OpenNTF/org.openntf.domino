...
public class diff_match_patch {
	...
	/**
	 * How far to search for a match (0 = exact location, 1000+ = broad match). A match this many characters away from
	 * the expected location will add 1.0 to the score (0.0 is a perfect match).
	 */
	public int Match_Distance = 1000;
	/**
	 * When deleting a large block of text (over ~64 characters), how close do the contents have to be to match the
	 * expected contents. (0.0 = perfection, 1.0 = very loose). Note that Match_Threshold controls how closely the end
	 ...
	 *            New string to be diffed.
	 * @param checklines
	 *            Speedup flag. If false, then don't run a line-level diff first to identify the changed areas. If true,
	 ...
	 *            then run a faster slightly less optimal diff.
	 * @param deadline
	 *            Time when the diff should be complete by. Used internally for recursive calls. Users should set
	 *            DiffTimeout instead.
	 * @return Linked List of Diff objects.
	 */
	...
	/**
	 * Find the 'middle snake' of a diff, split the problem in two and return the recursively constructed diff. See
	...
	/**
	 * Locate the best instance of 'pattern' in 'text' near 'loc' using the Bitap algorithm. Returns -1 if no match
	 * found.
	 * 
	 * @param text
	 *            The text to search.
	...
	protected void patch_addContext(Patch patch, String text) {
		...
		while (text.indexOf(pattern) != text.lastIndexOf(pattern)
			...
			pattern = text.substring(Math.max(0, patch.start2 - padding), Math.min(text.length(), patch.start2
	...
	/**
	 * Compute a list of patches to turn text1 into text2. text2 is ignored, diffs are the delta between text1 and
	 * text2.
	 * 
	 * @param text1
	 *            Old text
	...
	/**
	 * Compute a list of patches to turn text1 into text2. text2 is not provided, diffs are the delta between text1 and
	 * text2.
	 * 
	 * @param text1
	 *            Old text.
	...
	/**
	 * Merge a set of patches onto the text. Return a patched text, as well as an array of true/false values indicating
			...
			while (!bigpatch.diffs.isEmpty()) {
				...
				while (!bigpatch.diffs.isEmpty() && patch.length1 < patch_size - Patch_Margin) {
					...
					if (diff_type == Operation.INSERT) {
						...
						diff_text = diff_text.substring(0, Math.min(diff_text.length(), patch_size - patch.length1
								- Patch_Margin));
						patch.length1 += diff_text.length();
						start1 += diff_text.length();
	...
	/**
	 * Unescape selected chars for compatability with JavaScript's encodeURI. In speed critical applications this could
	 * be dropped since the receiving application will certainly decode these fine. Note that this function is
	 * case-sensitive. Thus "%3f" would not be unescaped. But this is ok because it is only called with the output of
