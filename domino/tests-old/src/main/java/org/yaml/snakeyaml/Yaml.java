...
public class Yaml {
	 ...
	 */
	@Deprecated
	public Yaml(final BaseConstructor constructor, final LoaderOptions loaderOptions, final Representer representer,
			final DumperOptions dumperOptions, final Resolver resolver) {
	 ...
	 */
	public String dumpAll(final Iterator<? extends Object> data) {
	...
	public void dumpAll(final Iterator<? extends Object> data, final Writer output) {
		dumpAll(data, output, dumperOptions.getExplicitRoot());
	}

	private void dumpAll(final Iterator<? extends Object> data, final Writer output, final Tag rootTag) {
	...
	/**
	 * <p>
	 * Serialize a Java object into a YAML string. Override the default root tag with <code>rootTag</code>.
	 * </p>
	 * 
	 * <p>
	 * This method is similar to <code>Yaml.dump(data)</code> except that the root tag for the whole document is replaced with the given
	 * tag. This has two main uses.
	 * </p>
	 * 
	 * <p>
	 * First, if the root tag is replaced with a standard YAML tag, such as <code>Tag.MAP</code>, then the object will be dumped as a map.
	 * The root tag will appear as <code>!!map</code>, or blank (implicit !!map).
	 * </p>
	 * 
	 * <p>
	 * Second, if the root tag is replaced by a different custom tag, then the document appears to be a different type when loaded. For
	 * example, if an instance of MyClass is dumped with the tag !!YourClass, then it will be handled as an instance of YourClass when
	 * loaded.
	 * </p>
	 * 
	 * @param data
	 *            Java object to be serialized to YAML
	 * @param rootTag
	 *            the tag for the whole YAML document. The tag should be Tag.MAP for a JavaBean to make the tag disappear (to use implicit
	 *            tag !!map). If <code>null</code> is provided then the standard tag with the full class name is used.
	 * @param flowStyle
	 *            flow style for the whole document. See Chapter 10. Collection Styles http://yaml.org/spec/1.1/#id930798. If
	 *            <code>null</code> is provided then the flow style from DumperOptions is used.
	 * 
	 * @return YAML String
	 */
	public String dumpAs(final Object data, final Tag rootTag, final FlowStyle flowStyle) {
	...
	/**
	 * <p>
	 * Serialize a Java object into a YAML string. Override the default root tag with <code>Tag.MAP</code>.
	 * </p>
	 * <p>
	 * This method is similar to <code>Yaml.dump(data)</code> except that the root tag for the whole document is replaced with
	 * <code>Tag.MAP</code> tag (implicit !!map).
	 * </p>
	 * <p>
	...
	public List<Event> serialize(final Node data) {
		...
		@SuppressWarnings("deprecation")
		Serializer serializer = new Serializer(emitter, resolver, dumperOptions, dumperOptions.getExplicitRoot());
		try {
			serializer.open();
	...
	/**
	 * Parse the only YAML document in a String and produce the corresponding Java object. (Because the encoding in known BOM is not
	 * respected.)
	 * 
	 * @param yaml
	...
	/**
	 * Add an implicit scalar detector. If an implicit scalar value matches the given regexp, the corresponding tag is assigned to the
	 * scalar.
	 * 
	...
	/**
	 * Get a meaningful name. It simplifies debugging in a multi-threaded environment. If nothing is set explicitly the address of the
