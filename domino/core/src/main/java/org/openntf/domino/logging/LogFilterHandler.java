...
import org.openntf.domino.logging.LogConfig.L_LogHandler;
...
/**
 * Represents FilterHandler items from the configuration file. FilterHandlers attach handlers to loggers, taking into account optional
 * Filter Config Entries which fine tune the logging behavior with conditions and validation dates.
 * <p>
 * The LogFilterHandler actually attaches itself to configured loggers and when a LogRecord needs to be published, it goes through defined
...
public class LogFilterHandler extends Handler {
	...
	private void getMyHandlers() {
		_myHandlers.clear();
		for (L_LogHandler _defaultHandler : _myConfigLFH._defaultHandlers) {
			_myHandlers.put(_defaultHandler, null);
		}
		for (LogConfig.L_LogFilterHandler.L_LogFilterConfigEntry fce : _myConfigLFH._logFCEList) {
			for (L_LogHandler _logHandlerObj : fce._logHandlerObjs) {
				_myHandlers.put(_logHandlerObj, null);
			}
		}
	}
	...
	public void finishUp() {
		if (_myConfigLFH._loggers != null) {
			for (Logger _logger : _myConfigLFH._loggers) {
	...
	private void publishDefault(final LogRecord logRec) {
		...
		for (L_LogHandler _defaultHandler : _myConfigLFH._defaultHandlers) {
			publishConditionally(_defaultHandler, logRec);
		}
	}
