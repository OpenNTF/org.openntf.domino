...
public class EMBridgeMessageQueue implements Runnable {
	protected MessageQueue queue_;
	protected boolean stopOrdered_ = false;

	private void sleep(final int millis) throws InterruptedException {
		try {
			Thread.sleep(millis);
		} catch (Throwable t) {
			t.printStackTrace();
	...
	public static final boolean STAT_REPORT = true;
		...
		if (i == 0) {
			...
			while (getStatus == 0) {
				...
				try {
					...
					if (getStatus == 0) {
						Xots.getService().addEMMessage(sb.toString());
						eventCount++;
