...
<xsl:stylesheet version="1.0" xmlns="http://www.lotus.com/dxl"
	...
	<!-- For Agent Non-Binary DXL For both LotusScript and Java agents For Java 
		Agents You may also wish to look at some extra ones like javaproject->codepath 
		or item->$JavaCompilerSource item->$JavaComplierTarget -->
	...
	<!-- For the Database Properties Non-Binary DXL. Most of these attributes/elements 
		are guaranteed to be different on different developer copies -->
	...
	<!-- Ignore the database ACL Note: You may or may not want to do this! 
		In my case I have no need to keep acl in source repo. -->

	<xsl:template match="//database/acl" />

	<!-- Remove any items that begin with $ and end with _O for example <item 
		name="$yourfield_O" ....></item> These Items are Script Object items, they 
		are not source code! you freshly check out a repo version of the design element, 
		but at least you won't get merge conflicts all the time These items will 
		come back to the Design Element after a recompile, they just won't end up 
		in your repository, which is good news, because they are like .class files. -->
	<xsl:template match="//item">
		<xsl:if
			test="not(starts-with(@name,'$') and substring(@name,string-length(@name)-1,2) = '_O')">
