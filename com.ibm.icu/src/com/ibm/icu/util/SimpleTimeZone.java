 ...
 * 
 * <p>
 * <code>SimpleTimeZone</code> is a concrete subclass of <code>TimeZone</code> that represents a time zone for use with a Gregorian
 * calendar. This class does not handle historical changes.
 * 
 * <p>
 * Use a negative value for <code>dayOfWeekInMonth</code> to indicate that <code>SimpleTimeZone</code> should count from the end of the
 * month backwards. For example, if Daylight Savings Time starts or ends at the last Sunday in a month, use
 * <code>dayOfWeekInMonth = -1</code> along with <code>dayOfWeek = Calendar.SUNDAY</code> to specify the rule.
 * 
 * @see Calendar
...
public class SimpleTimeZone extends BasicTimeZone {
	...
	/**
	 * Constructs a SimpleTimeZone with the given base time zone offset from GMT and time zone ID. Timezone IDs can be obtained from
	 * TimeZone.getAvailableIDs. Normally you should use TimeZone.getDefault to construct a TimeZone.
	 * 
	 * @param rawOffset
	 ...
	 * Timezone IDs can be obtained from TimeZone.getAvailableIDs. Normally you should use TimeZone.getDefault to create a TimeZone. For a
	 * time zone that does not use daylight saving time, do not use this constructor; instead you should use SimpleTimeZone(rawOffset, ID).
	 * 
	 * By default, this constructor specifies day-of-week-in-month rules. That is, if the startDay is 1, and the startDayOfWeek is SUNDAY,
	 * then this indicates the first Sunday in the startMonth. A startDay of -1 likewise indicates the last Sunday. However, by using
	 * negative or zero values for certain parameters, other types of rules can be specified.
	 * 
	 * Day of month. To specify an exact day of the month, such as March 1, set startDayOfWeek to zero.
	 * 
	 * Day of week after day of month. To specify the first day of the week occurring on or after an exact day of the month, make the day of
	 * the week negative. For example, if startDay is 5 and startDayOfWeek is -MONDAY, this indicates the first Monday on or after the 5th
	 * day of the startMonth.
	 * 
	 * Day of week before day of month. To specify the last day of the week occurring on or before an exact day of the month, make the day
	 * of the week and the day of the month negative. For example, if startDay is -21 and startDayOfWeek is -WEDNESDAY, this indicates the
	 * last Wednesday on or before the 21st of the startMonth.
	 * 
	 * The above examples refer to the startMonth, startDay, and startDayOfWeek; the same applies for the endMonth, endDay, and
	 * endDayOfWeek.
	 * 
	 * @param rawOffset
	 *            The given base time zone offset to GMT.
	 ...
	 *            The time zone ID which is obtained from TimeZone.getAvailableIDs.
	 * @param startMonth
	 *            The daylight savings starting month. Month is 0-based. eg, 0 for January.
	 * @param startDay
	 ...
	 * @param startTime
	 *            The daylight savings starting time in local wall time, which is standard time in this case. Please see the member
	 *            description for an example.
	 * @param endMonth
	 *            The daylight savings ending month. Month is 0-based. eg, 0 for January.
	 * @param endDay
	 ...
	 * @param endTime
	 *            The daylight savings ending time in local wall time, which is daylight time in this case. Please see the member
	 *            description for an example.
	 * @throws IllegalArgumentException
	 *             the month, day, dayOfWeek, or time parameters are out of range for the start or end rule
	 * @stable ICU 2.0
	 */
	public SimpleTimeZone(final int rawOffset, final String ID, final int startMonth, final int startDay, final int startDayOfWeek,
			final int startTime, final int endMonth, final int endDay, final int endDayOfWeek, final int endTime) {
		super(ID);
		construct(rawOffset, startMonth, startDay, startDayOfWeek, startTime, WALL_TIME, endMonth, endDay, endDayOfWeek, endTime,
				WALL_TIME, Grego.MILLIS_PER_HOUR);
	}

	/**
	 * Constructs a SimpleTimeZone with the given base time zone offset from GMT, time zone ID, time and its mode to start and end the
	 * daylight time. The mode specifies either {@link #WALL_TIME} or {@link #STANDARD_TIME} or {@link #UTC_TIME}.
	 * 
	 * @param rawOffset
	 *            The given base time zone offset to GMT.
	 * @param ID
	 ...
	 */
	public SimpleTimeZone(final int rawOffset, final String ID, final int startMonth, final int startDay, final int startDayOfWeek,
			final int startTime, final int startTimeMode, final int endMonth, final int endDay, final int endDayOfWeek, final int endTime,
			final int endTimeMode, final int dstSavings) {
		super(ID);
		construct(rawOffset, startMonth, startDay, startDayOfWeek, startTime, startTimeMode, endMonth, endDay, endDayOfWeek, endTime,
	 ...
	 */
	public SimpleTimeZone(final int rawOffset, final String ID, final int startMonth, final int startDay, final int startDayOfWeek,
			final int startTime, final int endMonth, final int endDay, final int endDayOfWeek, final int endTime, final int dstSavings) {
		super(ID);
		construct(rawOffset, startMonth, startDay, startDayOfWeek, startTime, WALL_TIME, endMonth, endDay, endDayOfWeek, endTime,
	...
	/**
	 * Sets the daylight savings starting rule. For example, Daylight Savings Time starts at the second Sunday in March, at 2 AM in standard
	 * time. Therefore, you can set the start rule by calling: setStartRule(Calendar.MARCH, 2, Calendar.SUNDAY, 2*60*60*1000);
	 * 
	 * @param month
	 *            The daylight savings starting month. Month is 0-based. eg, 0 for January.
	...
	/**
	 * Sets the daylight savings starting rule. For example, in the U.S., Daylight Savings Time starts at the second Sunday in March, at 2
	 * AM in standard time. Therefore, you can set the start rule by calling:
	 * <code>setStartRule(Calendar.MARCH, 2, Calendar.SUNDAY, 2*60*60*1000);</code> The dayOfWeekInMonth and dayOfWeek parameters together
	 * specify how to calculate the exact starting date. Their exact meaning depend on their respective signs, allowing various types of
	 * rules to be constructed, as follows:
	 * <ul>
	 * <li>If both dayOfWeekInMonth and dayOfWeek are positive, they specify the day of week in the month (e.g., (2, WEDNESDAY) is the
	 * second Wednesday of the month).
	 * <li>If dayOfWeek is positive and dayOfWeekInMonth is negative, they specify the day of week in the month counting backward from the
	 * end of the month. (e.g., (-1, MONDAY) is the last Monday in the month)
	 * <li>If dayOfWeek is zero and dayOfWeekInMonth is positive, dayOfWeekInMonth specifies the day of the month, regardless of what day of
	 * the week it is. (e.g., (10, 0) is the tenth day of the month)
	 * <li>If dayOfWeek is zero and dayOfWeekInMonth is negative, dayOfWeekInMonth specifies the day of the month counting backward from the
	 * end of the month, regardless of what day of the week it is (e.g., (-2, 0) is the next-to-last day of the month).
	 * <li>If dayOfWeek is negative and dayOfWeekInMonth is positive, they specify the first specified day of the week on or after the
	 * specfied day of the month. (e.g., (15, -SUNDAY) is the first Sunday after the 15th of the month [or the 15th itself if the 15th is a
	 * Sunday].)
	 * <li>If dayOfWeek and DayOfWeekInMonth are both negative, they specify the last specified day of the week on or before the specified
	 * day of the month. (e.g., (-20, -TUESDAY) is the last Tuesday before the 20th of the month [or the 20th itself if the 20th is a
	 * Tuesday].)
	 * </ul>
	 * 
	 * @param month
	 ...
	 * 
	 * @param month
	 *            The month in which this rule occurs (0-based).
	 * @param dayOfMonth
	 *            The date in that month (1-based).
	 * @param time
	 *            The time of that day (number of millis after midnight) when DST takes effect in local wall time, which is standard time in
	 *            this case.
	 * @throws IllegalArgumentException
	 ...
	 *            The month in which this rule occurs (0-based).
	 * @param dayOfMonth
	 *            A date within that month (1-based).
	 * @param dayOfWeek
	 *            The day of the week on which this rule occurs.
	 * @param time
	 *            The time of that day (number of millis after midnight) when DST takes effect in local wall time, which is standard time in
	 *            this case.
	 * @param after
	 *            If true, this rule selects the first dayOfWeek on or after dayOfMonth. If false, this rule selects the last dayOfWeek on
	 *            or before dayOfMonth.
	 * @throws IllegalArgumentException
	...
	/**
	 * Sets the daylight savings ending rule. For example, if Daylight Savings Time ends at the last (-1) Sunday in October, at 2 AM in
	 * standard time, you can set the end rule by calling: <code>setEndRule(Calendar.OCTOBER, -1, Calendar.SUNDAY, 2*60*60*1000);</code>
	 * 
	 * @param month
	 *            The daylight savings ending month. Month is 0-based. eg, 0 for January.
	 * @param dayOfWeekInMonth
	 ...
	 *            The date in that month (1-based).
	 * @param time
	 *            The time of that day (number of millis after midnight) when DST ends in local wall time, which is daylight time in this
	 *            case.
	 * @throws IllegalArgumentException
	 *             the month, dayOfMonth, or time parameters are out of range
	 * @stable ICU 2.0
	...
	/**
	 * Sets the daylight savings ending rule. For example, in the U.S., Daylight Savings Time ends at the first Sunday in November, at 2 AM
	 * in standard time. Therefore, you can set the end rule by calling: setEndRule(Calendar.NOVEMBER, 1, Calendar.SUNDAY, 2*60*60*1000);
	 * Various other types of rules can be specified by manipulating the dayOfWeek and dayOfWeekInMonth parameters. For complete details,
	 * see the documentation for setStartRule().
	 * 
	 * @param month
	 *            the daylight savings ending month. Month is 0-based. eg, 0 for January.
	 * @param dayOfWeekInMonth
	 *            the daylight savings ending day-of-week-in-month. See setStartRule() for a complete explanation.
	 * @param dayOfWeek
	 *            the daylight savings ending day-of-week. See setStartRule() for a complete explanation.
	 * @param time
	 ...
	 *            the number of milliseconds the time is advanced with respect to standard time when the daylight savings rules are in
	 ...
	 * 
	 * @return the number of milliseconds the time is advanced with respect to standard time when the daylight savings rules are in effect.
	 *         A positive number, typically one hour (3600000).
	 * @stable ICU 2.0
	 */
	@Override
	 ...
	 * 
	 * java.util.SimpleTimeZone unwrapSTZ() { return (java.util.SimpleTimeZone) unwrap(); }
	 */
	...
	private int getOffset(final int era, final int year, final int month, final int day, final int dayOfWeek, final int millis,
			final int monthLength, final int prevMonthLength) {
		...
		if (true) {
			 ...
			 * file.  */
			if ((era != GregorianCalendar.AD && era != GregorianCalendar.BC) || month < Calendar.JANUARY || month > Calendar.DECEMBER
					|| day < 1 || day > monthLength || dayOfWeek < Calendar.SUNDAY || dayOfWeek > Calendar.SATURDAY || millis < 0
					|| millis >= Grego.MILLIS_PER_DAY || monthLength < 28 || monthLength > 31 || prevMonthLength < 28
					|| prevMonthLength > 31) {
				throw new IllegalArgumentException();
			}
		}
		...
		int startCompare = compareToRule(month, monthLength, prevMonthLength, day, dayOfWeek, millis, startTimeMode == UTC_TIME ? -raw : 0,
		...
		if (southern != (startCompare >= 0)) {
			...
			endCompare = compareToRule(month, monthLength, prevMonthLength, day, dayOfWeek, millis, endTimeMode == WALL_TIME ? dst
					: (endTimeMode == UTC_TIME ? -raw : 0), endMode, endMonth, endDayOfWeek, endDay, endTime);
		}
	...
	public void getOffsetFromLocal(long date, final int nonExistingTimeOpt, final int duplicatedTimeOpt, final int[] offsets) {
		...
		if (offsets[1] > 0) {
			if ((nonExistingTimeOpt & STD_DST_MASK) == LOCAL_STD || (nonExistingTimeOpt & STD_DST_MASK) != LOCAL_DST
					&& (nonExistingTimeOpt & FORMER_LATTER_MASK) != LOCAL_LATTER) {
				date -= getDSTSavings();
				recalc = true;
			}
		} else {
			if ((duplicatedTimeOpt & STD_DST_MASK) == LOCAL_DST || (duplicatedTimeOpt & STD_DST_MASK) != LOCAL_STD
	...
	/**
	 * Compare a given date in the year to a rule. Return 1, 0, or -1, depending on whether the date is after, equal to, or before the rule
	 * date. The millis are compared directly against the ruleMillis, so any standard-daylight adjustments must be handled by the caller.
	 * 
	 * @return 1 if the date is after the rule date, -1 if the date is before the rule date, or 0 if the date is equal to the rule date.
	 */
	private int compareToRule(int month, final int monthLen, final int prevMonthLen, int dayOfMonth, int dayOfWeek, int millis,
			final int millisDelta, final int ruleMode, final int ruleMonth, final int ruleDayOfWeek, int ruleDay, final int ruleMillis) {
	 ...
	 */
	private void construct(final int _raw, final int _startMonth, final int _startDay, final int _startDayOfWeek, final int _startTime,
			final int _startTimeMode, final int _endMonth, final int _endDay, final int _endDayOfWeek, final int _endTime,
			final int _endTimeMode, final int _dst) {
		raw = _raw;
	...
	/**
	 * Decode the start rule and validate the parameters. The parameters are expected to be in encoded form, which represents the various
	 * rule modes by negating or zeroing certain values. Representation formats are:
	 * <p>
	 * 
	 * <pre>
	 ...
	 * </pre>
	 * 
	 * The range for month does not include UNDECIMBER since this class is really specific to GregorianCalendar, which does not use that
	 * month. The range for time includes ONEDAY (vs. ending at ONEDAY-1) because the end rule is an exclusive limit point. That is, the
	 * range of times that are in DST include those >= the start and < the end. For this reason, it should be possible to specify an end of
	 * ONEDAY in order to include the entire day. Although this is equivalent to time 0 of the following day, it's not always possible to
	 * specify that, for example, on December 31. While arguably the start range should still be 0..ONEDAY-1, we keep the start and end
	 * ranges the same for consistency.
	 */
	...
	public boolean equals(final Object obj) {
		...
		return raw == that.raw && useDaylight == that.useDaylight
				&& idEquals(getID(), that.getID())
				&& (!useDaylight
				// Only check rules if using DST
				|| (dst == that.dst && startMode == that.startMode && startMonth == that.startMonth && startDay == that.startDay
						&& startDayOfWeek == that.startDayOfWeek && startTime == that.startTime && startTimeMode == that.startTimeMode
						&& endMode == that.endMode && endMonth == that.endMonth && endDay == that.endDay
						&& endDayOfWeek == that.endDayOfWeek && endTime == that.endTime && endTimeMode == that.endTimeMode && startYear == that.startYear));
	...
	public int hashCode() {
		...
		if (!useDaylight) {
			ret += dst ^ (dst >>> 10) + startMode ^ (startMode >>> 11) + startMonth ^ (startMonth >>> 12) + startDay ^ (startDay >>> 13)
					+ startDayOfWeek ^ (startDayOfWeek >>> 14) + startTime ^ (startTime >>> 15) + startTimeMode ^ (startTimeMode >>> 16)
					+ endMode ^ (endMode >>> 17) + endMonth ^ (endMonth >>> 18) + endDay ^ (endDay >>> 19) + endDayOfWeek
					^ (endDayOfWeek >>> 20) + endTime ^ (endTime >>> 21) + endTimeMode ^ (endTimeMode >>> 22) + startYear
					^ (startYear >>> 23);
		}
		return ret;
	}
	...
	public boolean hasSameRules(final TimeZone othr) {
		...
		return other != null && raw == other.raw
				...
				&& (!useDaylight
				// Only check rules if using DST
				|| (dst == other.dst && startMode == other.startMode && startMonth == other.startMonth && startDay == other.startDay
						&& startDayOfWeek == other.startDayOfWeek && startTime == other.startTime && startTimeMode == other.startTimeMode
						&& endMode == other.endMode && endMonth == other.endMonth && endDay == other.endDay
						&& endDayOfWeek == other.endDayOfWeek && endTime == other.endTime && endTimeMode == other.endTimeMode && startYear == other.startYear));
	}
	...
	private synchronized void initTransitionRules() {
		...
		if (useDaylight) {
			...
			dstRule = new AnnualTimeZoneRule(getID() + "(DST)", getRawOffset(), getDSTSavings(), dtRule, startYear,
			...
			timeRuleType = (endTimeMode == STANDARD_TIME) ? DateTimeRule.STANDARD_TIME : ((endTimeMode == UTC_TIME) ? DateTimeRule.UTC_TIME
			...
			stdRule = new AnnualTimeZoneRule(getID() + "(STD)", getRawOffset(), 0, dtRule, startYear, AnnualTimeZoneRule.MAX_YEAR);
