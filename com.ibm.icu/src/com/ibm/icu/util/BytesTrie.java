...
/**
 * Light-weight, non-const reader class for a BytesTrie. Traverses a byte-serialized data structure with minimal state, for mapping byte
...
public final class BytesTrie implements Cloneable, Iterable<BytesTrie.Entry> {
	 ...
	 * 
	 * <p>
	 * The array must contain a copy of a byte sequence from the BytesTrieBuilder, with the offset indicating the first byte of that
	 * sequence. The BytesTrie object will not read more bytes than the BytesTrieBuilder generated in the corresponding build() call.
	 * 
	 * <p>
	 * The array is not copied/cloned and must not be modified while the BytesTrie object is in use.
	 * 
	 * @param trieBytes
	 *            Bytes array that contains the serialized trie.
	 * @param offset
	 ...
	 * 
	 * @param state
	 *            The State object to hold the trie's state.
	 * @return this
	 * @see #resetToState
	 * @stable ICU 4.8
	 */
	 ...
	 * 
	 * @param state
	 *            The State object which holds a saved trie state.
	 * @return this
	...
	public enum Result {
		/**
		 * The input unit(s) did not continue a matching string. Once current()/next() return NO_MATCH, all further calls to
		 * current()/next() will also return NO_MATCH, until the trie is reset to its original state or to a saved state.
		 * 
		 * @stable ICU 4.8
		 */
		NO_MATCH,
		...
		NO_VALUE,
		/**
		 * The input unit(s) continued a matching string and there is a value for the string so far. This value will be returned by
		 * getValue(). No further input byte/unit can continue a matching string.
		 * 
		 * @stable ICU 4.8
		 */
		FINAL_VALUE,
		/**
		 * The input unit(s) continued a matching string and there is a value for the string so far. This value will be returned by
		 * getValue(). Another input byte/unit can continue a matching string.
		 * 
		 * @stable ICU 4.8
		 */
		INTERMEDIATE_VALUE;
	...
	/**
	 * Determines whether the byte sequence so far matches, whether it has a value, and whether another input byte can continue a matching
	 * byte sequence.
	 * 
	...
	public Result current() /*const*/{
		...
		if (pos < 0) {
			...
			int node;
			return (remainingMatchLength_ < 0 && (node = bytes_[pos] & 0xff) >= kMinValueLead) ? valueResults_[node & kValueIsFinal]
					: Result.NO_VALUE;
		}
	}
	 ...
	 * 
	 * @param inByte
	 *            Input byte value. Values -0x100..-1 are treated like 0..0xff. Values below -0x100 and above 0xff will never match.
	 * @return The match/value Result.
	 * @stable ICU 4.8
	 */
	 ...
	 * 
	 * @param s
	 *            Contains a string or byte sequence.
	 * @param sIndex
	 *            The start index of the byte sequence in s.
	 * @param sLimit
	 *            The (exclusive) end index of the byte sequence in s.
	 * @return The match/value Result.
	 * @stable ICU 4.8
	 */
		...
		for (;;) {
			...
			for (;;) {
				if (sIndex == sLimit) {
					...
					pos_ = pos;
					int node;
					return (length < 0 && (node = (bytes_[pos] & 0xff)) >= kMinValueLead) ? valueResults_[node & kValueIsFinal]
							: Result.NO_VALUE;
				}
				inByte = s[sIndex++];
				if (length < 0) {
	...
	/**
	 * Returns a matching byte sequence's value if called immediately after current()/first()/next() returned Result.INTERMEDIATE_VALUE or
	 * Result.FINAL_VALUE. getValue() can be called multiple times.
	 * 
	 ...
	 * 
	 * @return The unique value in bits 32..1 with bit 0 set, if all byte sequences reachable from the current state map to the same value;
	 *         otherwise returns 0.
	 * @stable ICU 4.8
	 */
	...
	/**
	 * Finds each byte which continues the byte sequence from the current state. That is, each byte b for which it would be
	 * next(b)!=Result.NO_MATCH now.
	 * 
	 * @param out
	 *            Each next byte is 0-extended to a char and appended to this object. (Only uses the out.append(c) method.)
	 ...
	 *            If 0, the iterator returns full strings/byte sequences. Otherwise, the iterator returns strings with this maximum length.
	 * @return A new BytesTrie.Iterator.
	 * @stable ICU 4.8
	 */
	...
	public static final class Iterator implements java.util.Iterator<Entry> {
		 ...
		 * 
		 * If the byte sequence is truncated to the maximum length and does not have a real value, then the value is set to -1. In this
		 * case, this "not a real value" is indistinguishable from a real value of -1.
		 * 
			...
			if (inByte == (bytes_[pos++] & 0xff)) {
				...
				if ((node & kValueIsFinal) != 0) {
					...
					if (node < kMinTwoByteValueLead) {
						...
						delta = (bytes_[pos] << 24) | ((bytes_[pos + 1] & 0xff) << 16) | ((bytes_[pos + 2] & 0xff) << 8)
								| (bytes_[pos + 3] & 0xff);
						pos += 4;
					}
					// end readValue()
