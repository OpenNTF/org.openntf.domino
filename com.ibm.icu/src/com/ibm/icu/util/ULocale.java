 ...
 * 
 * A class analogous to {@link java.util.Locale} that provides additional support for ICU protocol. In ICU 3.0 this class is enhanced to
 * support RFC 3066 language identifiers.
 * 
 * <p>
 * Many classes and services in ICU follow a factory idiom, in which a factory method or object responds to a client request with an object.
 * The request includes a locale (the <i>requested</i> locale), and the returned object is constructed using data for that locale. The
 * system may lack data for the requested locale, in which case the locale fallback mechanism will be invoked until a populated locale is
 * found (the <i>valid</i> locale). Furthermore, even when a populated locale is found (the <i>valid</i> locale), further fallback may be
 * required to reach a locale containing the specific data required by the service (the <i>actual</i> locale).
 * 
 * <p>
 * ULocale performs <b>'normalization'</b> and <b>'canonicalization'</b> of locale ids. Normalization 'cleans up' ICU locale ids as follows:
 * <ul>
 ...
 * <li>three-letter language and country ids are converted to two-letter equivalents where available</li>
 ...
 * <li>'PREEURO' and 'EURO' variants are converted to currency keyword form, with the currency id appropriate to the country of the locale
 * (for PREEURO) or EUR (for EURO).
 * </ul>
 * All ULocale constructors automatically normalize the locale id. To handle POSIX ids, <code>canonicalize</code> can be called to convert
 * the id to canonical form, or the <code>canonicalInstance</code> factory method can be called.
 * </p>
 * 
 * <p>
 * This class provides selectors {@link #VALID_LOCALE} and {@link #ACTUAL_LOCALE} intended for use in methods named <tt>getLocale()</tt>.
 * These methods exist in several ICU classes, including {@link com.ibm.icu.util.Calendar}, {@link com.ibm.icu.util.Currency},
 * {@link com.ibm.icu.text.UFormat}, {@link com.ibm.icu.text.BreakIterator}, <a href="../text/Collator.html"
 * title="class in com.ibm.icu.text"><code>Collator</code></a>, {@link com.ibm.icu.text.DateFormatSymbols}, and
 * {@link com.ibm.icu.text.DecimalFormatSymbols} and their subclasses, if any. Once an object of one of these classes has been created,
 * <tt>getLocale()</tt> may be called on it to determine the valid and actual locale arrived at during the object's construction.
 * 
 * <p>
 * Note: The <i>actual</i> locale is returned correctly, but the <i>valid</i> locale is not, in most cases.
 * 
...
public final class ULocale implements Serializable {
	...
	/**
	 * Enum for locale categories. These locale categories are used to get/set the default locale for the specific functionality represented
	 * by the category.
	 * 
	 * @stable ICU 49
	 */
	...
	/**
	 * Cache the locale data container fields. In future, we want to use them as the primary locale identifier storage.
	 */
	...
	private static void initCANONICALIZE_MAP() {
		if (CANONICALIZE_MAP == null) {
			/**
			 * This table lists pairs of locale ids for canonicalization. The The 1st item is the normalized id. The 2nd item is the
			...
			String[][] tempCANONICALIZE_MAP = {
					...
					{ "ca_ES_PREEURO", "ca_ES", "currency", "ESP" }, { "cel_GAULISH", "cel__GAULISH", null, null }, /* registered name */
					{ "de_1901", "de__1901", null, null }, /* registered name */
					{ "de_1906", "de__1906", null, null }, /* registered name */
					{ "de__PHONEBOOK", "de", "collation", "phonebook" }, /* Old ICU name */
					{ "de_AT_PREEURO", "de_AT", "currency", "ATS" }, { "de_DE_PREEURO", "de_DE", "currency", "DEM" },
					{ "de_LU_PREEURO", "de_LU", "currency", "EUR" }, { "el_GR_PREEURO", "el_GR", "currency", "GRD" },
					{ "en_BOONT", "en__BOONT", null, null }, /* registered name */
					{ "en_SCOUSE", "en__SCOUSE", null, null }, /* registered name */
					{ "en_BE_PREEURO", "en_BE", "currency", "BEF" }, { "en_IE_PREEURO", "en_IE", "currency", "IEP" },
					{ "es__TRADITIONAL", "es", "collation", "traditional" }, /* Old ICU name */
					{ "es_ES_PREEURO", "es_ES", "currency", "ESP" }, { "eu_ES_PREEURO", "eu_ES", "currency", "ESP" },
					{ "fi_FI_PREEURO", "fi_FI", "currency", "FIM" }, { "fr_BE_PREEURO", "fr_BE", "currency", "BEF" },
					{ "fr_FR_PREEURO", "fr_FR", "currency", "FRF" }, { "fr_LU_PREEURO", "fr_LU", "currency", "LUF" },
					{ "ga_IE_PREEURO", "ga_IE", "currency", "IEP" }, { "gl_ES_PREEURO", "gl_ES", "currency", "ESP" },
					{ "hi__DIRECT", "hi", "collation", "direct" }, /* Old ICU name */
					{ "it_IT_PREEURO", "it_IT", "currency", "ITL" }, { "ja_JP_TRADITIONAL", "ja_JP", "calendar", "japanese" },
					//              { "nb_NO_NY",       "nn_NO", null, null },
					{ "nl_BE_PREEURO", "nl_BE", "currency", "BEF" }, { "nl_NL_PREEURO", "nl_NL", "currency", "NLG" },
					{ "pt_PT_PREEURO", "pt_PT", "currency", "PTE" }, { "sl_ROZAJ", "sl__ROZAJ", null, null }, /* registered name */
		...
		if (variantsToKeywords == null) {
			/**
			 * This table lists pairs of locale ids for canonicalization. The The first item is the normalized variant id.
			 */
			String[][] tempVariantsToKeywords = { { "EURO", "currency", "EUR" }, { "PINYIN", "collation", "pinyin" }, /* Solaris variant */
	...
	/**
	 * {@icu} Constructs a ULocale from a RFC 3066 locale ID. The locale ID consists of optional language, script, country, and variant
	 * fields in that order, separated by underscores, followed by an optional keyword list. The script, if present, is four characters
	 * long-- this distinguishes it from a country code, which is two characters long. Other fields are distinguished by position as
	 * indicated by the underscores. The start of the keyword list is indicated by '@', and consists of two or more keyword/value pairs
	 * separated by semicolons(';').
	 * 
	 * <p>
	 * This constructor does not canonicalize the localeID. So, for example, "zh__pinyin" remains unchanged instead of converting to
	 * "zh@collation=pinyin". By default ICU only recognizes the latter as specifying pinyin collation. Use {@link #createCanonical} or
	 * {@link #canonicalize} if you need to canonicalize the localeID.
	 * 
	 * @param localeID
	...
	/**
	 * Constructs a ULocale from a localeID constructed from the three 'fields' a, b, and c. These fields are concatenated using underscores
	 * to form a localeID of the form a_b_c, which is then handled like the localeID passed to <code>ULocale(String
	 * localeID)</code>.
	 * 
	 * <p>
	 * Java locale strings consisting of language, country, and variant will be handled by this form, since the country code (being shorter
	 * than four letters long) will not be interpreted as a script code. If a script code is present, the final argument ('c') will be
	 * interpreted as the country code. It is recommended that this constructor only be used to ease porting, and that clients instead use
	 * the single-argument constructor when constructing a ULocale from a localeID.
	 * 
	 * @param a
	 ...
	 * 
	 * @return a JDK locale that either exactly represents this object or is the closest approximation.
	 * @stable ICU 2.8
	 */
	 ...
	 * <p>
	 * The default ULocale is synchronized to the default Java Locale. This method checks the current default Java Locale and returns an
	 * equivalent ULocale.
	 * <p>
	 * <b>Note:</b> Before Java 7, the JDK Locale was not able to represent a locale's script. Therefore, the script field in the default
	 * ULocale is always empty unless a ULocale with non-empty script is explicitly set by {@link #setDefault(ULocale)} on Java 6 or older
	 * systems.
	 * <p>
	 * <b>Note for ICU 49 or later:</b> Some JRE implementations allow users to override the default JDK Locale using system properties -
	 * <code>user.language</code>, <code>user.country</code> and <code>user.variant</code>. In addition to these system properties, some
	 * Java 7 implementations support <code>user.script</code> for overriding the default Locale's script. ICU 49 and later versions use the
	 * <code>user.script</code> system property on Java 6 or older systems supporting other <code>user.*</code> system properties to
	 * initialize the default ULocale. The <code>user.script</code> override for default ULocale is not used on Java 7, or if the current
	 * Java default Locale is changed after start up.
	 * 
	...
	/**
	 * {@icu} Sets the default ULocale. This also sets the default Locale. If the caller does not have write permission to the user.language
	 * property, a security exception will be thrown, and the default ULocale will remain unchanged.
	 * <p>
	 ...
	 *             if a security manager exists and its <code>checkPermission</code> method doesn't allow the operation.
	...
	/**
	 * Sets the default <code>ULocale</code> for the specified <code>Category</code>. This also sets the default <code>Locale</code> for the
	 * specified <code>Category</code> of the JVM. If the caller does not have write permission to the user.language property, a security
	 * exception will be thrown, and the default ULocale for the specified Category will remain unchanged.
	 * 
	 * @param category
	...
	/**
	 * This is for compatibility with Locale-- in actuality, since ULocale is immutable, there is no reason to clone it, so this API returns
	 * 'this'.
	 * 
	 * @stable ICU 3.0
	 */
	...
	/**
	 * Returns true if the other object is another ULocale with the same full name. Note that since names are not canonicalized, two
	 * ULocales that function identically might not compare equal.
	 * 
	...
	/**
	 * {@icunote} Unlike the Locale API, this returns an array of <code>ULocale</code>, not <code>Locale</code>. Returns a list of all
	 * installed locales.
	 * 
	 * @stable ICU 3.0
	 */
	...
	/**
	 * {@icu} Returns the value for a keyword in the specified locale. If the keyword is not defined, returns null. The locale name does not
	...
	/**
	 * {@icu} Returns the canonical name for the specified locale ID. This is used to convert POSIX and other grandfathered IDs to standard
	 * ICU form.
	 * 
	 * @param localeID
	 *            the locale id
	...
	/**
	 * Given a keyword and a value, return a new locale with an updated keyword and value. If keyword is null, this removes all keywords
	 * from the locale id. Otherwise, if the value is null, this removes the value for this keyword from the locale id. Otherwise, this
	 * adds/replaces the value for this keyword in the locale id. The keyword and value must not be empty.
	 * 
	 * @param keyword
	...
	/**
	 * Given a locale id, a keyword, and a value, return a new locale id with an updated keyword and value. If keyword is null, this removes
	 * all keywords from the locale id. Otherwise, if the value is null, this removes the value for this keyword from the locale id.
	 * Otherwise, this adds/replaces the value for this keyword in the locale id. The keyword and value must not be empty.
	 * 
	 * @param localeID
	...
	/**
	 * Returns a three-letter abbreviation for this locale's language. If the locale doesn't specify a language, returns the empty string.
	 * Otherwise, returns a lowercase ISO 639-2/T language code. The ISO 639-2 language codes can be found on-line at <a
	...
	/**
	 * Returns a three-letter abbreviation for this locale's country/region. If the locale doesn't specify a country, returns the empty
	 * string. Otherwise, returns an uppercase ISO 3166 3-letter country code.
	 * 
	 * @exception MissingResourceException
	...
	/**
	 * {@icu} Returns this locale's language localized for display in the default <code>DISPLAY</code> locale. If a dialect name is present
	 * in the data, then it is returned.
	 * 
	 * @return the localized language name.
	 * @see Category#DISPLAY
	 * @stable ICU 4.4
	...
	/**
	 * {@icu} Returns this locale's language localized for display in the provided locale. If a dialect name is present in the data, then it
	 * is returned.
	 * 
	 * @param displayLocale
	 *            the locale in which to display the name.
	...
	/**
	 * {@icu} Returns a locale's language localized for display in the provided locale. If a dialect name is present in the data, then it is
	 * returned. This is a cover for the ICU4C API.
	 * 
	 * @param localeID
	 *            the id of the locale whose language will be displayed
	 * @param displayLocaleID
	...
	/**
	 * {@icu} Returns this locale name localized for display in the default <code>DISPLAY</code> locale. If a dialect name is present in the
	 * locale data, then it is returned.
	 * 
	 * @return the localized locale name.
	 * @see Category#DISPLAY
	 * @stable ICU 4.4
	...
	/**
	 * {@icu} Returns this locale name localized for display in the provided locale. If a dialect name is present in the locale data, then
	 * it is returned.
	 * 
	 * @param displayLocale
	 *            the locale in which to display the locale name.
	...
	/**
	 * {@icu} Returns the locale ID localized for display in the provided locale. If a dialect name is present in the locale data, then it
	 * is returned. This is a cover for the ICU4C API.
	 * 
	 * @param localeID
	 *            the locale whose name is to be displayed.
	 * @param displayLocaleID
	...
	/**
	 * {@icu} Returns this locale's layout orientation for characters. The possible values are "left-to-right", "right-to-left",
	...
	/**
	 * {@icu} Returns this locale's layout orientation for lines. The possible values are "left-to-right", "right-to-left", "top-to-bottom"
	 * or "bottom-to-top".
	 * 
	...
	/**
	 * {@icu} Selector for <tt>getLocale()</tt> indicating the locale of the resource containing the data. This is always at or above the
	 * valid locale. If the valid locale does not contain the specific data being requested, then the actual locale will be above the valid
	 * locale. If the object was not constructed from locale data, then the valid locale is <i>null</i>.
	 * 
	...
	/**
	 * {@icu} Selector for <tt>getLocale()</tt> indicating the most specific locale for which any data exists. This is always at or above
	 * the requested locale, and at or below the actual locale. If the requested locale does not correspond to any resource data, then the
	 * valid locale will be above the requested locale. If the object was not constructed from locale data, then the actual locale is
	 * <i>null</i>.
	 * 
	 * <p>
	...
	/**
	 * {@icu} Based on a HTTP formatted list of acceptable locales, determine an available locale for the user. NullPointerException is
	 * thrown if acceptLanguageList or availableLocales is null. If fallback is non-null, it will contain true if a fallback locale (one not
	 * in the acceptLanguageList) was returned. The value on entry is ignored. ULocale will be one of the locales in availableLocales, or
	 * the ROOT ULocale if if a ROOT locale was used as a fallback (because nothing else in availableLocales matched). No ULocale array
	 * element should be null; behavior is undefined if this is the case.
	 * 
	...
	/**
	 * {@icu} Based on a list of acceptable locales, determine an available locale for the user. NullPointerException is thrown if
	 * acceptLanguageList or availableLocales is null. If fallback is non-null, it will contain true if a fallback locale (one not in the
	 * acceptLanguageList) was returned. The value on entry is ignored. ULocale will be one of the locales in availableLocales, or the ROOT
	 * ULocale if if a ROOT locale was used as a fallback (because nothing else in availableLocales matched). No ULocale array element
	 * should be null; behavior is undefined if this is the case.
	 * 
	 * @param acceptLanguageList
		...
		for (i = 0; i < acceptLanguageList.length; i++) {
			...
			do {
				for (j = 0; j < availableLocales.length; j++) {
					...
					if (aLocale.getScript().length() == 0 && availableLocales[j].getScript().length() > 0
	 ...
	 * in the acceptLanguageList) was returned. The value on entry is ignored. ULocale will be one of the locales in availableLocales, or
	 * the ROOT ULocale if if a ROOT locale was used as a fallback (because nothing else in availableLocales matched). No ULocale array
	 * element should be null; behavior is undefined if this is the case. This function will choose a locale from the
	...
	/**
	 * {@icu} Based on an ordered array of acceptable locales, determine an available locale for the user. NullPointerException is thrown if
	 * acceptLanguageList or availableLocales is null. If fallback is non-null, it will contain true if a fallback locale (one not in the
	 * acceptLanguageList) was returned. The value on entry is ignored. ULocale will be one of the locales in availableLocales, or the ROOT
	 * ULocale if if a ROOT locale was used as a fallback (because nothing else in availableLocales matched). No ULocale array element
	 * should be null; behavior is undefined if this is the case. This function will choose a locale from the ULocale.getAvailableLocales()
	 * list as available.
	 * 
	 * @param acceptLanguageList
	...
	/**
	 * {@icu} Adds the likely subtags for a provided locale ID, per the algorithm described in the following CLDR technical report:
	 * 
	 * http://www.unicode.org/reports/tr35/#Likely_Subtags
	 * 
	 * If the provided ULocale instance is already in the maximal form, or there is no data available available for maximization, it will be
	 * returned. For example, "und-Zzzz" cannot be maximized, since there is no reasonable maximization. Otherwise, a new ULocale instance
	 * with the maximal form is returned.
	 * 
	 * Examples:
	 * 
	...
	/**
	 * {@icu} Minimizes the subtags for a provided locale ID, per the algorithm described in the following CLDR technical
	 * report:<blockquote>
	 * 
	 ...
	 * Since the minimization algorithm relies on proper maximization, see the comments for addLikelySubtags for reasons why there might not
	 * be any data.
	 * 
	 * Examples:
	 * 
	...
	public static ULocale minimizeSubtags(final ULocale loc) {
		...
		/**
		 * First, we need to first get the maximization by adding any likely subtags.
		 **/
		String maximizedLocaleID = createLikelySubtagsString(originalLang, originalScript, originalRegion, null);

		/**
		 * If maximization fails, there's nothing we can do.
		 **/
		...
		if (originalRegion.length() != 0) {
			...
			String tag = createLikelySubtagsString(originalLang, null, originalRegion, null);
		...
		/**
		 * Finally, try the language and script. This is our last chance, since trying with all three subtags would only yield the maximal
		 * version that we already have.
		 **/
		if (originalRegion.length() != 0 && originalScript.length() != 0) {

			String tag = createLikelySubtagsString(originalLang, originalScript, null, null);
	...
	/**
	 * A trivial utility function that checks for a null reference or checks the length of the supplied String.
	 * 
	 * @param string
	 *            The string to check
	 * 
	 ...
	 * 
	 * If any of the language, script or region parameters are empty, and the alternateTags parameter is not null, it will be parsed for
	 * potential language, script and region tags to be used when constructing the new tag. If the alternateTags parameter is null, or it
	 * contains no language tag, the default tag for the unknown language is used.
	 * 
	 * @param lang
	 *            The language tag to use.
	 * @param script
	 *            The script tag to use.
	 * @param region
	 *            The region tag to use.
	 * @param trailing
	 *            Any trailing data to append to the new tag.
	 * @param alternateTags
	...
	/**
	 * Create a tag string from the supplied parameters. The lang, script and region parameters may be null references.If the lang parameter
	 * is an empty string, the default value for an unknown language is written to the output buffer.
	 * 
	 * @param lang
	 *            The language tag to use.
	 * @param script
	...
	private static String lookupLikelySubtags(final String localeId) {
		UResourceBundle bundle = UResourceBundle.getBundleInstance(ICUResourceBundle.ICU_BASE_NAME, "likelySubtags");
		try {
	...
	private static String createLikelySubtagsString(final String lang, final String script, final String region, final String variants) {
		...
		if (!isEmptyString(script)) {
			...
			String searchTag = createTagString(lang, script, null, null);
		...
		if (!isEmptyString(region)) {
			...
			String searchTag = createTagString(lang, null, region, null);
	...
	/**
	 * {@icu} Returns the extension (or private use) value associated with the specified key, or null if there is no extension associated
	 * with the key. To be well-formed, the key must be one of <code>[0-9A-Za-z]</code>. Keys are case-insensitive, so for example 'z' and
	 * 'Z' represent the same extension.
	 * 
	 * @param key
	 *            the extension key
	...
	/**
	 * {@icu} Returns the set of extension keys associated with this locale, or the empty set if it has no extensions. The returned set is
	 * unmodifiable. The keys will all be lower-case.
	 * 
	...
	/**
	 * {@icu} Returns the set of unicode locale attributes associated with this locale, or the empty set if it has no attributes. The
	...
	/**
	 * {@icu} Returns the Unicode locale type associated with the specified Unicode locale key for this locale. Returns the empty string for
	 * keys that are defined with no type. Returns null if the key is not defined. Keys are case-insensitive. The key must be two
	 * alphanumeric characters ([0-9a-zA-Z]), or an IllegalArgumentException is thrown.
	 * 
	 * @param key
	 *            the Unicode locale key
	 * @return The Unicode locale type associated with the key, or null if the locale does not define the key.
	 * @throws IllegalArgumentException
	...
	/**
	 * {@icu} Returns the set of Unicode locale keys defined by this locale, or the empty set if this locale has none. The returned set is
	 * immutable. Keys are all lower case.
	 * 
	 * @return The set of Unicode locale keys, or the empty set if this locale has no Unicode locale keywords.
	 * 
	 * @stable ICU 4.4
	 */
	 ...
	 * 
	 * <p>
	 * If this <code>ULocale</code> has a language, script, country, or variant that does not satisfy the IETF BCP 47 language tag syntax
	 * requirements, this method handles these fields as described below:
	 * 
	 * <p>
	 * <b>Language:</b> If language is empty, or not well-formed (for example "a" or "e2"), it will be emitted as "und" (Undetermined).
	 * 
	 * <p>
	 * <b>Script:</b> If script is not well-formed (for example "12" or "Latin"), it will be omitted.
	 * 
	 * <p>
	 * <b>Country:</b> If country is not well-formed (for example "12" or "USA"), it will be omitted.
	 * 
	 * <p>
	 * <b>Variant:</b> If variant <b>is</b> well-formed, each sub-segment (delimited by '-' or '_') is emitted as a subtag. Otherwise:
	 * <ul>
	 * 
	 * <li>if all sub-segments match <code>[0-9a-zA-Z]{1,8}</code> (for example "WIN" or "Oracle_JDK_Standard_Edition"), the first
	 * ill-formed sub-segment and all following will be appended to the private use subtag. The first appended subtag will be "lvariant",
	 * followed by the sub-segments in order, separated by hyphen. For example, "x-lvariant-WIN", "Oracle-x-lvariant-JDK-Standard-Edition".
	 * 
	 * <li>if any sub-segment does not match <code>[0-9a-zA-Z]{1,8}</code>, the variant will be truncated and the problematic sub-segment
	 * and all following sub-segments will be omitted. If the remainder is non-empty, it will be emitted as a private use subtag as above
	 * (even if the remainder turns out to be well-formed). For example, "Solaris_isjustthecoolestthing" is emitted as "x-lvariant-Solaris",
	 * not as "solaris".</li>
	 * </ul>
	 * 
	 * <p>
	 * <b>Note:</b> Although the language tag created by this method is well-formed (satisfies the syntax requirements defined by the IETF
	 * BCP 47 specification), it is not necessarily a valid BCP 47 language tag. For example,
	 * 
	 * <pre>
	 * new Locale(&quot;xx&quot;, &quot;YY&quot;).toLanguageTag();
	 * </pre>
	 * 
	 * will return "xx-YY", but the language subtag "xx" and the region subtag "YY" are invalid because they are not registered in the IANA
	 ...
	 * 
	 * <p>
	 * If the specified language tag contains any ill-formed subtags, the first such subtag and all following subtags are ignored. Compare
	 * to {@link ULocale.Builder#setLanguageTag} which throws an exception in this case.
	 * 
	 * <p>
	 ...
	 * 
	 * <li>The portion of a private use subtag prefixed by "lvariant", if any, is removed and appended to the variant field in the result
	 * locale (without case normalization). If it is then empty, the private use subtag is discarded:
	 * 
	 * <pre>
	 *     ULocale loc;
	 ...
	 * </pre>
	 * 
	 * <li>When the languageTag argument contains an extlang subtag, the first such subtag is used as the language, and the primary language
	 * subtag and other extlang subtags are ignored:
	 * 
	 * <pre>
	 ...
	 * </pre>
	 * 
	 * <li>Case is normalized. Language is normalized to lower case, script to title case, country to upper case, variant to upper case, and
	 * extensions to lower case.
	 * 
	 * <p>
	 * This implements the 'Language-Tag' production of BCP47, and so supports grandfathered (regular and irregular) as well as private use
	 * language tags. Stand alone private use tags are represented as empty language and extension 'x-whatever', and grandfathered tags are
	 ...
	 * 
	 * <p>
	 * <b>Note</b>: there is no guarantee that <code>toLanguageTag</code> and <code>forLanguageTag</code> will round-trip.
	 * 
	 * @param languageTag
	 *            the language tag
	...
	/**
	 * <code>Builder</code> is used to build instances of <code>ULocale</code> from values configured by the setters. Unlike the
	 * <code>ULocale</code> constructors, the <code>Builder</code> checks if a value configured by a setter satisfies the syntax
	 * requirements defined by the <code>ULocale</code> class. A <code>ULocale</code> object created by a <code>Builder</code> is
	 * well-formed and can be transformed to a well-formed IETF BCP 47 language tag without losing information.
	 * 
	 * <p>
	 * <b>Note:</b> The <code>ULocale</code> class does not provide any syntactic restrictions on variant, while BCP 47 requires each
	 * variant subtag to be 5 to 8 alphanumerics or a single numeric followed by 3 alphanumerics. The method <code>setVariant</code> throws
	 * <code>IllformedLocaleException</code> for a variant that does not satisfy this restriction. If it is necessary to support such a
	 * variant, use a ULocale constructor. However, keep in mind that a <code>ULocale</code> object created this way might lose the variant
	 * information when transformed to a BCP 47 language tag.
	 * 
	 * <p>
	 * The following example shows how to create a <code>Locale</code> object with the <code>Builder</code>. <blockquote>
	 * 
	 * <pre>
	 ...
	 * 
	 * <p>
	 * Builders can be reused; <code>clear()</code> resets all fields to their default values.
	 * 
	...
	public static final class Builder {
		...
		/**
		 * Resets the <code>Builder</code> to match the provided <code>locale</code>. Existing state is discarded.
		 * 
		 * <p>
		 * All fields of the locale must be well-formed, see {@link Locale}.
		 * 
		 * <p>
		 * Locales with any ill-formed fields cause <code>IllformedLocaleException</code> to be thrown.
		 * 
		 * @param locale
		 *            the locale
		 * @return This builder.
		...
		/**
		 * Resets the Builder to match the provided IETF BCP 47 language tag. Discards the existing state. Null and the empty string cause
		 * the builder to be reset, like {@link #clear}. Grandfathered tags (see {@link ULocale#forLanguageTag}) are converted to their
		 * canonical form before being processed. Otherwise, the language tag must be well-formed (see {@link ULocale}) or an exception is
		 * thrown (unlike <code>ULocale.forLanguageTag</code>, which just discards ill-formed and following portions of the tag).
		 * 
		 * @param languageTag
		 *            the language tag
		 * @return This builder.
		...
		/**
		 * Sets the language. If <code>language</code> is the empty string or null, the language in this <code>Builder</code> is removed.
		 * Otherwise, the language must be <a href="./Locale.html#def_language">well-formed</a> or an exception is thrown.
		 * 
		 * <p>
		...
		/**
		 * Sets the script. If <code>script</code> is null or the empty string, the script in this <code>Builder</code> is removed.
		...
		/**
		 * Sets the region. If region is null or the empty string, the region in this <code>Builder</code> is removed. Otherwise, the region
		 ...
		 * 
		 * <p>
		 * The country value in the <code>Locale</code> created by the <code>Builder</code> is always normalized to upper case.
		 * 
		 * @param region
		 *            the region
		 * @return This builder.
		...
		/**
		 * Sets the variant. If variant is null or the empty string, the variant in this <code>Builder</code> is removed. Otherwise, it must
		 * consist of one or more well-formed subtags, or an exception is thrown.
		 * 
		 * <p>
		 * <b>Note:</b> This method checks if <code>variant</code> satisfies the IETF BCP 47 variant subtag's syntax requirements, and
		 * normalizes the value to lowercase letters. However, the <code>ULocale</code> class does not impose any syntactic restriction on
		...
		/**
		 * Sets the extension for the given key. If the value is null or the empty string, the extension is removed. Otherwise, the
		 * extension must be well-formed or an exception is thrown.
		 * 
		 * <p>
		 * <b>Note:</b> The key {@link ULocale#UNICODE_LOCALE_EXTENSION UNICODE_LOCALE_EXTENSION} ('u') is used for the Unicode locale
		 * extension. Setting a value for this key replaces any existing Unicode locale key/type pairs with those defined in the extension.
		 * 
		 * <p>
		 * <b>Note:</b> The key {@link ULocale#PRIVATE_USE_EXTENSION PRIVATE_USE_EXTENSION} ('x') is used for the private use code. To be
		 * well-formed, the value for this key needs only to have subtags of one to eight alphanumeric characters, not two to eight as in
		 * the general case.
		 * 
		 * @param key
		 *            the extension key
		 ...
		 * @return This builder.
		 * @throws IllformedLocaleException
		 *             if <code>key</code> is illegal or <code>value</code> is ill-formed
		...
		/**
		 * Sets the Unicode locale keyword type for the given key. If the type is null, the Unicode keyword is removed. Otherwise, the key
		 * must be non-null and both key and type must be well-formed or an exception is thrown.
		 * 
		 * <p>
		 * Keys and types are converted to lower case.
		 * 
		 * <p>
		 * <b>Note</b>:Setting the 'u' extension via {@link #setExtension} replaces all Unicode locale keywords with those defined in the
		 * extension.
		 * 
		 * @param key
		 *            the Unicode locale key
		 ...
		 * @return This builder.
		 * @throws IllformedLocaleException
		 *             if <code>key</code> or <code>type</code> is ill-formed
		 * @throws NullPointerException
		 *             if <code>key</code> is null
		...
		/**
		 * Adds a unicode locale attribute, if not already present, otherwise has no effect. The attribute must not be null and must be
		...
		/**
		 * Removes a unicode locale attribute, if present, otherwise has no effect. The attribute must not be null and must be well-formed
	...
	private static final class JDKLocaleHelper {
		...
		private static final String[][] JAVA6_MAPDATA = {
				...
				{ "ja_JP_JP", "ja_JP", "calendar", "japanese", "ja" }, { "no_NO_NY", "nn_NO", null, null, "nn" },
		...
		private static Locale toLocale6(final ULocale uloc) {
			...
			for (String[] element : JAVA6_MAPDATA) {
				if (locstr.equals(element[1]) || locstr.equals(element[4])) {
					if (element[2] != null) {
						String val = uloc.getKeywordValue(element[2]);
						if (val != null && val.equals(element[3])) {
							locstr = element[0];
							break;
						}
					} else {
		...
		public static boolean isOriginalDefaultLocale(final Locale loc) {
			if (isJava7orNewer) {
				...
				return loc.getLanguage().equals(getSystemProperty("user.language"))
						...
						&& loc.getVariant().equals(getSystemProperty("user.variant")) && script.equals(getSystemProperty("user.script"));
			}
			return loc.getLanguage().equals(getSystemProperty("user.language"))
					&& loc.getCountry().equals(getSystemProperty("user.country"))
