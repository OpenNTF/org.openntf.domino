...
/**
 * <code>Region</code> is the class representing a Unicode Region Code, also known as a Unicode Region Subtag, which is defined based upon
 * the BCP 47 standard. We often think of "regions" as "countries" when defining the characteristics of a locale. Region codes There are
 * different types of region codes that are important to distinguish.
 * <p>
 * Macroregion - A code for a "macro geographical (continental) region, geographical sub-region, or selected economic and other grouping" as
 * defined in UN M.49 (http://unstats.un.org/unsd/methods/m49/m49regin.htm). These are typically 3-digit codes, but contain some 2-letter
 * codes, such as the LDML code QO added for Outlying Oceania. Not all UNM.49 codes are defined in LDML, but most of them are. Macroregions
 ...
 * <p>
 * TERRITORY - A Region that is not a Macroregion. These are typically codes for countries, but also include areas that are not separate
 * countries, such as the code "AQ" for Antarctica or the code "HK" for Hong Kong (SAR China). Overseas dependencies of countries may or may
 * not have separate codes. The codes are typically 2-letter codes aligned with the ISO 3166 standard, but BCP47 allows for the use of
 * 3-digit codes in the future.
 * <p>
 * UNKNOWN - The code ZZ is defined by Unicode LDML for use to indicate that the Region is unknown, or that the value supplied as a region
 * was invalid.
 * <p>
 * DEPRECATED - Region codes that have been defined in the past but are no longer in modern usage, usually due to a country splitting into
 ...
 * it. Some of these are UNM.49 codes that do't fall into the world/continent/sub-continent hierarchy, while others are just well known
 * groupings that have their own region code. Region "EU" (European Union) is one such region code that is a grouping. Groupings will never
 * be returned by the getContainingRegion() API, since a different type of region ( WORLD, CONTINENT, or SUBCONTINENT ) will always be the
...
public class Region implements Comparable<Region> {
	...
	/**
	 * RegionType is an enumeration defining the different types of regions. Current possible values are WORLD, CONTINENT, SUBCONTINENT,
	 * TERRITORY, GROUPING, DEPRECATED, and UNKNOWN.
	 * 
	 * @draft ICU 50
	 * @provisional This API might change or be removed in a future release.
	 */
	...
	public enum RegionType {
		...
		SUBCONTINENT,
		/**
		 * Type representing a grouping of territories that is not to be used in the normal WORLD/CONTINENT/SUBCONTINENT/TERRITORY
		 * containment tree.
		 * 
		 * @draft ICU 50
		 * @provisional This API might change or be removed in a future release.
		...
		GROUPING,
		/**
		 * Type representing a region whose code has been deprecated, usually due to a country splitting into multiple territories or
		 * changing its name.
		 * 
		 * @draft ICU 50
		 * @provisional This API might change or be removed in a future release.
	...
	private static synchronized void loadRegionData() {
		...
		UResourceBundle rb = UResourceBundle.getBundleInstance(ICUResourceBundle.ICU_BASE_NAME, "metadata",
				ICUResourceBundle.ICU_DATA_CLASS_LOADER);
		regionCodes = rb.get("regionCodes");
		territoryAlias = rb.get("territoryAlias");

		UResourceBundle rb2 = UResourceBundle.getBundleInstance(ICUResourceBundle.ICU_BASE_NAME, "supplementalData",
				ICUResourceBundle.ICU_DATA_CLASS_LOADER);
	...
	/**
	 * Returns a Region using the given region ID. The region ID can be either a 2-letter ISO code, 3-letter ISO code, UNM.49 numeric code,
	 * or other valid Unicode Region Code as defined by the CLDR.
	 * 
	 * @param id
	 *            The id of the region to be retrieved.
	 ...
	 * 
	 * @param code
	 *            The numeric code of the region to be retrieved.
	 * @return The corresponding region.
	 * @throws IllegalArgumentException
	 ...
	 * 
	 * @param type
	 *            The type of regions to be returned ( TERRITORY, MACROREGION, etc. )
	 ...
	 * 
	 * @return The region that geographically contains this region. Returns NULL if this region is code "001" (World) or "ZZ" (Unknown
	 *         region). For example, calling this method with region "IT" (Italy) returns the region "039" (Southern Europe).
	 * @draft ICU 50
	 * @provisional This API might change or be removed in a future release.
	 */
	 ...
	 * 
	 * @return The region that geographically contains this region and matches the given type. May return NULL if no containing region can
	 *         be found that matches the given type. For example, calling this method with region "IT" (Italy) and type CONTINENT returns
	 *         the region "150" (Europe).
	 * @draft ICU 50
	 * @provisional This API might change or be removed in a future release.
	 */
	 ...
	 * 
	 * @return An unmodifiable set containing all the regions that are immediate children of this region in the region hierarchy. These
	 *         returned regions could be either macro regions, territories, or a mixture of the two, depending on the containment data as
	 *         defined in CLDR. This API may return an empty set if this region doesn't have any sub-regions. For example, calling this
	 *         method with region "150" (Europe) returns a set containing the various sub regions of Europe - "039" (Southern Europe) -
	 *         "151" (Eastern Europe) - "154" (Northern Europe) and "155" (Western Europe).
	 * 
	 * @draft ICU 50
	 * @provisional This API might change or be removed in a future release.
	 */
	 ...
	 * 
	 * @return An unmodifiable set containing all the regions that are children of this region anywhere in the region hierarchy and match
	 *         the given type. This API may return an empty set if this region doesn't have any sub-regions that match the given type. For
	 *         example, calling this method with region "150" (Europe) and type "TERRITORY" returns a set containing all the territories in
	 *         Europe ( "FR" (France) - "IT" (Italy) - "DE" (Germany) etc. )
	 * @draft ICU 50
	 * @provisional This API might change or be removed in a future release.
	 */
	...
	/**
	 * @return For deprecated regions, return an unmodifiable list of the regions that are the preferred replacement regions for this
	 *         region. Returns null for a non-deprecated region. For example, calling this method with region "SU" (Soviet Union) would
	 ...
	 * 
	 * @return The numeric code for this region. Returns UNDEFINED_NUMERIC_CODE (-1) if the given region does not have a numeric code
	 *         assigned to it. This is a very rare case and only occurs for a few very small territories.
	 * 
	 * @draft ICU 50
	 * @provisional This API might change or be removed in a future release.
	 */
