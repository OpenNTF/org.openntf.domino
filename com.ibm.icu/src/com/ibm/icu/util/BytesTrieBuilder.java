...
public final class BytesTrieBuilder extends StringTrieBuilder {
	...
	/**
	 * Adds a (byte sequence, value) pair. The byte sequence must be unique. Bytes 0..length-1 will be copied; the builder does not keep a
	 * reference to the input array.
	 * 
	 * @param sequence
	 *            The array that contains the byte sequence, starting at index 0.
	 * @param length
	 ...
	 * 
	 * <p>
	 * Multiple calls to build() or buildByteBuffer() return tries or buffers which share the builder's byte array, without rebuilding.
	 * <em>The byte array must not be modified via the buildByteBuffer() result object.</em> After clear() has been called, a new array will
	 * be used.
	 * 
	 * @param buildOption
	 ...
	 * @stable ICU 4.8
	 */
	public BytesTrie build(final StringTrieBuilder.Option buildOption) {
	 ...
	 * <p>
	 * Multiple calls to build() or buildByteBuffer() return tries or buffers which share the builder's byte array, without rebuilding.
	 * <em>Do not modify the bytes in the buffer!</em> After clear() has been called, a new array will be used.
	 * 
	 * <p>
	 ...
	 * @param buildOption
	 *            Build option, see StringTrieBuilder.Option.
	 * @return A ByteBuffer with the byte-serialized BytesTrie for the add()ed data. The buffer is not read-only and array() can be called.
	 * @stable ICU 4.8
	 */
	public ByteBuffer buildByteBuffer(final StringTrieBuilder.Option buildOption) {
		buildBytes(buildOption);
		return ByteBuffer.wrap(bytes, bytes.length - bytesLength, bytesLength);
	}

	private void buildBytes(final StringTrieBuilder.Option buildOption) {
