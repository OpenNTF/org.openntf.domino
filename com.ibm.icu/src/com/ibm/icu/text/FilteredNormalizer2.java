...
/**
 * Normalization filtered by a UnicodeSet. Normalizes portions of the text contained in the filter set and leaves portions not contained in
 * the filter set unchanged. Filtering is done via UnicodeSet.span(..., UnicodeSet.SpanCondition.SIMPLE). Not-in-the-filter text is treated
 * as "is normalized" and "quick check yes". This class implements all of (and only) the Normalizer2 API. An instance of this class is
...
public class FilteredNormalizer2 extends Normalizer2 {
	/**
	 * Constructs a filtered normalizer wrapping any Normalizer2 instance and a filter set. Both are aliased and must not be modified or
	 * deleted while this object is used. The filter set should be frozen; otherwise the performance will suffer greatly.
	 * 
	 * @param n2
	...
	public Normalizer.QuickCheckResult quickCheck(final CharSequence s) {
		...
		for (int prevSpanLimit = 0; prevSpanLimit < s.length();) {
			...
			if (spanCondition == UnicodeSet.SpanCondition.NOT_CONTAINED) {
				...
				Normalizer.QuickCheckResult qcResult = norm2.quickCheck(s.subSequence(prevSpanLimit, spanLimit));
