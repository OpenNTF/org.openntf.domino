...
public final class MessagePattern implements Cloneable, Freezable<MessagePattern> {
	/**
	 * Mode for when an apostrophe starts quoted literal text for MessageFormat output. The default is DOUBLE_OPTIONAL unless overridden via
	 * ICUConfig (/com/ibm/icu/ICUConfig.properties).
	 * <p>
	 * A pair of adjacent apostrophes always results in a single apostrophe in the output, even when the pair is between two single,
	 * text-quoting apostrophes.
	 * <p>
	...
	public enum ApostropheMode {
		/**
		 * A literal apostrophe is represented by either a single or a double apostrophe pattern character. Within a MessageFormat pattern,
		 * a single apostrophe only starts quoted literal text if it immediately precedes a curly brace {}, or a pipe symbol | if inside a
		 * choice format, or a pound symbol # if inside a plural format.
		 * <p>
		...
		DOUBLE_OPTIONAL,
		/**
		 * A literal apostrophe must be represented by a double apostrophe pattern character. A single apostrophe always starts quoted
		 * literal text.
		 * <p>
	...
	public boolean equals(Object other) {
		...
		return aposMode.equals(o.aposMode) && (msg == null ? o.msg == null : msg.equals(o.msg)) && parts.equals(o.parts);
	...
	/**
	 * Validates and parses an argument name or argument number string. An argument name must be a "pattern identifier", that is, it must
	 * contain no Unicode Pattern_Syntax or Pattern_White_Space characters. If it only contains ASCII digits, then it must be a small
	 * integer with no leading zero.
	 * 
	 * @param name
	 *            Input string.
	 * @return &gt;=0 if the name is a valid number, ARG_NAME_NOT_NUMBER (-1) if it is a "pattern identifier" but not all ASCII digits,
	...
	/**
	 * Return value from {@link #validateArgumentName(String)} for when the string is a valid "pattern identifier" but not a number.
	 * 
	 * @stable ICU 4.8
	 */
	...
	/**
	 * Return value from {@link #validateArgumentName(String)} for when the string is invalid. It might not be a valid "pattern identifier",
	...
	/**
	 * Returns a version of the parsed pattern string where each ASCII apostrophe is doubled (escaped) if it is not already, and if it is
	 * not interpreted as quoting syntax.
	 * <p>
	 * For example, this turns "I don't '{know}' {gender,select,female{h''er}other{h'im}}." into
	 ...
	 * @return The first i>start where getPart(i).getType()==ARG|MSG_LIMIT at the same nesting level, or start itself if
	...
	/**
	 * A message pattern "part", representing a pattern parsing event. There is a part for the start and end of a message or argument, for
	 * quoting and escaping of and with ASCII apostrophes, and for syntax elements of "complex" arguments.
	 * 
	 * @stable ICU 4.8
	 */
	public static final class Part {
		...
		public enum Type {
			/**
			 * Start of a message pattern (main or nested). The length is 0 for the top-level message and for a choice argument sub-message,
			...
			MSG_START,
			/**
			 * End of a message pattern (main or nested). The length is 0 for the top-level message and the last sub-message of a choice
			 * argument, otherwise 1 for the '}' or (in a choice argument style) the '|'. The value indicates the nesting level, starting
			 * with 0 for the main message.
			 * 
			 * @stable ICU 4.8
			 */
			MSG_LIMIT,
			/**
			 * Indicates a substring of the pattern string which is to be skipped when formatting. For example, an apostrophe that begins or
			 * ends quoted text would be indicated with such a part. The value is undefined and currently always 0.
			 * 
			 * @stable ICU 4.8
			 */
			SKIP_SYNTAX,
			/**
			 * Indicates that a syntax character needs to be inserted for auto-quoting. The length is 0. The value is the character code of
			 * the insertion character. (U+0027=APOSTROPHE)
			 * 
			 * @stable ICU 4.8
			 */
			INSERT_CHAR,
			/**
			 * Indicates a syntactic (non-escaped) # symbol in a plural variant. When formatting, replace this part's substring with the
			 * (value-offset) for the plural argument value. The value is undefined and currently always 0.
			 * 
			 * @stable ICU 4.8
			 */
			REPLACE_NUMBER,
			 ...
			 * <p>
			 * This part is followed by either an ARG_NUMBER or ARG_NAME, followed by optional argument sub-parts (see ArgType constants)
			...
			ARG_SELECTOR,
			/**
			 * An integer value, for example the offset or an explicit selector value in a PluralFormat style. The part value is the integer
			 * value.
			 * 
			 * @stable ICU 4.8
			 */
			ARG_INT,
			/**
			 * A numeric value, for example the offset or an explicit selector value in a PluralFormat style. The part value is an index
			 * into an internal array of numeric values; use getNumericValue().
			 * 
			 * @stable ICU 4.8
			 */
			ARG_DOUBLE;
			...
			/**
			 * Indicates whether this part has a numeric value. If so, then that numeric value can be retrieved via
			 * {@link MessagePattern#getNumericValue(Part)}.
			 * 
		...
		public boolean equals(Object other) {
			...
			Part o = (Part) other;
			return type.equals(o.type) && index == o.index && length == o.length && value == o.value && limitPartIndex == o.limitPartIndex;
		}
	 ...
	 * 
	 * Messages nested inside an argument are each delimited by MSG_START and MSG_LIMIT, with a nesting level one greater than the
	 * surrounding message.
	 * 
	 * @stable ICU 4.8
	 */
	public enum ArgType {
		...
		CHOICE,
		/**
		 * The argument is a cardinal-number PluralFormat with an optional ARG_INT or ARG_DOUBLE offset (e.g., offset:1) and one or more
		 * (ARG_SELECTOR [explicit-value] message) tuples. If the selector has an explicit value (e.g., =2), then that value is provided by
		 * the ARG_INT or ARG_DOUBLE part preceding the message. Otherwise the message immediately follows the ARG_SELECTOR.
		 * 
		 * @stable ICU 4.8
		 */
		PLURAL,
		...
		SELECT,
		/**
		 * The argument is an ordinal-number PluralFormat with the same style parts sequence and semantics as {@link ArgType#PLURAL}.
		 * 
		 * @draft ICU 50
		...
		/**
		 * @return true if the argument type has a plural style part sequence and semantics, for example {@link ArgType#PLURAL} and
		 *         {@link ArgType#SELECTORDINAL}.
		 * @draft ICU 50
		 * @provisional This API might change or be removed in a future release.
		 */
		...
		while (index < msg.length()) {
			...
			if (c == '\'') {
				if (index == msg.length()) {
					...
					} else if (aposMode == ApostropheMode.DOUBLE_REQUIRED || c == '{' || c == '}'
							|| (parentType == ArgType.CHOICE && c == '|') || (parentType.hasPluralStyle() && c == '#')) {
	...
	private int parseSimpleStyle(int index) {
		...
		while (index < msg.length()) {
			...
			if (c == '\'') {
				...
				if (index < 0) {
					throw new IllegalArgumentException("Quoted literal argument style text reaches to the end of the message: "
							+ prefix(start));
				}
				// skip the quote-ending apostrophe
				++index;
	...
	private int parseChoiceStyle(int index, int nestingLevel) {
		...
		for (;;) {
			...
			char c = msg.charAt(index);
			if (!(c == '#' || c == '<' || c == '\u2264')) {  // U+2264 is <=
				throw new IllegalArgumentException("Expected choice separator (#<\u2264) instead of '" + c + "' in choice pattern "
						+ prefix(start));
			}
		...
		for (;;) {
			...
			if (eos || msg.charAt(index) == '}') {
				if (eos == inMessageFormatPattern(nestingLevel)) {
					throw new IllegalArgumentException("Bad " + argType.toString().toLowerCase(Locale.ENGLISH) + " pattern syntax: "
							+ prefix(start));
				}
				if (!hasOther) {
					throw new IllegalArgumentException("Missing 'other' keyword in " + argType.toString().toLowerCase(Locale.ENGLISH)
							+ " pattern in " + prefix());
				}
				return index;
			}
			...
			if (argType.hasPluralStyle() && msg.charAt(selectorIndex) == '=') {
				...
				if (argType.hasPluralStyle() && length == 6 && index < msg.length() && msg.regionMatches(selectorIndex, "offset:", 0, 7)) {
					...
					if (!isEmpty) {
						throw new IllegalArgumentException("Plural argument 'offset:' (if present) must precede key-message pairs: "
								+ prefix(start));
					}
			...
			index = skipWhiteSpace(index);
			if (index == msg.length() || msg.charAt(index) != '{') {
				throw new IllegalArgumentException("No message fragment after " + argType.toString().toLowerCase(Locale.ENGLISH)
	...
	/**
	 * Validates and parses an argument name or argument number string. This internal method assumes that the input substring is a
	 * "pattern identifier".
	 * 
	 * @return &gt;=0 if the name is a valid number, ARG_NAME_NOT_NUMBER (-1) if it is a "pattern identifier" but not all ASCII digits,
	 *         ARG_NAME_NOT_VALID (-2) if it is neither.
	 ...
	 * @internal
	 */
	/* package */static void appendReducedApostrophes(String s, int start, int limit, StringBuilder sb) {
		int doubleApos = -1;
		for (;;) {
	...
	private boolean isChoice(int index) {
		char c;
		return ((c = msg.charAt(index++)) == 'c' || c == 'C') && ((c = msg.charAt(index++)) == 'h' || c == 'H')
				&& ((c = msg.charAt(index++)) == 'o' || c == 'O') && ((c = msg.charAt(index++)) == 'i' || c == 'I')
				&& ((c = msg.charAt(index++)) == 'c' || c == 'C') && ((c = msg.charAt(index)) == 'e' || c == 'E');
	}

	private boolean isPlural(int index) {
		char c;
		return ((c = msg.charAt(index++)) == 'p' || c == 'P') && ((c = msg.charAt(index++)) == 'l' || c == 'L')
				&& ((c = msg.charAt(index++)) == 'u' || c == 'U') && ((c = msg.charAt(index++)) == 'r' || c == 'R')
				&& ((c = msg.charAt(index++)) == 'a' || c == 'A') && ((c = msg.charAt(index)) == 'l' || c == 'L');
	}

	private boolean isSelect(int index) {
		char c;
		return ((c = msg.charAt(index++)) == 's' || c == 'S') && ((c = msg.charAt(index++)) == 'e' || c == 'E')
				&& ((c = msg.charAt(index++)) == 'l' || c == 'L') && ((c = msg.charAt(index++)) == 'e' || c == 'E')
				&& ((c = msg.charAt(index++)) == 'c' || c == 'C') && ((c = msg.charAt(index)) == 't' || c == 'T');
	}

	private boolean isOrdinal(int index) {
		char c;
		return ((c = msg.charAt(index++)) == 'o' || c == 'O') && ((c = msg.charAt(index++)) == 'r' || c == 'R')
				&& ((c = msg.charAt(index++)) == 'd' || c == 'D') && ((c = msg.charAt(index++)) == 'i' || c == 'I')
				&& ((c = msg.charAt(index++)) == 'n' || c == 'N') && ((c = msg.charAt(index++)) == 'a' || c == 'A')
				&& ((c = msg.charAt(index)) == 'l' || c == 'L');
	}

	/**
	 * @return true if we are inside a MessageFormat (sub-)pattern, as opposed to inside a top-level choice/plural/select pattern.
	 */
	...
	private boolean inTopLevelChoiceMessage(int nestingLevel, ArgType parentType) {
		return nestingLevel == 1 && parentType == ArgType.CHOICE && parts.get(0).type != Part.Type.MSG_START;
	}
	...
	private static final ApostropheMode defaultAposMode = ApostropheMode.valueOf(ICUConfig.get(
