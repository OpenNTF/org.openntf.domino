...
/**
 * An object that matches a fixed input string, implementing the UnicodeMatcher API. This object also implements the UnicodeReplacer API,
 * allowing it to emit the matched text as output. Since the match text may contain flexible match elements, such as UnicodeSets, the
 * emitted text is not the match pattern, but instead a substring of the actual matched text. Following convention, the output text is the
 * leftmost match seen up to this point.
 * 
 * A StringMatcher may represent a segment, in which case it has a positive segment number. This affects how the matcher converts itself to
 * a pattern but does not otherwise affect its function.
 * 
...
class StringMatcher implements UnicodeMatcher, UnicodeReplacer {
	 ...
	 * 
	 * @param theString
	 *            the pattern to be matched, possibly containing stand-ins that represent nested UnicodeMatcher objects.
	 * @param segmentNum
	 *            the segment number from 1..n, or 0 if this is not a segment.
	 * @param theData
	 ...
	 *            the pattern to be matched, possibly containing stand-ins that represent nested UnicodeMatcher objects.
	 * @param start
	 *            first character of theString to be matched
	 * @param limit
	 *            index after the last character of theString to be matched.
	 * @param segmentNum
	 *            the segment number from 1..n, or 0 if this is not a segment.
	 * @param theData
	 *            context object mapping stand-ins to UnicodeMatcher objects.
	 */
	public StringMatcher(final String theString, final int start, final int limit, final int segmentNum,
	...
	public int matches(final Replaceable text, final int[] offset, final int limit, final boolean incremental) {
		...
		if (limit < cursor[0]) {
			...
			for (i = pattern.length() - 1; i >= 0; --i) {
				...
				if (subm == null) {
					if (cursor[0] > limit && keyChar == text.charAt(cursor[0])) { // OK; see note (1) above
						--cursor[0];
					} else {
						return U_MISMATCH;
					}
