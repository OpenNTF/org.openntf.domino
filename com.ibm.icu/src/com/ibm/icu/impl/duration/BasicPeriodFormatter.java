...
class BasicPeriodFormatter implements PeriodFormatter {
	...
	private String format(int tl, boolean inFuture, int[] counts) {
		...
		if (data.useMilliseconds() != EMilliSupport.YES && (mask & (1 << TimeUnit.MILLISECOND.ordinal)) != 0) {
		...
		for (int i = first, j = i; i <= last; i = j) {
			...
			boolean mustSkip = data.appendUnit(unit, count, cv, customs.unitVariant, countSep, useDigitPrefix, multiple, isLast,
					wasSkipped, sb);
			skipped |= mustSkip;
			wasSkipped = false;
