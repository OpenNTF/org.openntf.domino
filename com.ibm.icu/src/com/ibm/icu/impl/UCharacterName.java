...
/**
 * Internal class to manage character names. Since data for names are stored in an array of char, by default indexes used in this class is
 * refering to a 2 byte count, unless otherwise stated. Cases where the index is refering to a byte count, the index is halved and depending
 * on whether the index is even or odd, the MSB or LSB of the result char at the halved index is returned. For indexes to an array of int,
 * the index is multiplied by 2, result char at the multiplied index and its following char is returned as an int. <a
 * href=../lang/UCharacter.html>UCharacter</a> acts as a public facade for this class Note : 0 - 0x1F are control characters without names
 * in Unicode 3.0
 * 
 * @author Syn Wee Quek
 * @since nov0700
...
public final class UCharacterName {
	...
	/**
	 * Retrieve the name of a Unicode code point. Depending on <code>choice</code>, the character name written into the buffer is the
	 * "modern" name or the name that was defined in Unicode version 1.0. The name contains only "invariant" characters like A-Z, 0-9,
	 * space, and '-'.
	 * 
	 * @param ch
	 ...
	 * 
	 * @param choice
	 *            selector to indicate if argument name is a Unicode 1.0 or the most current version
	 * @param name
	...
	public int getCharFromName(int choice, String name) {
		...
		if (choice >= UCharacterNameChoice.CHAR_NAME_CHOICE_COUNT || name == null || name.length() == 0) {
			return -1;
		}
	...
	/**
	 * Reads a block of compressed lengths of 32 strings and expands them into offsets and lengths for each string. Lengths are stored with
	 * a variable-width encoding in consecutive nibbles: If a nibble<0xc, then it is the length itself (0 = empty string). If a nibble>=0xc,
	 * then it forms a length value with the following nibble. The offsets and lengths arrays must be at least 33 (one more) long because
	 * there is no check here at the end if the last nibble is still used.
	 * 
	 * @param index
	 *            of group string object in array
	 * @param offsets
	 *            array to store the value of the string offsets
	 * @param lengths
	 *            array to store the value of the string length
	...
	public int getGroupLengths(int index, char offsets[], char lengths[]) {
		...
		int stringoffset = UCharacterUtility.toInt(m_groupinfo_[index + OFFSET_HIGH_OFFSET_], m_groupinfo_[index + OFFSET_LOW_OFFSET_]);
	...
	/**
	 * Gets the name of the argument group index. UnicodeData.txt uses ';' as a field separator, so no field can contain ';' as part of its
	 * contents. In unames.icu, it is marked as token[';'] == -1 only if the semicolon is used in the data file - which is iff we have
	 * Unicode 1.0 names or ISO comments or aliases. So, it will be token[';'] == -1 if we store U1.0 names/ISO comments/aliases although we
	 * know that it will never be part of a name. Equivalent to ICU4C's expandName.
	 * 
	 * @param index
	 *            of the group name string in byte count
	 * @param length
	 *            of the group name string
	 * @param choice
	 *            of Unicode 1.0 name or the most current name
	...
	public String getGroupName(int index, int length, int choice) {
		if (choice != UCharacterNameChoice.UNICODE_CHAR_NAME && choice != UCharacterNameChoice.EXTENDED_CHAR_NAME) {
			if (';' >= m_tokentable_.length || m_tokentable_[';'] == 0xFFFF) {
				...
				do {
					int oldindex = index;
					index += UCharacterUtility.skipByteSubString(m_groupstring_, index, length, (byte) ';');
				...
				if (b >= m_tokentable_.length) {
					...
					if (token == 0xFFFF) {
						if (b == ';') {
							...
							// is a 1.0 name.
							if (m_utilStringBuffer_.length() == 0 && choice == UCharacterNameChoice.EXTENDED_CHAR_NAME) {
								continue;
							}
							break;
						}
	 ...
	 * 
	 * @param msb
	 *            most significant byte of the group
	 ...
	 * 
	 * @param ch
	 *            character to get the group name
	 * @param choice
	...
	static final class AlgorithmName {
		 ...
		 * @param rangeend
		 *            end code point that lies within this name group
		 * @param type
		 *            algorithm type. There's 2 kinds of algorithmic type. First which uses code point as part of its name and the other
		 *            uses variant postfix strings
		 * @param variant
		 *            algorithmic variant
		 * @return true if values are valid
		 */
		boolean setInfo(int rangestart, int rangeend, byte type, byte variant) {
			if (rangestart >= UCharacter.MIN_VALUE && rangestart <= rangeend && rangeend <= UCharacter.MAX_VALUE
		...
		/**
		 * Appends algorithm name of code point into StringBuffer. Note this method does not check for validity of code point in Algorithm,
		 ...
		 * 
		 * @param set
		 *            int set to add the chars of the algorithm names into
		 * @param maxlength
		 *            maximum length to compare to
		 * @return the length that is either maxlength of the length of this algorithm name if it is longer than maxlength
		 */
		 ...
		 * 
		 * @param index
		 *            array with each index corresponding to each factor block
		 * @param length
	 ...
	 *            array of tokens
	 * @param tokenstring
	 *            array of string values of the tokens
	 ...
	 */
	private static final String TYPE_NAMES_[] = { "unassigned", "uppercase letter", "lowercase letter", "titlecase letter",
			"modifier letter", "other letter", "non spacing mark", "enclosing mark", "combining spacing mark", "decimal digit number",
			"letter number", "other number", "space separator", "line separator", "paragraph separator", "control", "format",
			"private use area", "surrogate", "dash punctuation", "start punctuation", "end punctuation", "connector punctuation",
			"other punctuation", "math symbol", "currency symbol", "modifier symbol", "other symbol", "initial punctuation",
			"final punctuation", "noncharacter", "lead surrogate", "trail surrogate" };
	/**
	 * Unknown type name
	 */
	 ...
	 * 
	 * @param ch
	 *            character to determine name for
	 * @param choice
	 *            name choice
	 ...
	 * 
	 * @param index
	 *            index where the set of names reside in the group block
	 * @param length
	 *            list of lengths of the strings
	 * @param name
	 *            character name to search for
	 * @param choice
	 *            of either 1.0 or the most current unicode name
	 * @return relative character in the group which matches name, otherwise if not found, -1 will be returned
	 */
	...
	private static int getType(int ch) {
		...
		if (result == ECharacterCategory.SURROGATE) {
	 ...
	 * 
	 * @param name
	 *            of the character to be found
	 * @param choice
	 *            name choice
	 * @return character associated with the name, -1 if such character is not found and -2 if we should continue with the search.
	 */
	 ...
	 * 
	 * @param set
	 *            set to add all chars of str to
	 * @param str
	 *            string to add
	 */
	 ...
	 * @return the maximum length of any possible algorithmic name if it is > maxlength, otherwise maxlength is returned.
	 */
	 ...
	 *            of the group name string
	 * @param tokenlength
	 *            array to store the length of each token
	 * @param set
	 *            to add to
	...
	/**
	 * Adds names of all group to the argument set. Sets the data member m_max*Length_. Method called only once. Equivalent to
	 ...
	 * 
	 * @param set
	 *            Set of 256 bit flags corresponding to a set of chars.
	 * @param uset
	 *            USet to receive characters. Existing contents are deleted.
	 */
