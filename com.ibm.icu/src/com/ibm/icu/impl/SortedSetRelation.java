...
public class SortedSetRelation {
	...
	/**
	 * The relationship between two sets A and B can be determined by looking at: A - B A & B (intersection) B - A These are represented by
	 * a set of bits. Bit 2 is true if A - B is not empty Bit 1 is true if A & B is not empty BIT 0 is true if B - A is not empty
	 */
	public static final int A_NOT_B = 4, A_AND_B = 2, B_NOT_A = 1;

	/**
	 * There are 8 combinations of the relationship bits. These correspond to the filters (combinations of allowed bits) in hasRelation.
	 * They also correspond to the modification functions, listed in comments.
	 */
	...
	/**
	 * Utility that could be on SortedSet. Allows faster implementation than what is in Java for doing addAll, removeAll, retainAll,
	 * (complementAll).
	 * 
	 * @param a
	 *            first set
	 * @param relation
	 *            the relation filter, using ANY, CONTAINS, etc.
	 * @param b
	 *            second set
	 * @return the new set
	 */
