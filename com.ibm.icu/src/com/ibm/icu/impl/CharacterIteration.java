...
public final class CharacterIteration {
	...
	/**
	 * Move the iterator forward to the next code point, and return that code point, leaving the iterator positioned at char returned. For
	 * Supplementary chars, the iterator is left positioned at the lead surrogate.
	 * 
	 * @param ci
	...
	public static int nextTrail32(CharacterIterator ci, int lead) {
		...
		if (lead <= UTF16.LEAD_SURROGATE_MAX_VALUE) {
			...
			if (UTF16.isTrailSurrogate(cTrail)) {
				retVal = ((lead - UTF16.LEAD_SURROGATE_MIN_VALUE) << 10) + (cTrail - UTF16.TRAIL_SURROGATE_MIN_VALUE)
	...
	public static int previous32(CharacterIterator ci) {
		...
		if (UTF16.isTrailSurrogate(trail) && ci.getIndex() > ci.getBeginIndex()) {
			...
			if (UTF16.isLeadSurrogate(lead)) {
				retVal = (((int) lead - UTF16.LEAD_SURROGATE_MIN_VALUE) << 10) + ((int) trail - UTF16.TRAIL_SURROGATE_MIN_VALUE)
						+ UTF16.SUPPLEMENTARY_MIN_VALUE;
			} else {
				ci.next();
			}
	...
	public static int current32(CharacterIterator ci) {
		...
		if (UTF16.isLeadSurrogate(lead)) {
			...
			if (UTF16.isTrailSurrogate((char) trail)) {
				retVal = ((lead - UTF16.LEAD_SURROGATE_MIN_VALUE) << 10) + (trail - UTF16.TRAIL_SURROGATE_MIN_VALUE)
						+ UTF16.SUPPLEMENTARY_MIN_VALUE;
			}
		} else {
