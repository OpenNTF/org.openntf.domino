...
public class UnicodeSetStringSpan {
	...
	/**
	 * Compare 16-bit Unicode strings (which may be malformed UTF-16) at code point boundaries. That is, each edge of a match must not be in
	 * the middle of a surrogate pair.
	 * 
	 * @param start
	 *            The start index of s.
	 * @param slength
	 *            The length of s from start.
	 * @param tlength
	 *            The length of t.
	 */
	static boolean matches16CPB(CharSequence s, int start, int slength, final String t, int tlength) {
		return !(0 < start && com.ibm.icu.text.UTF16.isLeadSurrogate(s.charAt(start - 1)) && com.ibm.icu.text.UTF16.isTrailSurrogate(s
				.charAt(start + 0)))
				&& !(tlength < slength && com.ibm.icu.text.UTF16.isLeadSurrogate(s.charAt(start + tlength - 1)) && com.ibm.icu.text.UTF16
						.isTrailSurrogate(s.charAt(start + tlength))) && matches16(s, start, t, tlength);
	}
