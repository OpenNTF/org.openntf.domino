...
public class ICULocaleService extends ICUService {
	...
	/**
	 * Convenience override for callers using locales. This calls get(ULocale, int, ULocale[]) with KIND_ANY for kind and null for
	 * actualReturn.
	 */
	...
	/**
	 * Convenience override for callers using locales. This uses createKey(ULocale.toString(), kind) to create a key, calls getKey, and then
	 * if actualReturn is not null, returns the actualResult from getKey (stripping any prefix) into a ULocale.
	 */
	...
	/**
	 * Convenience override for callers using locales. This calls registerObject(Object, ULocale, int kind, boolean visible) passing
	...
	/**
	 * Convenience function for callers using locales. This calls registerObject(Object, ULocale, int kind, boolean visible) passing true
	 * for the visibility.
	 */
	...
	/**
	 * A subclass of Key that implements a locale fallback mechanism. The first locale to search for is the locale provided by the client,
	 * and the fallback locale to search for is the current default locale. If a prefix is present, the currentDescriptor includes it before
	 * the locale proper, separated by "/". This is the default key instantiated by ICULocaleService.</p>
	 * 
	 * <p>
	 * Canonicalization adjusts the locale string so that the section before the first understore is in lower case, and the rest is in upper
	 * case, with no trailing underscores.
	 * </p>
	 */
	public static class LocaleKey extends ICUService.Key {
		...
		/**
		 * PrimaryID is the user's requested locale string, canonicalPrimaryID is this string in canonical form, fallbackID is the current
		 * default locale's string in canonical form.
		 */
		...
		/**
		 * Return the (canonical) current descriptor, or null if no current id. Includes the keywords, whereas the ID does not include
		 * keywords.
		 */
		@Override
		...
		/**
		 * If the key has a fallback, modify the key and return true, otherwise return false.</p>
		 * 
		 * <p>
		 * First falls back through the primary ID, then through the fallbackID. The final fallback is "" (root) unless the primary id was
		 * "" (root), in which case there is no fallback.
		 */
		@Override
	...
	public static abstract class LocaleKeyFactory implements Factory {
		...
		/**
		 * Implement superclass abstract method. This checks the currentID of the key against the supported IDs, and passes the
		 * canonicalLocale and kind off to handleCreate (which subclasses must implement).
		 */
		...
		/**
		 * Return the set of ids that this factory supports (visible or otherwise). This can be called often and might need to be cached if
	...
	/**
	 * A LocaleKeyFactory that creates a service based on the ICU locale data. This is a base class for most ICU factories. Subclasses
	 * instantiate it with a constructor that takes a bundle name, which determines the supported IDs. Subclasses then override handleCreate
	 * to create the actual service object. The default implementation returns a resource bundle.
	 */
	public static class ICUResourceBundleFactory extends LocaleKeyFactory {
		...
		/**
		 * Create the service. The default implementation returns the resource bundle for the locale, ignoring kind, and service.
		 */
		@Override
		protected Object handleCreate(ULocale loc, int kind, ICUService service) {
			return UResourceBundle.getBundleInstance(bundleName, loc, loader());
		}
