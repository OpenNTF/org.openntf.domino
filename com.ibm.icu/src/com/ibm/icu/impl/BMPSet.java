...
public final class BMPSet {
	...
	public final int span(CharSequence s, int start, int end, SpanCondition spanCondition) {
		...
		if (SpanCondition.NOT_CONTAINED != spanCondition) {
			...
			while (i < limit) {
				...
				} else if (c < 0xd800 || c >= 0xdc00 || (i + 1) == limit || (c2 = s.charAt(i + 1)) < 0xdc00 || c2 >= 0xe000) {
					int lead = c >> 12;
					int twoBits = (bmpBlockBits[(c >> 6) & 0x3f] >> lead) & 0x10001;
					if (twoBits <= 1) {
						// All 64 code points with the same bits 15..6
	...
	public final int spanBack(CharSequence s, int limit, SpanCondition spanCondition) {
		...
		if (SpanCondition.NOT_CONTAINED != spanCondition) {
			...
			for (;;) {
				...
				} else if (c < 0xd800 || c < 0xdc00 || 0 == limit || (c2 = s.charAt(limit - 1)) < 0xd800 || c2 >= 0xdc00) {
					int lead = c >> 12;
					int twoBits = (bmpBlockBits[(c >> 6) & 0x3f] >> lead) & 0x10001;
					if (twoBits <= 1) {
						// All 64 code points with the same bits 15..6
	 ...
	 * 
	 * For restricting the search for finding in the range start..end, pass in lo=findCodePoint(start) and hi=findCodePoint(end) with
	 * 0<=lo<=hi<len. findCodePoint(c) defaults to lo=0 and hi=len-1.
	 * 
	 * @param c
