 ...
 */
@Incomplete
public class DominoScheduledExecutor extends ScheduledThreadPoolExecutor {

	public static enum ThreadCleaner implements Runnable {
		INSTANCE;
		private Set<DominoScheduledExecutor> executors_ = Collections.synchronizedSet(new HashSet<DominoScheduledExecutor>());
		...
		public void clean() {
			...
			for (Object raw : copy) {
				try {
					if (raw instanceof DominoScheduledExecutor) {
						DominoScheduledExecutor executor = (DominoScheduledExecutor) raw;
		...
		public synchronized void addExecutor(final DominoScheduledExecutor executor) {
			executors_.add(executor);
		}

		public synchronized DominoScheduledExecutor removeExecutor(final DominoScheduledExecutor executor) {
	...
	private static final Logger log_ = Logger.getLogger(DominoScheduledExecutor.class.getName());
	...
	public DominoScheduledExecutor() {
		this(50);
	}

	public DominoScheduledExecutor(final int corePoolSize) {
		this(corePoolSize, new DominoThreadFactory());
	}

	public DominoScheduledExecutor(final int corePoolSize, final RejectedExecutionHandler paramRejectedExecutionHandler) {
		this(corePoolSize, new DominoThreadFactory(), paramRejectedExecutionHandler);
	}

	public DominoScheduledExecutor(final int corePoolSize, final DominoThreadFactory threadFactory) {
	...
	public DominoScheduledExecutor(final int corePoolSize, final DominoThreadFactory threadFactory,
		...
		super(corePoolSize, threadFactory, paramRejectedExecutionHandler);
		ThreadCleaner.INSTANCE.addExecutor(this);
		init();
	}
