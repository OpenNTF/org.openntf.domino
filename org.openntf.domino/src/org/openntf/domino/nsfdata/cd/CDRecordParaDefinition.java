...
public class CDRecordParaDefinition extends CDRecord {
	private static final long serialVersionUID = 1L;

	public CDRecordParaDefinition(final CDSignature signature, final ByteBuffer data, final int dataLength) {
	...
	public int getExtraLength() {
		...
		int result = 0;
		if((flags2 & PABFLAG2_LM_OFFSET) > 0) { result += 2; }
		if((flags2 & PABFLAG2_LM_PERCENT) > 0) { result += 2; }
		if((flags2 & PABFLAG2_FLLM_OFFSET) > 0) { result += 2; }
		if((flags2 & PABFLAG2_FLLM_PERCENT) > 0) { result += 2; }
		if((flags2 & PABFLAG2_RM_OFFSET) > 0) { result += 2; }
		if((flags2 & PABFLAG2_RM_PERCENT) > 0) { result += 2; }
	...
	public String toString() {
		return "[" + getClass().getSimpleName() + ", ID: " + getId() + ", Justify Mode: " + getJustifyMode() + ", Line Spacing: " + getLineSpacing() +
				", Paragraph Spacing Before: " + getParagraphSpacingBefore() + ", Paragraph Spacing After: " + getParagraphSpacingAfter() +
				", Left Margin: " + getLeftMargin() + ", Right Margin: " + getRightMargin() + ", First Line Left Margin: " + getFirstLineLeftMargin() +
				", Tab: " + getTab() + ", Flags: " + getFlags() + ", Tab Types: " + getTabTypes() + ", Flags2: " + getFlags2() + "]";
	}
}
