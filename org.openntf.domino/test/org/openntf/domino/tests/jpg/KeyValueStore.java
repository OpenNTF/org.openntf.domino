	...
	public void initializeDatabases() {
		if (places_ > 0) {
			for (int i = 0; i < pow(16, places_); i++) {
				...
				String server = serverStrategy_.getServerForHashChunk(hashChunk);
	...
	private DbDirectory getDbDirectoryForHashChunk(final String hashChunk) {
		if (server_ != null) {
			...
			if (!dbdirCache_.containsKey(hashChunk)) {
				dbdirCache_.put(hashChunk, session_.getDbDirectory(serverStrategy_.getServerForHashChunk(hashChunk)));
			}
