...
/**
 * Builder class to manipulate and generate a trie. This is useful for ICU data in primitive types. Provides a compact way to store
 * information that is indexed by Unicode values, such as character properties, types, keyboard values, etc. This is very useful when you
 * have a block of Unicode data that contains significant values while the rest of the Unicode data is unused in the application or when you
 * have a lot of redundance, such as where all 21,000 Han ideographs have the same value. However, lookup is much faster than a hash table.
...
public class IntTrieBuilder extends TrieBuilder {
	 ...
	 * 
	 * @param aliasdata
	 *            data to be filled into table
	 * @param maxdatalength
	 *            maximum data length allowed in table
	 * @param initialvalue
	 *            inital data value
	 * @param latin1linear
	...
	*/
	 ...
	 * 
	 * @param ch
	 *            codepoint which data is to be retrieved
	 ...
	 * 
	 * @param ch
	 *            code point for which data is to be retrieved.
	 * @param inBlockZero
	 *            Output parameter, inBlockZero[0] returns true if the char maps into block zero, otherwise false.
	 ...
	 * 
	 * @param ch
	 *            codepoint which data is to be set
	 * @param value
	 *            to set
	 ...
	 *            builder raw fold method implementation
	 * @param triedatamanipulate
	 *            result trie fold method
	 * @return a new trie
	 */
	 ...
	 * 
	 * Compacts the build-time trie after all values are set, and then writes the serialized form onto an output stream.
	 * 
	 * After this, this build-time Trie can only be serialized again and/or closed; no further values can be added.
	 * 
	 * This function is the rough equivalent of utrie_seriaize() in ICU4C.
	 * 
	 * @param os
	 *            the output stream to which the seriaized trie will be written. If nul, the function still returns the size of the
	 *            serialized Trie.
	 * @param reduceTo16Bits
	 *            If true, reduce the data size to 16 bits. The resulting serialized form can then be used to create a CharTrie.
	 * @param datamanipulate
	 *            builder raw fold method implementation
	...
	/**
	 * Set a value in a range of code points [start..limit]. All code points c with start &lt;= c &lt; limit will get the value if overwrite
	 * is true or if the old value is 0.
	 * 
	 * @param start
	 *            the first code point to get the value
	 * @param limit
	 *            one past the last code point to get the value
	 * @param value
	 *            the value
	 * @param overwrite
	 *            flag for whether old non-initial values are to be overwritten
	...
	public boolean setRange(int start, int limit, int value, boolean overwrite) {
		...
		if (m_isCompacted_ || start < UCharacter.MIN_VALUE || start > UCharacter.MAX_VALUE || limit < UCharacter.MIN_VALUE
	...
	/**
	 * Compact a folded build-time trie. The compaction - removes blocks that are identical with earlier ones - overlaps adjacent blocks as
	 * much as possible (if overlap == true) - moves blocks in steps of the data granularity - moves and overlaps blocks that overlap with
	 * multiple values in the overlap region
	 * 
	 * It does not - try to move and overlap blocks that are not already adjacent
	 * 
	 * @param overlap
	 *            flag
	 */
	private void compact(boolean overlap) {
		...
		for (int start = newStart; start < m_dataLength_;) {
			...
			if (start >= overlapStart) {
				i = findSameDataBlock(m_data_, newStart, start, overlap ? DATA_GRANULARITY_ : DATA_BLOCK_LENGTH);
				if (i >= 0) {
			...
			if (overlap && start >= overlapStart) {
				...
				for (i = DATA_BLOCK_LENGTH - DATA_GRANULARITY_; i > 0 && !equal_int(m_data_, newStart - i, start, i); i -= DATA_GRANULARITY_) {
				}
			} else {
				i = 0;
			}
	...
	/**
	 * Fold the normalization data for supplementary code points into a compact area on top of the BMP-part of the trie index, with the lead
	 * surrogates indexing this compact area.
	 * 
	 * Duplicate the index values for lead surrogates: From inside the BMP area, where some may be overridden with folded values, to just
	 * after the BMP area, where they can be retrieved for code point lookups.
	 * 
	 * @param manipulate
	 *            fold implementation
	 */
