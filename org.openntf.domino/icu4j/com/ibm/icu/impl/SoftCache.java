 ...
 * the createInstance() method, and call get() with the key and the data. The get() call will use the data only if it needs to call
 * createInstance(), otherwise the data is ignored.
 * 
 * By using SoftReferences to instances, the Java runtime can release instances once they are not used any more at all. If such an instance
 * is then requested again, the get() method will call createInstance() again and also create a new SoftReference. The cache holds on to its
 * map of keys to SoftReferenced instances forever.
 * 
 * @param <K>
 *            Cache lookup key type
 * @param <V>
...
public abstract class SoftCache<K, V, D> extends CacheBase<K, V, D> {
	...
	/**
	 * Value type for cache items: Has a SoftReference which can be set to a new value when the SoftReference has been cleared. The
	 * SoftCache class sometimes accesses the ref field directly.
	 * 
	 * @param <V>
	 *            Cache instance value type
	 */
	private static final class SettableSoftReference<V> {
		...
		/**
		 * If the SoftReference has been cleared, then this replaces it with a new SoftReference for the new value and returns the new
		 * value; otherwise returns the current SoftReference's value.
		 * 
		 * @param value
		 *            Replacement value, for when the current reference has been cleared
