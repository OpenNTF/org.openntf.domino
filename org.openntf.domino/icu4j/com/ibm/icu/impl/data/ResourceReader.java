...
/**
 * A reader for text resource data in the current package or the package of a given class object. The resource data is loaded through the
 * class loader, so it will typically be a file in the same directory as the *.class files, or a file within a JAR file in the corresponding
 * subdirectory. The file must be a text file in one of the supported encodings; when the resource is opened by constructing a
 * <code>ResourceReader</code> object the encoding is specified.
 * 
 * @author Alan Liu
 */
public class ResourceReader {
	...
	/**
	 * The one-based line number. Has the special value -1 before the object is initialized. Has the special value 0 after initialization
	 ...
	 * @param encoding
	 *            the encoding of the text file; if unsupported an exception is thrown
	 * @exception UnsupportedEncodingException
	 *                if <code>encoding</code> is not supported by the JDK.
	 */
	...
	/**
	 * Read and return the next line of the file or <code>null</code> if the end of the file has been reached.
	 */
	 ...
	 * 
	 * @param trim
	 *            if true then trim leading Pattern_White_Space.
	 */
	...
	/**
	 * Return the one-based line number of the last line returned by readLine() or readLineSkippingComments(). Should only be called after a
	 * call to one of these methods; otherwise the return value is undefined.
	 */
	...
	/**
	 * Reset this reader so that the next call to <code>readLine()</code> returns the first line of the file again. This is a somewhat
	 * expensive call, however, calling <code>reset()</code> after calling it the first time does nothing if <code>readLine()</code> has not
	...
	/**
	 * Reset to the start by reconstructing the stream and readers. We could also use mark() and reset() on the stream or reader, but that
	 * would cause them to keep the stream data around in memory. We don't want that because some of the resource files are large, e.g.,
	 * 400k.
	 */
