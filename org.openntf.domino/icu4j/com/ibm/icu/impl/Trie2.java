...
/**
 * This is the interface and common implementation of a Unicode Trie2. It is a kind of compressed table that maps from Unicode code points
 * (0..0x10ffff) to 16- or 32-bit integer values. It works best when there are ranges of characters with the same value, which is generally
 * the case with Unicode character properties.
 * 
...
public abstract class Trie2 implements Iterable<Trie2.Range> {
	 ...
	 * 
	 * To obtain the width of the Trie2, check the actual class type of the returned Trie2. Or use the createFromSerialized() function of
	 * Trie2_16 or Trie2_32, which will return only Tries of their specific type/size.
	 * 
	 ...
	 * 
	 * @param is
	 *            an InputStream containing the serialized form of a UTrie, version 1 or 2. The stream must support mark() and reset(). The
	 *            position of the input stream will be left unchanged.
	 * @param littleEndianOk
	 *            If FALSE, only big-endian (Java native) serialized forms are recognized. If TRUE, little-endian serialized forms are
	 *            recognized as well.
	 * @return the Trie version of the serialized form, or 0 if it is not recognized as a serialized UTrie
	 * @throws IOException
	 ...
	 * 
	 * A Trie2 stores two distinct values for input in the lead surrogate range, one for lead surrogates, which is the value that will be
	 * returned by this function, and a second value that is returned by Trie2.get().
	 * 
	 * For code units outside of the lead surrogate range, this function returns the same result as Trie2.get().
	 * 
	 * This function, together with the alternate value for lead surrogates, makes possible very efficient processing of UTF-16 strings
	 * without first converting surrogate pairs to their corresponding 32 bit code point values.
	 * 
	 * At build-time, enumerate the contents of the Trie2 to see if there is non-trivial (non-initialValue) data for any of the
	 * supplementary code points associated with a lead surrogate. If so, then set a special (application-specific) value for the lead
	 * surrogate code _unit_, with Trie2Writable.setForLeadSurrogateCodeUnit().
	 * 
	 * At runtime, use Trie2.getFromU16SingleLead(). If there is non-trivial data and the code unit is a lead surrogate, then check if a
	 * trail surrogate follows. If so, assemble the supplementary code point and look up its value with Trie2.get(); otherwise reset the
	 * lead surrogate's value or do a code point lookup for it.
	 * 
	 * If there is only trivial data for lead and trail surrogates, then processing can often skip them. For example, in normalization or
	 * case mapping all characters that do not have any mappings are simply copied as is.
	 * 
	 * @param c
	 *            the code point or lead surrogate value.
	 * @return the value
	 */
	abstract public int getFromU16SingleLead(char c);

	/**
	 * Equals function. Two Tries are equal if their contents are equal. The type need not be the same, so a Trie2Writable will be equal to
	 ...
	 * contiguous range of codepoints having the same value.
	 * 
	 * When iterating, the same Trie2EnumRange object will be reused and returned for each range. If you need to retain complete iteration
	 * results, clone each returned Trie2EnumRange, or save the range in some other way, before advancing to the next iteration step.
	 */
	public static class Range {
		...
		public boolean equals(Object other) {
			...
			return this.startCodePoint == tother.startCodePoint && this.endCodePoint == tother.endCodePoint && this.value == tother.value
	...
	/**
	 * Create an iterator over the value ranges in this Trie2. Values from the Trie2 are not remapped or filtered, but are returned as they
	...
	/**
	 * Create an iterator over the value ranges from this Trie2. Values from the Trie2 are passed through a caller-supplied remapping
	 * function, and it is the remapped values that determine the ranges that will be produced by the iterator.
	 * 
	 * 
	 * @param mapper
	 *            provides a function to remap values obtained from the Trie2.
	 * @return an Iterator
	 */
	...
	/**
	 * Create an iterator over the Trie2 values for the 1024=0x400 code points corresponding to a given lead surrogate. For example, for the
	 * lead surrogate U+D87E it will enumerate the values for [U+2F800..U+2FC00[. Used by data builder code that sets special lead surrogate
	 * code unit values for optimized UTF-16 string processing.
	 * 
	...
	/**
	 * When iterating over the contents of a Trie2, an instance of TrieValueMapper may be used to remap the values from the Trie2. The
	 * remapped values will be used both in determining the ranges of codepoints and as the value to be returned for each range.
	 * 
	 * Example of use, with an anonymous subclass of TrieValueMapper:
	 * 
	 * 
	 * ValueMapper m = new ValueMapper() { int map(int in) {return in & 0x1f;}; } for (Iterator<Trie2EnumRange> iter = trie.iterator(m);
	 * i.hasNext(); ) { Trie2EnumRange r = i.next(); ... // Do something with the range r. }
	 * 
	 */
	...
	/**
	 * Serialize a trie2 Header and Index onto an OutputStream. This is common code used for both the Trie2_16 and Trie2_32 serialize
	 * functions.
	 * 
	 * @param dos
	...
	/**
	 * Struct-like class for holding the results returned by a UTrie2 CharSequence iterator. The iteration walks over a CharSequence, and
	 * for each Unicode code point therein returns the character and its associated Trie2 value.
	 */
	 ...
	 * 
	 * @param text
	 *            A text string to be iterated over.
	 * @param index
	 *            The starting iteration position within the input text.
	...
	/**
	 * An iterator that operates over an input CharSequence, and for each Unicode code point in the input returns the associated value from
	 * the Trie2.
	 * 
	 * The iterator can move forwards or backwards, and can be reset to an arbitrary index.
	 * 
	 * Note that Trie2_16 and Trie2_32 subclass Trie2.CharSequenceIterator. This is done only for performance reasons. It does require that
	 * any changes made here be propagated into the corresponding code in the subclasses.
	 */
