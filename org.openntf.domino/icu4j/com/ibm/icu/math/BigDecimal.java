...
/**
 * The <code>BigDecimal</code> class implements immutable arbitrary-precision decimal numbers. The methods of the <code>BigDecimal</code>
 * class provide operations for fixed and floating point arithmetic, comparison, format conversions, and hashing.
 * <p>
 * As the numbers are decimal, there is an exact correspondence between an instance of a <code>BigDecimal</code> object and its
 * <code>String</code> representation; the <code>BigDecimal</code> class provides direct conversions to and from <code>String</code> and
 * character array (<code>char[]</code>) objects, as well as conversions to and from the Java primitive types (which may not be exact) and
 * <code>BigInteger</code>.
 * <p>
 * In the descriptions of constructors and methods in this documentation, the value of a <code>BigDecimal</code> number object is shown as
 * the result of invoking the <code>toString()</code> method on the object. The internal representation of a decimal number is neither
 * defined nor exposed, and is not permitted to affect the result of any operation.
 * <p>
 ...
 * <p>
 * Operations on <code>BigDecimal</code> numbers are controlled by a {@link MathContext} object, which provides the context (precision and
 * other information) for the operation. Methods that can take a <code>MathContext</code> parameter implement the standard arithmetic
 * operators for <code>BigDecimal</code> objects and are known as <i>operator methods</i>. The default settings provided by the constant
 * {@link MathContext#DEFAULT} (<code>digits=9,
 * form=SCIENTIFIC, lostDigits=false, roundingMode=ROUND_HALF_UP</code>) perform general-purpose floating point arithmetic to nine digits of
 * precision. The <code>MathContext</code> parameter must not be <code>null</code>.
 * <p>
 * Each operator method also has a version provided which does not take a <code>MathContext</code> parameter. For this version of each
 * method, the context settings used are <code>digits=0,
 * form=PLAIN, lostDigits=false, roundingMode=ROUND_HALF_UP</code>; these settings perform fixed point arithmetic with unlimited precision,
 * as defined for the original BigDecimal class in Java 1.1 and Java 1.2.
 * <p>
 * For monadic operators, only the optional <code>MathContext</code> parameter is present; the operation acts upon the current object.
 * <p>
 * For dyadic operators, a <code>BigDecimal</code> parameter is always present; it must not be <code>null</code>. The operation acts with
 * the current object being the left-hand operand and the <code>BigDecimal</code> parameter being the right-hand operand.
 * <p>
 * For example, adding two <code>BigDecimal</code> objects referred to by the names <code>award</code> and <code>extra</code> could be
 * written as any of:
 * <p>
 * <code>
 *     award.add(extra)
 ...
 * </code>
 * <p>
 * (where <code>acontext</code> is a <code>MathContext</code> object), which would return a <code>BigDecimal</code> object whose value is
 * the result of adding <code>award</code> and <code>extra</code> under the appropriate context settings.
 * <p>
 * When a <code>BigDecimal</code> operator method is used, a set of rules define what the result will be (and, by implication, how the
 * result would be represented as a character string). These rules are defined in the BigDecimal arithmetic documentation (see the URL
 * above), but in summary:
 * <ul>
 * <li>Results are normally calculated with up to some maximum number of significant digits. For example, if the <code>MathContext</code>
 * parameter for an operation were <code>MathContext.DEFAULT</code> then the result would be rounded to 9 digits; the division of 2 by 3
 * would then result in 0.666666667. <br>
 * You can change the default of 9 significant digits by providing the method with a suitable <code>MathContext</code> object. This lets you
 * calculate using as many digits as you need -- thousands, if necessary. Fixed point (scaled) arithmetic is indicated by using a
 * <code>digits</code> setting of 0 (or omitting the <code>MathContext</code> parameter). <br>
 * Similarly, you can change the algorithm used for rounding from the default "classic" algorithm.
 * <li>
 * In standard arithmetic (that is, when the <code>form</code> setting is not <code>PLAIN</code>), a zero result is always expressed as the
 * single digit <code>'0'</code> (that is, with no sign, decimal point, or exponent part).
 * <li>
 * Except for the division and power operators in standard arithmetic, trailing zeros are preserved (this is in contrast to binary floating
 * point operations and most electronic calculators, which lose the information about trailing zeros in the fractional part of results). <br>
 * So, for example:
 * <p>
 * <code>
 ...
 * </code>
 * <p>
 * where the value on the right of the <code>=&gt;</code> would be the result of the operation, expressed as a <code>String</code>, and
 * <code>def</code> (in this and following examples) refers to <code>MathContext.DEFAULT</code> ). This preservation of trailing zeros is
 * desirable for most calculations (including financial calculations). If necessary, trailing zeros may be easily removed using division by
 * 1.
 * <li>
 * In standard arithmetic, exponential form is used for a result depending on its value and the current setting of <code>digits</code> (the
 * default is 9 digits). If the number of places needed before the decimal point exceeds the <code>digits</code> setting, or the absolute
 * value of the number is less than <code>0.000001</code>, then the number will be expressed in exponential notation; thus
 * <p>
 * <code>
 ...
 * <p>
 * The form of the exponential notation (scientific or engineering) is determined by the <code>form</code> setting. <eul>
 * <p>
 * The names of methods in this class follow the conventions established by <code>java.lang.Number</code>, <code>java.math.BigInteger</code>
...
public class BigDecimal extends java.lang.Number implements java.io.Serializable, java.lang.Comparable<BigDecimal> {
	 ...
	 * </ul>
	 * <p>
	 * This property is an optimization; it allows us to defer number layout until it is actually needed as a string, hence avoiding
	 ...
	 * <p>
	 * If the first byte is 0 then the value of the number is zero (and mant.length=1, except when constructed from a plain number, for
	 * example, 0.000).
	 * 
	 * @serial
	 */
	 ...
	 * <p>
	 * Constructs a <code>BigDecimal</code> as though the parameter had been represented as a <code>String</code> (using its
	 * <code>toString</code> method) and the {@link #BigDecimal(java.lang.String)} constructor had then been used. The parameter must not be
	 * <code>null</code>.
	 * <p>
	 * <i>(Note: this constructor is provided only in the <code>com.ibm.icu.math</code> version of the BigDecimal class. It would not be
	 * present in a <code>java.math</code> version.)</i>
	 * 
	 * @param bd
	 ...
	 * <p>
	 * Constructs a <code>BigDecimal</code> which is the exact decimal representation of the <code>BigInteger</code>, with a scale of zero.
	 * The value of the <code>BigDecimal</code> is identical to the value of the <code>BigInteger
	 ...
	 * <p>
	 * Constructs a <code>BigDecimal</code> which is the exact decimal representation of the <code>BigInteger</code>, scaled by the second
	 * parameter, which may not be negative. The value of the <code>BigDecimal</code> is the <code>
	 * BigInteger</code> divided by ten to the power of the scale. The <code>BigInteger</code> parameter must not be <code>null</code>.
	 * <p>
	 ...
	 * <p>
	 * Constructs a <code>BigDecimal</code> as though a <code>String</code> had been constructed from the character array and the
	 * {@link #BigDecimal(java.lang.String)} constructor had then been used. The parameter must not be <code>null</code>.
	 * <p>
	 * Using this constructor is faster than using the <code>BigDecimal(String)</code> constructor if the string is already available in
	 ...
	 * Constructs a <code>BigDecimal</code> object from an array of characters.
	 * <p>
	 * Constructs a <code>BigDecimal</code> as though a <code>String</code> had been constructed from the character array (or a subarray of
	 * that array) and the {@link #BigDecimal(java.lang.String)} constructor had then been used. The first parameter must not be
	 * <code>null</code>, and the subarray must be wholly contained within it.
	 * <p>
	 * Using this constructor is faster than using the <code>BigDecimal(String)</code> constructor if the string is already available within
	 * a character array.
	 * 
	 * @param inchars
	 *            The <code>char[]</code> array containing the number to be converted.
	 ...
	 * <p>
	 * Constructs a <code>BigDecimal</code> which is the exact decimal representation of the 64-bit signed binary floating point parameter.
	 * <p>
	 ...
	 * {@link #BigDecimal(java.lang.String)} constructor. To get that result, use the static {@link #valueOf(double)} method to construct a
	 * <code>BigDecimal</code> from a <code>double</code>.
	 * 
	 * @param num
	 ...
	 * <p>
	 * Constructs a <code>BigDecimal</code> which is the exact decimal representation of the 32-bit signed binary integer parameter. The
	 * <code>BigDecimal</code> will contain only decimal digits, prefixed with a leading minus sign (hyphen) if the parameter is negative. A
	 * leading zero will be present only if the parameter is zero.
	 * 
	 * @param num
	 ...
	 * <p>
	 * Constructs a <code>BigDecimal</code> which is the exact decimal representation of the 64-bit signed binary integer parameter. The
	 * <code>BigDecimal</code> will contain only decimal digits, prefixed with a leading minus sign (hyphen) if the parameter is negative. A
	 * leading zero will be present only if the parameter is zero.
	 * 
	 * @param num
	 ...
	 * <p>
	 * Constructs a <code>BigDecimal</code> from the parameter, which must not be <code>null</code> and must represent a valid
	 * <i>number</i>, as described formally in the documentation referred to {@link BigDecimal above}.
	 * <p>
	 * In summary, numbers in <code>String</code> form must have at least one digit, may have a leading sign, may have a decimal point, and
	 * exponential notation may be used. They follow conventional syntax, and may not contain blanks.
	 * <p>
	 ...
	 * </pre>
	 * <p>
	 * (Exponential notation means that the number includes an optional sign and a power of ten following an '</code>E</code>' that
	 * indicates how the decimal point will be shifted. Thus the <code>"4E+9"</code> above is just a short way of writing
	 * <code>4000000000</code>, and the <code>"0.73e-7"</code> is short for <code>
	 * 0.000000073</code>.)
	 * <p>
	 * The <code>BigDecimal</code> constructed from the String is in a standard form, with no blanks, as though the {@link #add(BigDecimal)}
	 * method had been used to add zero to the number with unlimited precision. If the string uses exponential notation (that is, includes
	 * an <code>e</code> or an <code>E</code>), then the <code>BigDecimal
	 * </code> number will be expressed in scientific notation (where the power of ten is adjusted so there is a single non-zero digit to
	 * the left of the decimal point); in this case if the number is zero then it will be expressed as the single digit 0, and if non-zero
	 * it will have an exponent unless that exponent would be 0. The exponent must fit in nine digits both before and after it is expressed
	 * in scientific notation.
	 * <p>
	 * Any digits in the parameter must be decimal; that is, <code>Character.digit(c, 10)</code> (where </code>c</code> is the character in
	 * question) would not return -1.
	 * 
	 * @param string
	 ...
	 * <p>
	 * If the current object is zero or positive, then the same result as invoking the {@link #plus(MathContext)} method with the same
	 * parameter is returned. Otherwise, the same result as invoking the {@link #negate(MathContext)} method with the same parameter is
	 * returned.
	 * 
	 * @param set
	 ...
	 * <p>
	 * The same as {@link #add(BigDecimal, MathContext)}, where the <code>BigDecimal</code> is <code>rhs</code>, and the context is
	 ...
	 * <p>
	 * Implements the addition (<b><code>+</code></b>) operator (as defined in the decimal documentation, see {@link BigDecimal class
	 ...
	 * <p>
	 * The same as {@link #compareTo(BigDecimal, MathContext)}, where the <code>BigDecimal</code> is <code>rhs</code>, and the context is
	 * <code>new MathContext(0, MathContext.PLAIN)</code>.
	 * 
	 * @param rhs
	 *            The <code>BigDecimal</code> for the right hand side of the comparison.
	 * @return An <code>int</code> whose value is -1, 0, or 1 as <code>this</code> is numerically less than, equal to, or greater than
	 *         <code>rhs</code>.
	 * @stable ICU 2.0
	 */
	 ...
	 * <p>
	 * Implements numeric comparison, (as defined in the decimal documentation, see {@link BigDecimal class header}), and returns a result
	 ...
	 * Returns a plain <code>BigDecimal</code> whose value is <code>this/rhs</code>, using fixed point arithmetic.
	 * <p>
	 * The same as {@link #divide(BigDecimal, int)}, where the <code>BigDecimal</code> is <code>rhs</code>, and the rounding mode is
	 * {@link MathContext#ROUND_HALF_UP}.
	 * 
	 * The length of the decimal part (the scale) of the result will be the same as the scale of the current object, if the latter were
	...
	/**
	 * Returns a plain <code>BigDecimal</code> whose value is <code>this/rhs</code>, using fixed point arithmetic and a rounding mode.
	 * <p>
	 * The same as {@link #divide(BigDecimal, int, int)}, where the <code>BigDecimal</code> is <code>rhs</code>, and the second parameter is
	 * <code>this.scale()</code>, and the third is <code>round</code>.
	 * <p>
	 * The length of the decimal part (the scale) of the result will therefore be the same as the scale of the current object, if the latter
	 ...
	 * @return A plain <code>BigDecimal</code> whose value is <code>this/rhs</code>, using fixed point arithmetic and the specified rounding
	 *         mode.
	 ...
	 *             if <code>rhs</code> is zero.
	 * @throws ArithmeticException
	 *             if <code>round</code> is {@link MathContext#ROUND_UNNECESSARY} and <code>this.scale()</code> is insufficient to represent
	 *             the result exactly.
	 * @stable ICU 2.0
	 */
	...
	/**
	 * Returns a plain <code>BigDecimal</code> whose value is <code>this/rhs</code>, using fixed point arithmetic and a given scale and
	 * rounding mode.
	 * <p>
	 * The same as {@link #divide(BigDecimal, MathContext)}, where the <code>BigDecimal</code> is <code>rhs</code>,
	 * <code>new MathContext(0, MathContext.PLAIN, false, round)</code>, except that the length of the decimal part (the scale) to be used
	 * for the result is explicit rather than being taken from <code>this</code>.
	 * <p>
	 * The length of the decimal part (the scale) of the result will be the same as the scale of the current object, if the latter were
	 * formatted without exponential notation.
	 * <p>
	 ...
	 * @throws ArithmeticException
	 *             if <code>round</code> is {@link MathContext#ROUND_UNNECESSARY} and <code>scale</code> is insufficient to represent the
	 *             result exactly.
	 * @stable ICU 2.0
	 */
	 ...
	 * Returns a <code>BigDecimal</code> whose value is <code>this/rhs</code>.
	 * <p>
	 * Implements the division (<b><code>/</code></b>) operator (as defined in the decimal documentation, see {@link BigDecimal class
	 * header}), and returns the result as a <code>BigDecimal</code> object.
	 * 
	 * @param rhs
	 *            The <code>BigDecimal</code> for the right hand side of the division.
	 ...
	 * <p>
	 * Implements the integer division operator (as defined in the decimal documentation, see {@link BigDecimal class header}), and returns
	 * the result as a <code>BigDecimal</code> object.
	 * 
	 * @param rhs
	 *            The <code>BigDecimal</code> for the right hand side of the integer division.
	 * @param set
	 ...
	 * <p>
	 * The same as {@link #max(BigDecimal, MathContext)}, where the <code>BigDecimal</code> is <code>rhs</code>, and the context is
	 * <code>new MathContext(0, MathContext.PLAIN)</code>.
	 * 
	 * @param rhs
	 *            The <code>BigDecimal</code> for the right hand side of the comparison.
	 ...
	 * </code> or <code>0</code>, then the result of calling the {@link #plus(MathContext)} method on the current object (using the same
	 * <code>MathContext</code> parameter) is returned. Otherwise, the result of calling the {@link #plus(MathContext)} method on the first
	 * parameter object (using the same <code>MathContext</code> parameter) is returned.
	 * 
	 * @param rhs
	 *            The <code>BigDecimal</code> for the right hand side of the comparison.
	 * @param set
	 ...
	 * <p>
	 * The same as {@link #min(BigDecimal, MathContext)}, where the <code>BigDecimal</code> is <code>rhs</code>, and the context is
	 * <code>new MathContext(0, MathContext.PLAIN)</code>.
	 * 
	 * @param rhs
	 *            The <code>BigDecimal</code> for the right hand side of the comparison.
	 ...
	 * <code>new MathContext(0, MathContext.PLAIN)</code>.
	 * <p>
	 * The length of the decimal part (the scale) of the result will be the sum of the scales of the operands, if they were formatted
	 ...
	 * Returns a <code>BigDecimal</code> whose value is <code>this*rhs</code>.
	 * <p>
	 * Implements the multiplication (<b><code> </code></b>) operator (as defined in the decimal documentation, see {@link BigDecimal class
	 * header}), and returns the result as a <code>BigDecimal</code> object.
	 * 
	 * @param rhs
	 *            The <code>BigDecimal</code> for the right hand side of the multiplication.
	 ...
	 * <p>
	 * Implements the negation (Prefix <b><code>-</code></b>) operator (as defined in the decimal documentation, see {@link BigDecimal class
	 * header}), and returns the result as a <code>BigDecimal</code> object.
	 * 
	 * @param set
	 *            The <code>MathContext</code> arithmetic settings.
	...
	/**
	 * Returns a plain <code>BigDecimal</code> whose value is <code>+this</code>. Note that <code>this</code> is not necessarily a plain
	 * <code>BigDecimal</code>, but the result will always be.
	 * <p>
	 ...
	 * Returns a <code>BigDecimal</code> whose value is <code>+this</code>.
	 * <p>
	 * Implements the plus (Prefix <b><code>+</code></b>) operator (as defined in the decimal documentation, see {@link BigDecimal class
	 * header}), and returns the result as a <code>BigDecimal</code> object.
	 * <p>
	 ...
	 * Returns a plain <code>BigDecimal</code> whose value is <code>this**rhs</code>, using fixed point arithmetic.
	 * <p>
	 * The same as {@link #pow(BigDecimal, MathContext)}, where the <code>BigDecimal</code> is <code>rhs</code>, and the context is
	 * <code>new MathContext(0, MathContext.PLAIN)</code>.
	 * <p>
	 * The parameter is the power to which the <code>this</code> will be raised; it must be in the range 0 through 999999999, and must have
	 * a decimal part of zero. Note that these restrictions may be removed in the future, so they should not be used as a test for a whole
	 * number.
	 * <p>
	 * In addition, the power must not be negative, as no <code>MathContext</code> is used and so the result would then always be 0.
	 * 
	 * @param rhs
	 ...
	 * Returns a <code>BigDecimal</code> whose value is <code>this**rhs</code>.
	 * <p>
	 * Implements the power (<b><code> </code></b>) operator (as defined in the decimal documentation, see {@link BigDecimal class header}),
	 * and returns the result as a <code>BigDecimal</code> object.
	 * <p>
	 * The first parameter is the power to which the <code>this</code> will be raised; it must be in the range -999999999 through 999999999,
	 * and must have a decimal part of zero. Note that these restrictions may be removed in the future, so they should not be used as a test
	 * for a whole number.
	 * <p>
	 * If the <code>digits</code> setting of the <code>MathContext</code> parameter is 0, the power must be zero or positive.
	 * 
	 * @param rhs
	 *            The <code>BigDecimal</code> for the right hand side of the operation (the power).
	 * @param set
	...
	/**
	 * Returns a plain <code>BigDecimal</code> whose value is the remainder of <code>this/rhs</code>, using fixed point arithmetic.
	 * <p>
	 * The same as {@link #remainder(BigDecimal, MathContext)}, where the <code>BigDecimal</code> is <code>rhs</code>, and the context is
	 * <code>new MathContext(0, MathContext.PLAIN)</code>.
	 * <p>
	 ...
	 * @return A <code>BigDecimal</code> whose value is the remainder of <code>this/rhs</code>, using fixed point arithmetic.
	 * @throws ArithmeticException
	 *             if <code>rhs</code> is zero.
	 * @stable ICU 2.0
	 */
	 ...
	 * <p>
	 * Implements the remainder operator (as defined in the decimal documentation, see {@link BigDecimal class header}), and returns the
	 * result as a <code>BigDecimal</code> object.
	 * <p>
	 * This is not the modulo operator -- the result may be negative.
	 * 
	 * @param rhs
	 ...
	 * Returns a plain <code>BigDecimal</code> whose value is <code>this-rhs</code>, using fixed point arithmetic.
	 * <p>
	 * The same as {@link #subtract(BigDecimal, MathContext)}, where the <code>BigDecimal</code> is <code>rhs</code>, and the context is
	 * <code>new MathContext(0, MathContext.PLAIN)</code>.
	 * <p>
	 * The length of the decimal part (the scale) of the result will be the maximum of the scales of the two operands.
	 * 
	 ...
	 * Returns a <code>BigDecimal</code> whose value is <code>this-rhs</code>.
	 * <p>
	 * Implements the subtraction (<b><code>-</code></b>) operator (as defined in the decimal documentation, see {@link BigDecimal class
	 * header}), and returns the result as a <code>BigDecimal</code> object.
	 * 
	 * @param rhs
	 *            The <code>BigDecimal</code> for the right hand side of the subtraction.
	...
	/**
	 * Converts this <code>BigDecimal</code> to a <code>byte</code>. If the <code>BigDecimal</code> has a non-zero decimal part or is out of
	 * the possible range for a <code>byte</code> (8-bit signed integer) result then an <code>
	...
	/**
	 * Converts this <code>BigDecimal</code> to a <code>double</code>. If the <code>BigDecimal</code> is out of the possible range for a
	 * <code>double</code> (64-bit signed floating point) result then an <code>ArithmeticException
	 * </code> is thrown.
	 * <p>
	 * The double produced is identical to result of expressing the <code>BigDecimal</code> as a <code>String</code> and then converting it
	 * using the <code>Double(String)</code> constructor; this can result in values of <code>
	 ...
	 * <p>
	 * If the parameter is <code>null</code>, or is not an instance of the BigDecimal type, or is not exactly equal to the current
	 * <code>BigDecimal</code> object, then <i>false</i> is returned. Otherwise, <i>true</i> is returned.
	 * <p>
	 * "Exactly equal", here, means that the <code>String</code> representations of the <code>BigDecimal</code> numbers are identical (they
	 * have the same characters in the same sequence).
	 * <p>
	 ...
	 * @return A <code>boolean</code> whose value <i>true</i> if and only if the operands have identical string representations.
	...
	/**
	 * Converts this <code>BigDecimal</code> to a <code>float</code>. If the <code>BigDecimal</code> is out of the possible range for a
	 * <code>float</code> (32-bit signed floating point) result then an <code>ArithmeticException
	 * </code> is thrown.
	 * <p>
	 * The float produced is identical to result of expressing the <code>BigDecimal</code> as a <code>String</code> and then converting it
	 * using the <code>Float(String)</code> constructor; this can result in values of <code>
	 ...
	 * </code>, that can apply locale-sensitive editing of the result. The level of formatting that it provides is a necessary part of the
	 * BigDecimal class as it is sensitive to and must follow the calculation and rounding rules for BigDecimal arithmetic. However, if the
	 * function is provided elsewhere, it may be removed from this class. </i>
	 * <p>
	 * The parameters, for both forms of the <code>format</code> method are all of type <code>int</code>. A value of -1 for any parameter
	 * indicates that the default action or value for that parameter should be used.
	 * <p>
	 * The parameters, <code>before</code> and <code>after</code>, specify the number of characters to be used for the integer part and
	 * decimal part of the result respectively. Exponential notation is not used. If either parameter is -1 (which indicates the default
	 * action), the number of characters used will be exactly as many as are needed for that part.
	 * <p>
	 * <code>before</code> must be a positive number; if it is larger than is needed to contain the integer part, that part is padded on the
	 * left with blanks to the requested length. If <code>before</code> is not large enough to contain the integer part of the number
	 * (including the sign, for negative numbers) an exception is thrown.
	 * <p>
	 * <code>after</code> must be a non-negative number; if it is not the same size as the decimal part of the number, the number will be
	 * rounded (or extended with zeros) to fit. Specifying 0 for <code>after</code> will cause the number to be rounded to an integer (that
	 * is, it will have no decimal part or decimal point). The rounding method will be the default, <code>MathContext.ROUND_HALF_UP</code>.
	 * <p>
	 * Other rounding methods, and the use of exponential notation, can be selected by using {@link #format(int,int,int,int,int,int)}. Using
	 * the two-parameter form of the method has exactly the same effect as using the six-parameter form with the final four parameters all
	 * being -1.
	 * 
	 * @param before
	 *            The <code>int</code> specifying the number of places before the decimal point. Use -1 for 'as many as are needed'.
	 * @param after
	 *            The <code>int</code> specifying the number of places after the decimal point. Use -1 for 'as many as are needed'.
	...
	/**
	 * Returns the <code>String</code> representation of this <code>BigDecimal</code>, modified by layout parameters and allowing
	 * exponential notation.
	 * <p>
	 * <i>This method is provided as a primitive for use by more sophisticated classes, such as <code>DecimalFormat
	 * </code>, that can apply locale-sensitive editing of the result. The level of formatting that it provides is a necessary part of the
	 ...
	 * function is provided elsewhere, it may be removed from this class. </i>
	 * <p>
	 * The parameters are all of type <code>int</code>. A value of -1 for any parameter indicates that the default action or value for that
	 * parameter should be used.
	 * <p>
	 * The first two parameters (<code>before</code> and <code>after</code>) specify the number of characters to be used for the integer
	 * part and decimal part of the result respectively, as defined for {@link #format(int,int)}. If either of these is -1 (which indicates
	 * the default action), the number of characters used will be exactly as many as are needed for that part.
	 * <p>
	 * The remaining parameters control the use of exponential notation and rounding. Three (<code>explaces</code>, <code>exdigits</code>,
	 * and <code>exform</code>) control the exponent part of the result. As before, the default action for any of these parameters may be
	 * selected by using the value -1.
	 * <p>
	 * <code>explaces</code> must be a positive number; it sets the number of places (digits after the sign of the exponent) to be used for
	 * any exponent part, the default (when <code>explaces</code> is -1) being to use as many as are needed. If <code>explaces</code> is not
	 * -1, space is always reserved for an exponent; if one is not needed (for example, if the exponent will be 0) then
	 * <code>explaces</code>+2 blanks are appended to the result. <!-- (This preserves vertical alignment of similarly formatted numbers in
	 * a monospace font.) --> If <code>explaces
	 * </code> is not -1 and is not large enough to contain the exponent, an exception is thrown.
	 * <p>
	 * <code>exdigits</code> sets the trigger point for use of exponential notation. If, before any rounding, the number of places needed
	 * before the decimal point exceeds <code>exdigits</code>, or if the absolute value of the result is less than <code>0.000001</code>,
	 * then exponential form will be used, provided that <code>exdigits</code> was specified. When <code>exdigits</code> is -1, exponential
	 * notation will never be used. If 0 is specified for <code>exdigits</code>, exponential notation is always used unless the exponent
	 * would be 0.
	 * <p>
	 * <code>exform</code> sets the form for exponential notation (if needed). It may be either {@link MathContext#SCIENTIFIC} or
	 * {@link MathContext#ENGINEERING}. If the latter, engineering, form is requested, up to three digits (plus sign, if negative) may be
	 * needed for the integer part of the result (<code>before</code> ). Otherwise, only one digit (plus sign, if negative) is needed.
	 * <p>
	 * Finally, the sixth argument, <code>exround</code>, selects the rounding algorithm to be used, and must be one of the values indicated
	 * by a public constant in the {@link MathContext} class whose name starts with <code>ROUND_
	 * </code>. The default (<code>ROUND_HALF_UP</code>) may also be selected by using the value -1, as before.
	 * <p>
	 * The special value <code>MathContext.ROUND_UNNECESSARY</code> may be used to detect whether non-zero digits are discarded -- if
	 * <code>exround</code> has this value than if non-zero digits would be discarded (rounded) during formatting then an
	 ...
	 *            The <code>int</code> specifying the number of places after the decimal point. Use -1 for 'as many as are needed'.
	 * @param explaces
	 *            The <code>int</code> specifying the number of places to be used for any exponent. Use -1 for 'as many as are needed'.
	 * @param exdigits
	 *            The <code>int</code> specifying the trigger (digits before the decimal point) which if exceeded causes exponential
	 *            notation to be used. Use 0 to force exponential notation. Use -1 to force plain notation (no exponential notation).
	 * @param exformint
	 *            The <code>int</code> specifying the form of exponential notation to be used ( {@link MathContext#SCIENTIFIC} or
	 *            {@link MathContext#ENGINEERING}).
	 * @param exround
	 *            The <code>int</code> specifying the rounding mode to use. Use -1 for the default, {@link MathContext#ROUND_HALF_UP}.
	 * @return A <code>String</code> representing this <code>BigDecimal</code>, laid out according to the specified parameters
	 * @throws ArithmeticException
	 *             if the number cannot be laid out as requested.
	 * @throws IllegalArgumentException
	 ...
	 * <p>
	 * Note that two <code>BigDecimal</code> objects are only guaranteed to produce the same hashcode if they are exactly equal (that is,
	 * the <code>String</code> representations of the <code>BigDecimal</code> numbers are identical -- they have the same characters in the
	 * same sequence).
	 * 
	...
	/**
	 * Converts this <code>BigDecimal</code> to an <code>int</code>. If the <code>BigDecimal</code> has a non-zero decimal part it is
	 * discarded. If the <code>BigDecimal</code> is out of the possible range for an <code>int</code> (32-bit signed integer) result then
	 * only the low-order 32 bits are used. (That is, the number may be <i>decapitated</i>.) To avoid unexpected errors when these
	...
	/**
	 * Converts this <code>BigDecimal</code> to an <code>int</code>. If the <code>BigDecimal</code> has a non-zero decimal part or is out of
	 * the possible range for an <code>int</code> (32-bit signed integer) result then an <code>ArithmeticException</code> is thrown.
	 * 
	...
	/**
	 * Converts this <code>BigDecimal</code> to a <code>long</code>. If the <code>BigDecimal</code> has a non-zero decimal part it is
	 * discarded. If the <code>BigDecimal</code> is out of the possible range for a <code>long</code> (64-bit signed integer) result then
	 * only the low-order 64 bits are used. (That is, the number may be <i>decapitated</i>.) To avoid unexpected errors when these
	...
	/**
	 * Converts this <code>BigDecimal</code> to a <code>long</code>. If the <code>BigDecimal</code> has a non-zero decimal part or is out of
	 * the possible range for a <code>long</code> (64-bit signed integer) result then an <code>ArithmeticException</code> is thrown.
	 * 
	...
	/**
	 * Returns a plain <code>BigDecimal</code> whose decimal point has been moved to the left by a specified number of positions. The
	 * parameter, <code>n</code>, specifies the number of positions to move the decimal point. That is, if <code>n</code> is 0 or positive,
	 * the number returned is given by:
	 * <p>
	 ...
	 * @return A <code>BigDecimal</code> derived from <code>this</code>, with the decimal point moved <code>n</code> places to the left.
	 * @stable ICU 2.0
	 */
	...
	/**
	 * Returns a plain <code>BigDecimal</code> whose decimal point has been moved to the right by a specified number of positions. The
	 * parameter, <code>n</code>, specifies the number of positions to move the decimal point. That is, if <code>n</code> is 0 or positive,
	 * the number returned is given by:
	 * <p>
	 * <code> this.multiply(TEN.pow(new BigDecimal(n))) </code>
	 ...
	 * @return A <code>BigDecimal</code> derived from <code>this</code>, with the decimal point moved <code>n</code> places to the right.
	 * @stable ICU 2.0
	 */
	...
	/**
	 * Returns the scale of this <code>BigDecimal</code>. Returns a non-negative <code>int</code> which is the scale of the number. The
	 * scale is the number of digits in the decimal part of the number if the number were formatted without exponential notation.
	 * 
	 ...
	 * <p>
	 * If the given scale (which must be zero or positive) is the same as or greater than the length of the decimal part (the scale) of this
	 * <code>BigDecimal</code> then trailing zeros will be added to the decimal part as necessary.
	 * <p>
	 * If the given scale is less than the length of the decimal part (the scale) of this <code>BigDecimal</code> then trailing digits will
	 * be removed, and in this case an <code>ArithmeticException</code> is thrown if any discarded digits are non-zero.
	 * <p>
	 ...
	 * <p>
	 * If the given scale is less than the length of the decimal part (the scale) of this <code>BigDecimal</code> then trailing digits will
	 * be removed, and the rounding mode given by the second parameter is used to determine if the remaining digits are affected by a carry.
	 * In this case, an <code>IllegalArgumentException</code> is thrown if <code>round</code> is not a valid rounding mode.
	 * <p>
	 * If <code>round</code> is <code>MathContext.ROUND_UNNECESSARY</code>, an <code>ArithmeticException</code> is thrown if any discarded
	 * digits are non-zero.
	 * 
	 * @param scale
	 *            The <code>int</code> specifying the scale of the resulting <code>BigDecimal</code>.
	 ...
	 *             if <code>scale</code> is negative.
	 * @throws ArithmeticException
	 *             if <code>round</code> is <code>MathContext.ROUND_UNNECESSARY</code>, and reducing scale would discard non-zero digits.
	 * @stable ICU 2.0
	 */
	...
	/**
	 * Converts this <code>BigDecimal</code> to a <code>short</code>. If the <code>BigDecimal</code> has a non-zero decimal part or is out
	 * of the possible range for a <code>short</code> (16-bit signed integer) result then an <code>ArithmeticException</code> is thrown.
	 * 
	...
	/**
	 * Returns the sign of this <code>BigDecimal</code>, as an <code>int</code>. This returns the <i>signum</i> function value that
	 * represents the sign of this <code>BigDecimal</code>. That is, -1 if the <code>BigDecimal</code> is negative, 0 if it is numerically
	 * equal to zero, or 1 if it is positive.
	 * 
	 * @return An <code>int</code> which is -1 if the <code>BigDecimal</code> is negative, 0 if it is numerically equal to zero, or 1 if it
	 *         is positive.
	 * @stable ICU 2.0
	 */
	 ...
	 * <p>
	 * This is an exact conversion; the result is the same as if the <code>BigDecimal</code> were formatted as a plain number without any
	 * rounding or exponent and then the <code>java.math.BigDecimal(java.lang.String)</code> constructor were used to construct the result.
	 * <p>
	 * <i>(Note: this method is provided only in the <code>com.ibm.icu.math</code> version of the BigDecimal class. It would not be present
	 * in a <code>java.math</code> version.)</i>
	 * 
	...
	/**
	 * Returns the <code>BigDecimal</code> as a character array. The result of this method is the same as using the sequence
	 * <code>toString().toCharArray()</code>, but avoids creating the intermediate <code>String</code> and <code>char[]</code> objects.
	 * 
	...
	/**
	 * Returns the <code>BigDecimal</code> as a <code>String</code>. This returns a <code>String</code> that exactly represents this
	 * <code>BigDecimal</code>, as defined in the decimal documentation (see {@link BigDecimal class header}).
	 * <p>
	...
	/**
	 * Returns the number as a <code>BigInteger</code> after removing the scale. That is, the number is expressed as a plain number, any
	 * decimal point is then removed (retaining the digits of any decimal part), and the result is then converted to a
	 * <code>BigInteger</code>.
	 * 
	 * @return The <code>java.math.BigInteger</code> equal in value to this <code>BigDecimal</code> multiplied by ten to the power of
	 ...
	 * <p>
	 * Returns a <code>BigDecimal</code> which is the decimal representation of the 64-bit signed binary floating point parameter. If the
	 * parameter is infinite, or is not a number (NaN), a <code>NumberFormatException</code> is thrown.
	 * <p>
	 * The number is constructed as though <code>num</code> had been converted to a <code>String</code> using the <code>
	 * Double.toString()</code> method and the {@link #BigDecimal(java.lang.String)} constructor had then been used. This is typically not
	 * an exact conversion.
	 * 
	 * @param dub
	...
	/**
	 * Translates a <code>long</code> to a <code>BigDecimal</code>. That is, returns a plain <code>BigDecimal</code> whose value is equal to
	 ...
	 * BigDecimal</code> whose unscaled value is equal to the given <code>long</code>, adjusted by the second parameter, <code>scale</code>.
	 * <p>
				...
				if (this.mant[i] != 0) {
					...
					if (i > 0) {
						do {
							...
							java.lang.System
									.arraycopy((java.lang.Object) this.mant, i, (java.lang.Object) newmant, 0, this.mant.length - i);
							this.mant = newmant;
						} while (false);
					}/* delead */
