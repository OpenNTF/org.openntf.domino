 ...
 * <code>StringTokenizer</code> methods do not distinguish among identifiers, numbers, and quoted strings, nor do they recognize and skip
 * comments.
 * </p>
 * <p>
 ...
 * </p>
 * <p>
 * An instance of <code>StringTokenizer</code> behaves in one of three ways, depending on whether it was created with the
 * <code>returnDelims</code> and <code>coalesceDelims</code> flags having the value <code>true</code> or <code>false</code>:
 * <ul>
 * <li>If returnDelims is <code>false</code>, delimiter code points serve to separate tokens. A token is a maximal sequence of consecutive
 * code points that are not delimiters.
 * <li>If returnDelims is <code>true</code>, delimiter code points are themselves considered to be tokens. In this case, if coalesceDelims
 * is <code>true</code>, such tokens will be the maximal sequence of consecutive code points that <em>are</em> delimiters. If coalesceDelims
 * is false, a token will be received for each delimiter code point.
 * </ul>
 * <p>
 * A token is thus either one delimiter code point, a maximal sequence of consecutive code points that are delimiters, or a maximal sequence
 * of consecutive code points that are not delimiters.
 * </p>
 * <p>
 * A <tt>StringTokenizer</tt> object internally maintains a current position within the string to be tokenized. Some operations advance this
 * current position past the code point processed.
 * </p>
 * <p>
 * A token is returned by taking a substring of the string that was used to create the <tt>StringTokenizer</tt> object.
 * </p>
 * <p>
 ...
 * </p>
 * <p>
 * Example of the use of the tokenizer with user specified delimiter. <blockquote>
 * 
 * <pre>
 * StringTokenizer st = new StringTokenizer(&quot;this is a test with supplementary characters \ud800\ud800\udc00\udc00&quot;, &quot; \ud800\udc00&quot;);
 * while (st.hasMoreTokens()) {
 * 	println(st.nextToken());
 * }
 * </pre>
...
public final class StringTokenizer implements Enumeration<Object> {
	...
	/**
	 * {@icu} Constructs a string tokenizer for the specified string. All characters in the delim argument are the delimiters for separating
	 * tokens.
	 * <p>
	 ...
	 * </p>
	 * <p>
	 * If the returnDelims flag is true, then the delimiter characters are also returned as tokens. If coalescedelims is true, one token is
	 * returned for each run of delimiter characters, otherwise one token is returned per delimiter. Since surrogate pairs can be
	 ...
	 * @param returndelims
	 *            flag indicating whether to return the delimiters as tokens.
	 * @param coalescedelims
	 *            flag indicating whether to return a run of delimiters as a single token or as one token per delimiter. This only takes
	 *            effect if returndelims is true.
	 * @exception NullPointerException
	 *                if str is null
	 * @internal
	...
	/**
	 * {@icu} Constructs a string tokenizer for the specified string. The characters in the delim argument are the delimiters for separating
	 * tokens.
	 * <p>
	...
	/**
	 * <p>
	 * Constructs a string tokenizer for the specified string. All characters in the delim argument are the delimiters for separating
	 * tokens.
	 * </p>
	 * <p>
	 * If the returnDelims flag is false, the delimiter characters are skipped and only serve as separators between tokens.
	...
	/**
	 * <p>
	 * Constructs a string tokenizer for the specified string. The characters in the delim argument are the delimiters for separating
	 * tokens.
	 * </p>
	 * <p>
	 * Delimiter characters themselves will not be treated as tokens.
	 ...
	 * " &#92;t&#92;n&#92;r&#92;f": the space character, the tab character, the newline character, the carriage-return character, and the
	 * form-feed character.
	 * </p>
	 * <p>
	 * Delimiter characters themselves will not be treated as tokens.
	...
	/**
	 * Tests if there are more tokens available from this tokenizer's string. If this method returns <tt>true</tt>, then a subsequent call
	 * to <tt>nextToken</tt> with no argument will successfully return a token.
	 * 
	 * @return <code>true</code> if and only if there is at least one token in the string after the current position; <code>false</code>
	 *         otherwise.
	 * @stable ICU 2.4
	 */
	...
	public String nextToken() {
		if (m_tokenOffset_ < 0) {
			...
			if (m_returnDelimiters_) {
				...
				boolean contains = delims == null ? m_delimiters_.contains(c) : c < delims.length && delims[c];
				if (contains) {
	 ...
	 * <tt>StringTokenizer</tt> object is changed to be the characters in the string <tt>delim</tt>. Then the next token in the string after
	 * the current position is returned. The current position is advanced beyond the recognized token. The new delimiter set remains the
	...
	/**
	 * {@icu} Returns the next token in this string tokenizer's string. First, the set of characters considered to be delimiters by this
	 * <tt>StringTokenizer</tt> object is changed to be the characters in the string <tt>delim</tt>. Then the next token in the string after
	 * the current position is returned. The current position is advanced beyond the recognized token. The new delimiter set remains the
	 * default after this call.
	 * 
	...
	/**
	 * Returns the same value as the <code>nextToken</code> method, except that its declared return value is <code>Object</code> rather than
	 * <code>String</code>. It exists so that this class can implement the <code>Enumeration</code> interface.
	 * 
	...
	/**
	 * Array of pre-calculated tokens start indexes in source string terminated by -1. This is only set up during countTokens() and only
	 * stores the remaining tokens, not all tokens including parsed ones
	 */
	private int m_tokenStart_[];
	/**
	 * Array of pre-calculated tokens limit indexes in source string. This is only set up during countTokens() and only stores the remaining
	 ...
	 */
	private static final UnicodeSet DEFAULT_DELIMITERS_ = new UnicodeSet(0x09, 0x0a, 0x0c, 0x0d, 0x20, 0x20);   // UnicodeSet("[ \t\n\r\f]", false)
	/**
	 ...
	 * @return offset of the immediate next delimiter, otherwise (- source string length - 1) if there are no more delimiters after
	 *         m_nextOffset
	 */
	 ...
	 * @param offset
	 *            to the source string
	 * @return offset of the immediate next non-delimiter, otherwise (- source string length - 1) if there are no more delimiters after
	 *         m_nextOffset
	 */
