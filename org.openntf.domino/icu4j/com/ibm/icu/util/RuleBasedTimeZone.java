...
/**
 * <code>RuleBasedTimeZone</code> is a concrete subclass of <code>TimeZone</code> that allows users to define custom historic time
 * transition rules.
 * 
...
public class RuleBasedTimeZone extends BasicTimeZone {
	...
	/**
	 * Constructs a <code>RuleBasedTimeZone</code> object with the ID and the <code>InitialTimeZoneRule</code>
	 * 
	 * @param id
	 *            The time zone ID.
	 * @param initialRule
	 *            The initial time zone rule.
	 * 
	 * @stable ICU 3.8
	 */
	...
	/**
	 * Adds the <code>TimeZoneRule</code> which represents time transitions. The <code>TimeZoneRule</code> must have start times, that is,
	 * the result of {@link com.ibm.icu.util.TimeZoneRule#isTransitionRule()} must be true. Otherwise, <code>IllegalArgumentException</code>
	 * is thrown.
	 * 
	 * @param rule
	 *            The <code>TimeZoneRule</code>.
	 * 
	 * @stable ICU 3.8
	 */
	public void addTransitionRule(final TimeZoneRule rule) {
		...
		if (rule instanceof AnnualTimeZoneRule && ((AnnualTimeZoneRule) rule).getEndYear() == AnnualTimeZoneRule.MAX_YEAR) {
	...
	public boolean hasSameRules(final TimeZone other) {
		...
		if (finalRules != null && otherRBTZ.finalRules != null) {
			for (int i = 0; i < finalRules.length; i++) {
				...
				if (finalRules[i] != null && otherRBTZ.finalRules[i] != null && finalRules[i].isEquivalentTo(otherRBTZ.finalRules[i])) {
					continue;
	...
	public TimeZoneTransition getPreviousTransition(final long base, final boolean inclusive) {
		...
		if (inclusive && tt == base) {
			...
			if (inclusive && tt == base) {
				...
				if (finalRules != null) {
					// Find a transion time with finalRules
					Date start0 = finalRules[0].getPreviousStart(base, finalRules[1].getRawOffset(), finalRules[1].getDSTSavings(),
							inclusive);
					Date start1 = finalRules[1].getPreviousStart(base, finalRules[0].getRawOffset(), finalRules[0].getDSTSavings(),
							inclusive);
				...
				while (true) {
					...
					for (int i = 0; i < historicRules.size(); i++) {
						...
						if (d == null) {
							...
							if (r == curRule
									|| (r.getName().equals(curRule.getName()) && r.getRawOffset() == curRule.getRawOffset() && r
											.getDSTSavings() == curRule.getDSTSavings())) {
								continue;
							}
							tt = d.getTime();
	 ...
	 */
	private void getOffset(final long time, final boolean local, final int NonExistingTimeOpt, final int DuplicatedTimeOpt,
			final int[] offsets) {
		complete();
		...
		if (historicTransitions == null) {
			...
			long tstart = getTransitionTime(historicTransitions.get(0), local, NonExistingTimeOpt, DuplicatedTimeOpt);
			if (time < tstart) {
				rule = initialRule;
			} else {
				int idx = historicTransitions.size() - 1;
				long tend = getTransitionTime(historicTransitions.get(idx), local, NonExistingTimeOpt, DuplicatedTimeOpt);
				if (time > tend) {
					if (finalRules != null) {
					...
					while (idx >= 0) {
						if (time >= getTransitionTime(historicTransitions.get(idx), local, NonExistingTimeOpt, DuplicatedTimeOpt)) {
							break;
						}
						idx--;
					}
	...
	private TimeZoneRule findRuleInFinal(final long time, final boolean local, final int NonExistingTimeOpt, final int DuplicatedTimeOpt) {
		...
		base = time;
		if (local) {
			localDelta = getLocalDelta(finalRules[1].getRawOffset(), finalRules[1].getDSTSavings(), finalRules[0].getRawOffset(),
					finalRules[0].getDSTSavings(), NonExistingTimeOpt, DuplicatedTimeOpt);
			base -= localDelta;
		}
		...
		base = time;
		if (local) {
			localDelta = getLocalDelta(finalRules[0].getRawOffset(), finalRules[0].getDSTSavings(), finalRules[1].getRawOffset(),
					finalRules[1].getDSTSavings(), NonExistingTimeOpt, DuplicatedTimeOpt);
			base -= localDelta;
		}
	 ...
	 */
	private static long getTransitionTime(final TimeZoneTransition tzt, final boolean local, final int NonExistingTimeOpt,
			final int DuplicatedTimeOpt) {
		long time = tzt.getTime();
		if (local) {
			time += getLocalDelta(tzt.getFrom().getRawOffset(), tzt.getFrom().getDSTSavings(), tzt.getTo().getRawOffset(), tzt.getTo()
	 ...
	 */
	private static int getLocalDelta(final int rawBefore, final int dstBefore, final int rawAfter, final int dstAfter,
			final int NonExistingTimeOpt, final int DuplicatedTimeOpt) {
		int delta = 0;
