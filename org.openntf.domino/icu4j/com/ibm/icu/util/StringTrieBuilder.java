...
public abstract class StringTrieBuilder {
	...
	public enum Option {
		...
		FAST,
		/**
		 * Builds a trie more slowly, attempting to generate a shorter but equivalent serialization. This build option also uses more
		 * memory.
		 * 
		 * <p>
		 * This option can be effective when many integer values are the same and string/byte sequence suffixes can be shared. Runtime speed
	 ...
	 * 
	 * @param newNode
	 *            Input node. The builder takes ownership.
	 * @return newNode if it is the first of its kind, or an equivalent node if newNode is a duplicate.
	 */
	...
	/**
	 * Makes sure that there is only one unique FinalValueNode registered with this value. Avoids creating a node if the value is a
	 * duplicate.
	 * 
	 * @param value
	 *            A final value.
	...
	private static abstract class Node {
		...
		/**
		 * Recursive method for adding a new (string, value) pair. Matches the remaining part of s from start, and adds a new node where
		 * there is a mismatch.
		 * 
		...
		/**
		 * Recursive method for registering unique nodes, after all (string, value) pairs have been added. Final-value nodes are
		 * pre-registered while add()ing (string, value) pairs. Other nodes created while add()ing registerNode() themselves later and might
		 * replace themselves with new types of nodes for write()ing.
		 * 
		...
		/**
		 * Traverses the Node graph and numbers branch edges, with rightmost edges first. This is to avoid writing a duplicate node twice.
		 * 
		 * Branch nodes in this trie data structure are not symmetric. Most branch edges "jump" to other nodes but the rightmost branch
		 * edges just continue without a jump. Therefore, write() must write the rightmost branch edge last (trie units are written
		 * backwards), and must write it at that point even if it is a duplicate of a node previously written elsewhere.
		 * 
		 * This function visits and marks right branch edges first. Edges are numbered with increasingly negative values because we share
		 * the offset field which gets positive values when nodes are written. A branch edge also remembers the first number for any of its
		 * edges.
		 * 
		 * When a further-left branch edge has a number in the range of the rightmost edge's numbers, then it will be written as part of the
		 ...
		 * 
		 * @param edgeNumber
		 *            The first edge number for this node and its sub-nodes.
		 * @return An edge number that is at least the maximum-negative of the input edge number and the numbers of this node and all of its
		 *         sub-nodes.
		 */
	...
	private static final class DynamicBranchNode extends ValueNode {
		...
		private Node register(StringTrieBuilder builder, int start, int limit) {
			...
			if (length > builder.getMaxBranchLinearSubNodeLength()) {
				...
				return builder.registerNode(new SplitBranchNode(chars.charAt(middle), register(builder, start, middle), register(builder,
						middle, limit)));
			}
	...
	private static final class SplitBranchNode extends BranchNode {
		public SplitBranchNode(char middleUnit, Node lessThanNode, Node greaterOrEqualNode) {
			hash = ((0x555555 * 37 + middleUnit) * 37 + lessThanNode.hashCode()) * 37 + greaterOrEqualNode.hashCode();
			unit = middleUnit;
