...
/**
 * <code>VTimeZone</code> is a class implementing RFC2445 VTIMEZONE. You can create a <code>VTimeZone</code> instance from a time zone ID
 * supported by <code>TimeZone</code>. With the <code>VTimeZone</code> instance created from the ID, you can write out the rule in RFC2445
 * VTIMEZONE format. Also, you can create a <code>VTimeZone</code> instance from RFC2445 VTIMEZONE data stream, which allows you to
 * calculate time zone offset by the rules defined by the data.<br>
 * <br>
 * 
 * Note: The consumer of this class reading or writing VTIMEZONE data is responsible to decode or encode Non-ASCII text. Methods
 * reading/writing VTIMEZONE data in this class do nothing with MIME encoding.
 * 
 * @stable ICU 3.8
 */
...
public class VTimeZone extends BasicTimeZone {
	 ...
	 * 
	 * @param tzid
	 *            The time zone ID, such as America/New_York
	 * @return A <code>VTimeZone</code> initialized by the time zone ID, or null when the ID is unknown.
	 * 
	 * @stable ICU 3.8
	 */
	 ...
	 * 
	 * @param reader
	 *            The Reader for VTIMEZONE data input stream
	 * @return A <code>VTimeZone</code> initialized by the VTIMEZONE data or null if failed to load the rule from the VTIMEZONE data.
	 * 
	 * @stable ICU 3.8
	 */
	...
	/**
	 * Gets the RFC2445 TZURL property value. When a <code>VTimeZone</code> instance was created from VTIMEZONE data, the value is set by
	 * the TZURL property value in the data. Otherwise, the initial value is null.
	 * 
	...
	/**
	 * Gets the RFC2445 LAST-MODIFIED property value. When a <code>VTimeZone</code> instance was created from VTIMEZONE data, the value is
	 * set by the LAST-MODIFIED property value in the data. Otherwise, the initial value is null.
	 * 
	 ...
	 * 
	 * @param date
	 *            The <code>Date</code> object represents the date for RFC2445 LAST-MODIFIED property value.
	 * 
	 * @stable ICU 3.8
	 */
	 ...
	 * 
	 * @param writer
	 *            A <code>Writer</code> used for the output
	 * @throws IOException
	 ...
	 * 
	 * @param writer
	 *            The <code>Writer</code> used for the output
	 * @param start
	 *            The start time
	 * 
	 * @throws IOException
	...
	public void write(final Writer writer, final long start) throws IOException {
		...
		if (olsonzid != null && ICU_TZVERSION != null) {
			customProperties = new String[1];
			customProperties[0] = ICU_TZINFO_PROP + COLON + olsonzid + "[" + ICU_TZVERSION + "/Partial@" + start + "]";
		}
		writeZone(writer, rbtz, customProperties);
	}

	/**
	 * Writes RFC2445 VTIMEZONE data applicable near the specified date. Some common iCalendar implementations can only handle a single time
	 * zone property or a pair of standard and daylight time properties using BYDAY rule with day of week (such as BYDAY=1SUN). This method
	 * produce the VTIMEZONE data which can be handled these implementations. The rules produced by this method can be used only for
	 * calculating time zone offset around the specified date.
	 * 
	 * @param writer
	 *            The <code>Writer</code> used for the output
	 * @param time
	...
	public void writeSimple(final Writer writer, final long time) throws IOException {
		...
		if (olsonzid != null && ICU_TZVERSION != null) {
			customProperties = new String[1];
			customProperties[0] = ICU_TZINFO_PROP + COLON + olsonzid + "[" + ICU_TZVERSION + "/Simple@" + time + "]";
		}
		writeZone(writer, rbtz, customProperties);
	}
		...
		if (finalRuleCount == 1) {
			if (rules.size() == 1) {
				...
				if (start == finalStart) {
					...
					newRule = new TimeArrayTimeZoneRule(finalRule.getName(), finalRule.getRawOffset(), finalRule.getDSTSavings(),
							new long[] { finalStart.getTime() }, DateTimeRule.UTC_TIME);
				} else {
					// Update the end year
					int fields[] = Grego.timeToFields(start.getTime(), null);
					newRule = new AnnualTimeZoneRule(finalRule.getName(), finalRule.getRawOffset(), finalRule.getDSTSavings(),
							finalRule.getRule(), finalRule.getStartYear(), fields[0]);
				}
	 ...
	 */
	private static TimeZoneRule createRuleByRRULE(final String tzname, final int rawOffset, final int dstSavings, final long start,
			final List<String> dates, final int fromOffset) {
	 ...
	 */
	private static TimeZoneRule createRuleByRDATE(final String tzname, final int rawOffset, final int dstSavings, final long start,
			final List<String> dates, final int fromOffset) {
	...
	private void writeZone(final Writer w, final BasicTimeZone basictz, final String[] customProperties) throws IOException {
			...
			if (isDst) {
				...
				if (dstCount > 0) {
					if (year == dstStartYear + dstCount && name.equals(dstName) && dstFromOffset == fromOffset && dstToOffset == toOffset
							&& dstMonth == dtfields[1] && dstDayOfWeek == dtfields[3] && dstWeekInMonth == weekInMonth
					...
					if (!sameRule) {
						if (dstCount == 1) {
							...
							writeZonePropsByDOW(w, true, dstName, dstFromOffset, dstToOffset, dstMonth, dstWeekInMonth, dstDayOfWeek,
									dstStartTime, dstUntilTime);
						}
					}
				}
				if (!sameRule) {
				...
				if (stdCount > 0) {
					if (year == stdStartYear + stdCount && name.equals(stdName) && stdFromOffset == fromOffset && stdToOffset == toOffset
							&& stdMonth == dtfields[1] && stdDayOfWeek == dtfields[3] && stdWeekInMonth == weekInMonth
					...
					if (!sameRule) {
						if (stdCount == 1) {
							...
							writeZonePropsByDOW(w, false, stdName, stdFromOffset, stdToOffset, stdMonth, stdWeekInMonth, stdDayOfWeek,
									stdStartTime, stdUntilTime);
						}
					}
				}
				if (!sameRule) {
		...
		if (!hasTransitions) {
			...
			writeZonePropsByTime(w, isDst, getDefaultTZName(basictz.getID(), isDst), offset, offset, DEF_TZSTARTTIME - offset, false);
		} else {
			if (dstCount > 0) {
	 ...
	 */
	private static void writeZonePropsByTime(final Writer writer, final boolean isDst, final String tzname, final int fromOffset,
			final int toOffset, final long time, final boolean withRDATE) throws IOException {
	 ...
	 */
	private static void writeZonePropsByDOM(final Writer writer, final boolean isDst, final String tzname, final int fromOffset,
			final int toOffset, final int month, final int dayOfMonth, final long startTime, final long untilTime) throws IOException {
	 ...
	 */
	private static void writeZonePropsByDOW(final Writer writer, final boolean isDst, final String tzname, final int fromOffset,
			final int toOffset, final int month, final int weekInMonth, final int dayOfWeek, final long startTime, final long untilTime)
			throws IOException {
		beginZoneProps(writer, isDst, tzname, fromOffset, toOffset, startTime);
	 ...
	 */
	private static void writeZonePropsByDOW_GEQ_DOM(final Writer writer, final boolean isDst, final String tzname, final int fromOffset,
			final int toOffset, final int month, final int dayOfMonth, final int dayOfWeek, final long startTime, final long untilTime)
			throws IOException {
		...
		if (dayOfMonth % 7 == 1) {
			...
			writeZonePropsByDOW(writer, isDst, tzname, fromOffset, toOffset, month, -1 * ((MONTHLENGTH[month] - dayOfMonth + 1) / 7),
			...
			if (dayOfMonth <= 0) {
				...
				writeZonePropsByDOW_GEQ_DOM_sub(writer, nextMonth, 1, dayOfWeek, nextMonthDays, MAX_TIME /* Do not use UNTIL */,
						fromOffset);
			}
	 ...
	 */
	private static void writeZonePropsByDOW_GEQ_DOM_sub(final Writer writer, final int month, final int dayOfMonth, final int dayOfWeek,
			final int numDays, final long untilTime, final int fromOffset) throws IOException {
	 ...
	 */
	private static void writeZonePropsByDOW_LEQ_DOM(final Writer writer, final boolean isDst, final String tzname, final int fromOffset,
			final int toOffset, final int month, final int dayOfMonth, final int dayOfWeek, final long startTime, final long untilTime)
			throws IOException {
		// Check if this rule can be converted to DOW rule
		if (dayOfMonth % 7 == 0) {
			...
			// Can be represented by DOW rule with negative week number
			writeZonePropsByDOW(writer, isDst, tzname, fromOffset, toOffset, month, -1 * ((MONTHLENGTH[month] - dayOfMonth) / 7 + 1),
					dayOfWeek, startTime, untilTime);
	 ...
	 */
	private static void writeFinalRule(final Writer writer, final boolean isDst, final AnnualTimeZoneRule rule, final int fromRawOffset,
			final int fromDSTSavings, long startTime) throws IOException {
		...
		case DateTimeRule.DOM:
			writeZonePropsByDOM(writer, isDst, rule.getName(), fromRawOffset + fromDSTSavings, toOffset, dtrule.getRuleMonth(),
					dtrule.getRuleDayOfMonth(), startTime, MAX_TIME);
			break;
		case DateTimeRule.DOW:
			writeZonePropsByDOW(writer, isDst, rule.getName(), fromRawOffset + fromDSTSavings, toOffset, dtrule.getRuleMonth(),
					dtrule.getRuleWeekInMonth(), dtrule.getRuleDayOfWeek(), startTime, MAX_TIME);
			break;
		case DateTimeRule.DOW_GEQ_DOM:
			writeZonePropsByDOW_GEQ_DOM(writer, isDst, rule.getName(), fromRawOffset + fromDSTSavings, toOffset, dtrule.getRuleMonth(),
					dtrule.getRuleDayOfMonth(), dtrule.getRuleDayOfWeek(), startTime, MAX_TIME);
			break;
		case DateTimeRule.DOW_LEQ_DOM:
			writeZonePropsByDOW_LEQ_DOM(writer, isDst, rule.getName(), fromRawOffset + fromDSTSavings, toOffset, dtrule.getRuleMonth(),
					dtrule.getRuleDayOfMonth(), dtrule.getRuleDayOfWeek(), startTime, MAX_TIME);
			break;
		}
	}
	 ...
	 */
	private static void beginZoneProps(final Writer writer, final boolean isDst, final String tzname, final int fromOffset,
			final int toOffset, final long startTime) throws IOException {
	...
	private static long parseDateTimeString(final String str, final int offset) {
		...
		do {
			...
			if (year < 0 || month < 0 || month > 11 || day < 1 || day > maxDayOfMonth || hour < 0 || hour >= 24 || min < 0 || min >= 60
					|| sec < 0 || sec >= 60) {
				break;
			}
