...
/**
 * <code>HebrewCalendar</code> is a subclass of <code>Calendar</code> that that implements the traditional Hebrew calendar. This is the
 * civil calendar in Israel and the liturgical calendar of the Jewish faith worldwide.
 * <p>
 ...
 * the lunar month (approximately 29.53 days) an extra "leap month" is inserted in 7 out of every 19 years. To make matters even more
 * interesting, the start of a year can be delayed by up to three days in order to prevent certain holidays from falling on the Sabbath and
 * to prevent certain illegal year lengths. Finally, the lengths of certain months can vary depending on the number of days in the year.
 * <p>
 * The leap month is known as "Adar 1" and is inserted between the months of Shevat and Adar in leap years. Since the leap month does not
 * come at the end of the year, calculations involving month numbers are particularly complex. Users of this class should make sure to use
 * the {@link #roll roll} and {@link #add add} methods rather than attempting to perform date arithmetic by manipulating the fields
 * directly.
 * <p>
 * <b>Note:</b> In the traditional Hebrew calendar, days start at sunset. However, in order to keep the time fields in this class
 * synchronized with those of the other calendars and with local clock time, we treat days and months as beginning at midnight, roughly 6
 * hours after the corresponding sunset.
 * <p>
 * If you are interested in more information on the rules behind the Hebrew calendar, see one of the following references:
 * <ul>
 * <li>"<a href="http://www.amazon.com/exec/obidos/ASIN/0521564743">Calendrical Calculations</a>", by Nachum Dershowitz & Edward Reingold,
 * Cambridge University Press, 1997, pages 85-91.
 * 
 * <li>Hebrew Calendar Science and Myths, <a href="http://www.geocities.com/Athens/1584/"> http://www.geocities.com/Athens/1584/</a>
 * 
 * <li>The Calendar FAQ, <a href="http://www.faqs.org/faqs/calendars/faq/"> http://www.faqs.org/faqs/calendars/faq/</a>
 * </ul>
 * 
 * <p>
 ...
 * </p>
 * <p>
 * HebrewCalendar usually should be instantiated using {@link com.ibm.icu.util.Calendar#getInstance(ULocale)} passing in a
 * <code>ULocale</code> with the tag <code>"@calendar=hebrew"</code>.
 * </p>
 * 
...
public class HebrewCalendar extends Calendar {
	...
	/**
	 * Constant for Adar I, the 6th month of the Hebrew year (present in leap years only). In non-leap years, the calendar jumps from Shevat
	 * (5th month) to Adar (7th month).
	 * 
	 * @stable ICU 2.8
	 */
	...
	/**
	 * The absolute date, in milliseconds since 1/1/1970 AD, Gregorian, of the start of the Hebrew calendar. In order to keep this
	 * calendar's time of day in sync with that of the Gregorian calendar, we use midnight, rather than sunset the day before.
	 */
	...
	/**
	 * The lengths of the Hebrew months. This is complicated, because there are three different types of years, or six if you count leap
	 * years. Due to the rules for postponing the start of the year to avoid having certain holidays fall on the sabbath, the year can end
	 * up being three different lengths, called "deficient", "normal", and "complete".
	 */
	...
	/**
	 * The cumulative # of days to the end of each month in a non-leap year Although this can be calculated from the MONTH_LENGTH table,
	...
	/**
	 * Constructs a default <code>HebrewCalendar</code> using the current time in the default time zone with the default <code>FORMAT</code>
	 * locale.
	 * 
	 * @see Category#FORMAT
	 * @stable ICU 2.8
	...
	/**
	 * Constructs a <code>HebrewCalendar</code> based on the current time in the given time zone with the default <code>FORMAT</code>
	 * locale.
	 * 
	 * @param zone
	...
	/**
	 * Constructs a <code>HebrewCalendar</code> based on the current time in the default time zone with the given locale.
	 * 
	 * @param aLocale
	...
	/**
	 * Constructs a <code>HebrewCalendar</code> based on the current time in the given time zone with the given locale.
	 * 
	 * @param zone
	 *            The time zone for the new calendar.
	 * 
	...
	/**
	 * Constructs a <code>HebrewCalendar</code> with the given date set in the default time zone with the default <code>FORMAT</code>
	 * locale.
	 * 
	 * @param year
	 *            The value used to set the calendar's {@link #YEAR YEAR} time field.
	 * 
	 * @param month
	 *            The value used to set the calendar's {@link #MONTH MONTH} time field. The value is 0-based. e.g., 0 for Tishri.
	 * 
	 * @param date
	 *            The value used to set the calendar's {@link #DATE DATE} time field.
	 * @see Category#FORMAT
	 * @stable ICU 2.8
	 */
	...
	/**
	 * Constructs a <code>HebrewCalendar</code> with the given date and time set for the default time zone with the default
	 ...
	 * 
	 * @param hour
	 *            The value used to set the calendar's {@link #HOUR_OF_DAY HOUR_OF_DAY} time field.
	 * 
	 * @param minute
	 *            The value used to set the calendar's {@link #MINUTE MINUTE} time field.
	 * 
	 * @param second
	 *            The value used to set the calendar's {@link #SECOND SECOND} time field.
	 * @see Category#FORMAT
	 * @stable ICU 2.8
	 */
	 ...
	 * {@link #MONTH MONTH} field for the date "30 Av 5758", the {@link #DAY_OF_MONTH DAY_OF_MONTH} field must be adjusted so that the
	 * result is "29 Elul 5758" rather than the invalid "30 Elul 5758".
	 * <p>
	 * This method is able to add to all fields except for {@link #ERA ERA}, {@link #DST_OFFSET DST_OFFSET}, and {@link #ZONE_OFFSET
	 * ZONE_OFFSET}.
	 * <p>
	 * <b>Note:</b> You should always use {@link #roll roll} and add rather than attempting to perform arithmetic operations directly on the
	 * fields of a <tt>HebrewCalendar</tt>. Since the {@link #MONTH MONTH} field behaves discontinuously in non-leap years, simple
	 * arithmetic can give invalid results.
	 * <p>
	 * 
	 * @param field
	 *            the time field.
	 ...
	 * continue rolling. For example, calling <code>roll(Calendar.DATE, 10)</code> on a Hebrew calendar set to "25 Av 5758" will result in
	 * the date "5 Av 5758".
	 * <p>
	 * When rolling certain fields, the values of other fields may conflict and need to be changed. For example, when rolling the
	 * {@link #MONTH MONTH} field upward by one for the date "30 Av 5758", the {@link #DAY_OF_MONTH DAY_OF_MONTH} field must be adjusted so
	 * that the result is "29 Elul 5758" rather than the invalid "30 Elul".
	 * <p>
	 * This method is able to roll all fields except for {@link #ERA ERA}, {@link #DST_OFFSET DST_OFFSET}, and {@link #ZONE_OFFSET
	 * ZONE_OFFSET}. Subclasses may, of course, add support for additional fields in their overrides of <code>roll</code>.
	 * <p>
	 * <b>Note:</b> You should always use roll and {@link #add add} rather than attempting to perform arithmetic operations directly on the
	 * fields of a <tt>HebrewCalendar</tt>. Since the {@link #MONTH MONTH} field behaves discontinuously in non-leap years, simple
	 * arithmetic can give invalid results.
	 * <p>
	 * 
	...
	/**
	 * Finds the day # of the first day in the given Hebrew year. To do this, we want to calculate the time of the Tishri 1 new moon in that
	 * year.
	 * <p>
	 * The algorithm here is similar to ones described in a number of references, including:
	 * <ul>
	 * <li>"Calendrical Calculations", by Nachum Dershowitz & Edward Reingold, Cambridge University Press, 1997, pages 85-91.
	 * 
	 * <li>Hebrew Calendar Science and Myths, <a href="http://www.geocities.com/Athens/1584/"> http://www.geocities.com/Athens/1584/</a>
	 * 
	 * <li>The Calendar FAQ, <a href="http://www.faqs.org/faqs/calendars/faq/"> http://www.faqs.org/faqs/calendars/faq/</a>
	...
	/**
	 * Returns the the type of a given year. 0 "Deficient" year with 353 or 383 days 1 "Normal" year with 354 or 384 days 2 "Complete" year
	 * with 355 or 385 days
	 */
