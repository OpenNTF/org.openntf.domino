 ...
 * </p>
 * <p>
 * Generally passed as an argument object into a method. The method takes responsibility of writing into the internal byte array and
 * increasing its size when necessary.
 * </p>
 * 
 * @author syn wee
 * @stable ICU 2.8
...
public class ByteArrayWrapper implements Comparable<ByteArrayWrapper> {
	...
	/**
	 * Size of the internal byte array used. Different from bytes.length, size will be &lt;= bytes.length. Semantics of size is similar to
	 ...
	 * 
	 * @param byteBuffer
	 *            public ByteArrayWrapper(ByteArrayWrapper source) { size = source.size; bytes = new byte[size]; copyBytes(source.bytes, 0,
	 *            bytes, 0, size); }
	 */
	 ...
	 * @param start
	 * @param limit
	 *            public ByteArrayWrapper(byte[] src, int start, int limit) { size = limit - start; bytes = new byte[size]; copyBytes(src,
	 *            start, bytes, 0, size); }
	 */
	...
	/**
	 * Ensure that the internal byte array is at least of length capacity. If the byte array is null or its length is less than capacity, a
	 * new byte array of length capacity will be allocated. The contents of the array (between 0 and size) remain unchanged.
	 * 
	 * @param capacity
	...
	/**
	 * Set the internal byte array from offset 0 to (limit - start) with the contents of src from offset start to limit. If the byte array
	 * is null or its length is less than capacity, a new byte array of length (limit - start) will be allocated. This resets the size of
	 * the internal byte array to (limit - start).
	 * 
	 * @param src
	 *            source byte array to copy from
	 * @param start
	 *            start offset of src to copy from
	 * @param limit
	...
	*/
	...
	/**
	 * Appends the internal byte array from offset size with the contents of src from offset start to limit. This increases the size of the
	 * internal byte array to (size + limit - start).
	 * 
	 * @param src
	 *            source byte array to copy from
	 * @param start
	...
	*/
	 ...
	 *            start offset of src to copy from
	 * @param tgt
	 *            target byte array to copy to
	 * @param tgtoff
	 *            start offset of tgt to copy to
	 * @param length
