...
/**
 * The CompactDecimalFormat produces abbreviated numbers, suitable for display in environments will limited real estate. For example, 'Hits:
 * 1.2B' instead of 'Hits: 1,200,000,000'. The format will be appropriate for the given language, such as "1,2 Mrd." for German.
 * <p>
 * For numbers under 1000 trillion (under 10^15, such as 123,456,789,012,345), the result will be short for supported languages. However,
 * the result may sometimes exceed 7 characters, such as when there are combining marks or thin characters. In such cases, the visual width
 * in fonts should still be short.
 * <p>
 * By default, there are 2 significant digits. After creation, if more than three significant digits are set (with
 * setMaximumSignificantDigits), or if a fixed number of digits are set (with setMaximumIntegerDigits or setMaximumFractionDigits), then
 * result may be wider.
 * <p>
 * At this time, negative numbers and parsing are not supported, and will produce an UnsupportedOperationException. Resetting the pattern
 * prefixes or suffixes is not supported; the method calls are ignored.
 * <p>
...
public class CompactDecimalFormat extends DecimalFormat {
	...
	/**
	 * Create a CompactDecimalFormat appropriate for a locale. The result may be affected by the number system in the locale, such as
	 * ar-u-nu-latn.
	 * 
	 * @param locale
	 *            the desired locale
	...
	/**
	 * Create a short number "from scratch". Intended for internal use. The prefix, suffix, and divisor arrays are parallel, and provide the
	 * information for each power of 10. When formatting a value, the correct power of 10 is found, then the value is divided by the
	 * divisor, and the prefix and suffix are set (using setPositivePrefix/Suffix).
	 * 
	 * @param pattern
	 *            A number format pattern. Note that the prefix and suffix are discarded, and the decimals are overridden by default.
	 * @param formatSymbols
	 ...
	 *            A collection of strings for debugging. If null on input, then any errors found will be added to that collection instead of
	 *            throwing exceptions.
	 * @internal
	 * @deprecated This API is ICU internal only.
	 */
	@Deprecated
	public CompactDecimalFormat(final String pattern, final DecimalFormatSymbols formatSymbols, final String[] prefix,
			final String[] suffix, final long[] divisor, final Collection<String> debugCreationErrors, final CompactStyle style,
	...
	public boolean equals(final Object obj) {
		...
		return mapsAreEqual(units, other.units) && Arrays.equals(divisor, other.divisor)
				&& Arrays.equals(currencyAffixes, other.currencyAffixes) && pluralRules.equals(other.pluralRules);
	}

	private boolean mapsAreEqual(final Map<String, DecimalFormat.Unit[]> lhs, final Map<String, DecimalFormat.Unit[]> rhs) {
	...
	private Map<String, DecimalFormat.Unit[]> otherPluralVariant(final String[] prefix, final String[] suffix) {
	 ...
	 */
	private Data getData(final ULocale locale, final CompactStyle style) {
