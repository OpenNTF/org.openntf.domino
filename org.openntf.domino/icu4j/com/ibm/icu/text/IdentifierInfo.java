...
/**
 * This class analyzes a possible identifier for script and identifier status. Use it by calling setIdentifierProfile then setIdentifier.
 * Available methods include:
 * <ol>
 * <li>call getScripts for the specific scripts in the identifier. The identifier contains at least one character in each of these.
 * <li>call getAlternates to get cases where a character is not limited to a single script. For example, it could be either Katakana or
 * Hiragana.
 * <li>call getCommonAmongAlternates to find out if any scripts are common to all the alternates.
 * <li>call getNumerics to get a representative character (with value zero) for each of the decimal number systems in the identifier.
...
public class IdentifierInfo {
	...
	/**
	 * Create an identifier info object. Subsequently, call {@link #setIdentifier(String)}, etc. {@link #setIdentifierProfile(UnicodeSet)}
	 * 
	 * @internal
	 * @deprecated This API is ICU internal only.
	 */
	...
	public IdentifierInfo setIdentifier(String identifier) {
		...
		for (int i = 0; i < identifier.length(); i += Character.charCount(i)) {
			...
			if (UCharacter.getType(cp) == ECharacterCategory.DECIMAL_DIGIT_NUMBER) {
	...
	/**
	 * Get the set of alternate scripts found in the identifiers. That is, when a character can be in two scripts, then the set consisting
	 * of those scripts will be returned.
	 * 
	 * @return the set of explicit scripts.
	 * @internal
	 * @deprecated This API is ICU internal only.
	...
	private final static BitSet JAPANESE = set(new BitSet(), UScript.LATIN, UScript.HAN, UScript.HIRAGANA, UScript.KATAKANA);
	...
	private final static BitSet CONFUSABLE_WITH_LATIN = set(new BitSet(), UScript.CYRILLIC, UScript.GREEK, UScript.CHEROKEE);
	...
	/**
	 * Get the number of scripts appearing in the identifier. Note: Common and Inherited scripts are omitted from the count. Note: If the
	 * identifier contains characters with alternate scripts (the character is used with more than one script), minimize the reported number
	 * of scripts by considering the character to be of a script that already appears elsewhere in the identifier when possible. The
	 * alternate script computation may not be perfect. The distinction between 0, 1 and > 1 scripts will be valid, however.
	 * 
	...
	public String toString() {
		return identifier + ", " + identifierProfile.toPattern(false) + ", " + getRestrictionLevel() + ", "
				+ displayScripts(requiredScripts) + ", " + displayAlternates(scriptSetSet) + ", " + numerics.toPattern(false);
	}
	 ...
	 * 
	 * @param alternates
	 *            a set of BitSets of script values.
	 * @return display form
	 * @internal
	 * @deprecated This API is ICU internal only.
	 */
	 ...
	 *            a list of alternates, separated by ;
	 ...
	 * 
	 * @param container
	 *            possible container to be tested
	 * @param containee
	 *            possible containee to be tested
	 ...
	 * 
	 * @param bitset
	 *            bitset to be affected
	 * @param values
	 *            values to be set in the bitset
