...
/**
 * This class is part of the Rule Based Break Iterator rule compiler. It scans the rules and builds the parse tree. There is no public API
 * here.
 */
	...
	void parse() {
		...
		for (;;) {
			...
			if (fRB.fDebugEnv != null && fRB.fDebugEnv.indexOf("scan") >= 0) {
				System.out.println("char, line, col = (\'" + (char) fC.fChar + "\', " + fLineNum + ", " + fCharNum + "    state = "
						+ tableEl.fStateName);
			}
