...
/**
 * A transliterator that converts Unicode escape forms to the characters they represent. Escape forms have a prefix, a suffix, a radix, and
...
class UnescapeTransliterator extends Transliterator {
	...
	/**
	 * The encoded pattern specification. The pattern consists of zero or more forms. Each form consists of a prefix, suffix, radix, minimum
	 * digit count, and maximum digit count. These values are stored as a five character header. That is, their numeric values are cast to
	 * 16-bit characters and stored in the string. Following these five characters, the prefix characters, then suffix characters are
	 * stored. Each form thus takes n+5 characters, where n is the total length of the prefix and suffix. The end is marked by a header of
	 * length one consisting of the character END.
	 */
	private char spec[];
	...
	static void register() {
		...
		Transliterator.registerFactory("Hex-Any", new Transliterator.Factory() {
			public Transliterator getInstance(String ID) {
				return new UnescapeTransliterator("Hex-Any", new char[] { 2, 0, 16, 4, 6, 'U', '+',            // Unicode
						2, 0, 16, 4, 4, '\\', 'u',           // Java
