...
/**
 * A mutable set of Unicode characters and multicharacter strings. Objects of this class represent <em>character classes</em> used in
 * regular expressions. A character specifies a subset of Unicode code points. Legal code points are U+0000 to U+10FFFF, inclusive.
 * 
 * Note: method freeze() will not only makes the set immutable, but also makes important methods much higher performance: contains(c),
 * containsNone(...), span(...), spanBack(...) etc. After the object is frozen, any subsequent call that wants to change the object will
 ...
 * 
 * <p>
 * <code>UnicodeSet</code> supports two APIs. The first is the <em>operand</em> API that allows the caller to modify the value of a
 * <code>UnicodeSet</code> object. It conforms to Java 2's <code>java.util.Set</code> interface, although <code>UnicodeSet</code> does not
 * actually implement that interface. All methods of <code>Set</code> are supported, with the modification that they take a character range
 * or single character instead of an <code>Object</code>, and they take a <code>UnicodeSet</code> instead of a <code>Collection</code>. The
 * operand API may be thought of in terms of boolean logic: a boolean OR is implemented by <code>add</code>, a boolean AND is implemented by
 * <code>retain</code>, a boolean XOR is implemented by <code>complement</code> taking an argument, and a boolean NOT is implemented by
 * <code>complement</code> with no argument. In terms of traditional set theory function names, <code>add</code> is a union,
 * <code>retain</code> is an intersection, <code>remove</code> is an asymmetric difference, and <code>complement</code> with no argument is
 * a set complement with respect to the superset range <code>MIN_VALUE-MAX_VALUE</code>
 * 
 * <p>
 * The second API is the <code>applyPattern()</code>/<code>toPattern()</code> API from the <code>java.text.Format</code>-derived classes.
 * Unlike the methods that add characters, add categories, and control the logic of the set, the method <code>applyPattern()</code> sets all
 * attributes of a <code>UnicodeSet</code> at once, based on a string pattern.
 * 
 * <p>
 * <b>Pattern syntax</b>
 * </p>
 * 
 * Patterns are accepted by the constructors and the <code>applyPattern()</code> methods and returned by the <code>toPattern()</code>
 ...
 * <td valign="top">The characters 'a' through 'e' inclusive, in Unicode code point order</td>
 * </tr>
 * <tr>
 ...
 * <td valign="top">The character 'a' and the multicharacter strings &quot;ab&quot; and &quot;ac&quot;</td>
 * </tr>
 * <tr>
 ...
 * </blockquote>
 * 
 * Any character may be preceded by a backslash in order to remove any special meaning. White space characters, as defined by the Unicode
 * Pattern_White_Space property, are ignored, unless they are escaped.
 * 
 * <p>
 * Property patterns specify a set of characters having a certain property as defined by the Unicode standard. Both the POSIX-like "[:Lu:]"
 * and the Perl-like syntax "\p{Lu}" are recognized. For a complete list of supported property patterns, see the User's Guide for UnicodeSet
 * at <a href="http://www.icu-project.org/userguide/unicodeSet.html"> http://www.icu-project.org/userguide/unicodeSet.html</a>. Actual
 * determination of property data is defined by the underlying Unicode database as implemented by UCharacter.
 * 
 * <p>
 * Patterns specify individual characters, ranges of characters, and Unicode property sets. When elements are concatenated, they specify
 * their union. To complement a set, place a '^' immediately after the opening '['. Property patterns are inverted by modifying their
 * delimiters; "[:^foo]" and "\P{foo}". In any other location, '^' has no special meaning.
 * 
 * <p>
 * Ranges are indicated by placing two a '-' between two characters, as in "a-z". This specifies the range of all characters from the left
 * to the right, in Unicode order. If the left character is greater than or equal to the right character it is a syntax error. If a '-'
 * occurs as the first character after the opening '[' or '[^', or if it occurs as the last character before the closing ']', then it is
 * taken as a literal. Thus "[a\\-b]", "[-ab]", and "[ab-]" all indicate the same set of three characters, 'a', 'b', and '-'.
 * 
 * <p>
 * Sets may be intersected using the '&' operator or the asymmetric set difference may be taken using the '-' operator, for example,
 * "[[:L:]&[\\u0000-\\u0FFF]]" indicates the set of all Unicode letters with values less than 4096. Operators ('&' and '|') have equal
 * precedence and bind left-to-right. Thus "[[:L:]-[a-z]-[\\u0100-\\u01FF]]" is equivalent to "[[[:L:]-[a-z]]-[\\u0100-\\u01FF]]". This only
 * really matters for difference; intersection is commutative.
 * 
 * <table>
 * <tr valign=top>
 * <td nowrap><code>[a]</code>
 * <td>The set containing 'a'
 * <tr valign=top>
 * <td nowrap><code>[a-z]</code>
 * <td>The set containing 'a' through 'z' and all letters in between, in Unicode order
 * <tr valign=top>
 * <td nowrap><code>[^a-z]</code>
 * <td>The set containing all characters but 'a' through 'z', that is, U+0000 through 'a'-1 and 'z'+1 through U+10FFFF
 * <tr valign=top>
 ...
 * <tr valign=top>
 * <td nowrap><code>[[<em>pat1</em>]-[<em>pat2</em>]]</code>
 * <td>The asymmetric difference of sets specified by <em>pat1</em> and <em>pat2</em>
 * <tr valign=top>
 * <td nowrap><code>[:Lu:] or \p{Lu}</code>
 * <td>The set of characters having the specified Unicode property; in this case, Unicode uppercase letters
 * <tr valign=top>
 * <td nowrap><code>[:^Lu:] or \P{Lu}</code>
...
public class UnicodeSet extends UnicodeFilter implements Iterable<String>, Comparable<UnicodeSet>, Freezable<UnicodeSet> {
	...
	/**
	 * The pattern representation of this set. This may not be the most economical pattern. It is the pattern supplied to applyPattern(),
	 * with variables substituted and whitespace removed. For sets constructed without applyPattern(), or modified using the non-pattern
	 * API, this string will be null, indicating that toPattern() must generate a pattern representation from the inversion list.
	 */
	...
	/**
	 * A set of all characters _except_ the second through last characters of certain ranges. These ranges are ranges of characters whose
	 * properties are all exactly alike, e.g. CJK Ideographs from U+4E00 to U+9FA5.
	 */
	...
	/**
	 * Quickly constructs a set from a set of ranges <s0, e0, s1, e1, s2, e2, ..., sn, en>. There must be an even number of integers, and
	 * they must be all greater than zero, all less than or equal to Character.MAX_CODE_POINT. In each pair (..., si, ei, ...) it must be
	 * true that si <= ei Between adjacent pairs (...ei, sj...), it must be true that ei+1 < sj
	 * 
	 * @param pairs
	 ...
	 *            a string specifying what characters are in the set
	 * @param options
	 *            a bitmask indicating which options to apply. Valid options are IGNORE_SPACE and CASE.
	 * @exception java.lang.IllegalArgumentException
	 *                if the pattern contains a syntax error.
	 * @stable ICU 3.8
	 */
	...
	/**
	 * Make this object represent the range <code>start - end</code>. If <code>end > start</code> then this object is set to an an empty
	 * range.
	 * 
	 * @param start
	 ...
	 * 
	 * @param other
	 *            a <code>UnicodeSet</code> whose value will be copied to this object
	 * @stable ICU 2.0
	 */
	...
	/**
	 * Modifies this set to represent the set specified by the given pattern. See the class description for the syntax of the pattern
	 * language. Whitespace is ignored.
	 * 
	 * @param pattern
	 *            a string specifying what characters are in the set
	 * @exception java.lang.IllegalArgumentException
	...
	/**
	 * Modifies this set to represent the set specified by the given pattern, optionally ignoring whitespace. See the class description for
	...
	/**
	 * Returns a string representation of this set. If the result of calling this function is passed to a UnicodeSet constructor, it will
	 * produce another set that is equal to this one.
	 * 
	 * @stable ICU 2.0
	 */
	...
	/**
	 * Append a string representation of this set to result. This will be a cleaned version of the string passed to applyPattern(), if there
	...
	/**
	 * Generate and append a string representation of this set to result. This does not use this.pat, the cleaned up copy of the string
	 * passed to applyPattern().
	 * 
	 * @param result
	 *            the buffer into which to generate the pattern
	...
	/**
	 * Returns the number of elements in this set (its cardinality) Note than the elements of a set may include both individual codepoints
	 * and strings.
	 * 
	...
	/**
	 * Implementation of UnicodeMatcher API. Returns <tt>true</tt> if this set contains any character whose low byte is the given value.
	 * This is used by <tt>RuleBasedTransliterator</tt> for indexing.
	 * 
	 * @stable ICU 2.0
	 */
	...
	/**
	 * Returns the longest match for s in text at the given position. If limit > start then match forward from start+1 to limit matching all
	 * characters except s.charAt(0). If limit < start, go backward starting from start-1 matching all characters except
	 * s.charAt(s.length()-1). This method assumes that the first character, text.charAt(start), matches s, so it does not check it.
	 * 
	 * @param text
	 *            the text to match
	 * @param start
	 *            the first character to match. In the forward direction, text.charAt(start) is matched against s.charAt(0). In the reverse
	 *            direction, it is matched against s.charAt(s.length()-1).
	 * @param limit
	 *            the limit offset for matching, either last+1 in the forward direction, or last-1 in the reverse direction, where last is
	 *            the index of the last character to match.
	 * @return If part of s matches up to the limit, return |limit - start|. If all of s matches before reaching the limit, return
	 *         s.length(). If there is a mismatch between s and text, return 0
	 */
	 ...
	 * @param substring
	 *            substring to match at offset in text
	...
	/**
	 * Returns the index of the given character within this set, where the set is ordered by ascending code point. If the character is not
	 * in this set, return -1. The inverse of this method is <code>charAt()</code>.
	 * 
	...
	/**
	 * Returns the character at the given index within this set, where the set is ordered by ascending code point. If the index is out of
	 * range, return -1. The inverse of this method is <code>indexOf()</code>.
	 * 
	 * @param index
	...
	/**
	 * Adds the specified range to this set if it is not already present. If this set already contains the specified range, the call leaves
	 * this set unchanged. If <code>end > start</code> then an empty range is added, leaving the set unchanged.
	 * 
	 * @param start
	...
	/**
	 * Adds the specified character to this set if it is not already present. If this set already contains the specified character, the call
	...
	/**
	 * Adds the specified multicharacter to this set if it is not already present. If this set already contains the multicharacter, the call
	 * leaves this set unchanged. Thus "ch" => {"ch"} <br>
	...
	/**
	 * Adds each of the characters in this string to the set. Thus "ch" => {"c", "h"} If this set already any particular character, it has
	 * no effect on that character.
	 * 
	 * @param s
	 *            the source string
	 * @return this object, for chaining
	...
	/**
	 * Retains EACH of the characters in this string. Note: "ch" == {"c", "h"} If this set already any particular character, it has no
	 * effect on that character.
	 * 
	 * @param s
	 *            the source string
	 * @return this object, for chaining
	...
	/**
	 * Complement EACH of the characters in this string. Note: "ch" == {"c", "h"} If this set already any particular character, it has no
	 * effect on that character.
	 * 
	 * @param s
	 *            the source string
	...
	/**
	 * Remove EACH of the characters in this string. Note: "ch" == {"c", "h"} If this set already any particular character, it has no effect
	 * on that character.
	 * 
	 * @param s
	 *            the source string
	...
	/**
	 * Retain only the elements in this set that are contained in the specified range. If <code>end > start</code> then an empty range is
	 * retained, leaving the set empty.
	 * 
	 * @param start
	 *            first character, inclusive, of range to be retained to this set.
	 * @param end
	 *            last character, inclusive, of range to be retained to this set.
	 * @stable ICU 2.0
	 */
	...
	/**
	 * Retain the specified character from this set if it is present. Upon return this set will be empty if it did not contain c, or will
	 * only contain c if it did contain c.
	 * 
	 * @param c
	...
	/**
	 * Retain the specified string in this set if it is present. Upon return this set will be empty if it did not contain s, or will only
	 * contain s if it did contain s.
	 * 
	 * @param s
	...
	/**
	 * Removes the specified range from this set if it is present. The set will not contain the specified range once the call returns. If
	 * <code>end > start</code> then an empty range is removed, leaving the set unchanged.
	 * 
	 * @param start
	...
	/**
	 * Removes the specified character from this set if it is present. The set will not contain the specified character once the call
	 * returns.
	 * 
	 * @param c
	...
	/**
	 * Complements the specified range in this set. Any character in the range will be removed if it is in this set, or will be added if it
	 * is not in this set. If <code>end > start</code> then an empty range is complemented, leaving the set unchanged.
	 * 
	 * @param start
	 *            first character, inclusive, of range to be removed from this set.
	 * @param end
	...
	/**
	 * Complements the specified character in this set. The character will be removed if it is in this set, or will be added if it is not in
	 * this set.
	 * 
	 * @stable ICU 2.0
	 */
	...
	/**
	 * Complement the specified string in this set. The set will not contain the specified string once the call returns. <br>
	 * <b>Warning: you cannot add an empty string ("") to a UnicodeSet.</b>
	 * 
	 * @param s
	...
	/**
	 * Returns the smallest value i such that c < list[i]. Caller must ensure that c is a legal value or this method will enter an infinite
	 * loop. This method performs a binary search.
	 * 
	 * @param c
	...
	/**
	 * Returns true if there is a partition of the string such that this set contains each of the partitioned strings. For example, for the
	 * Unicode set [a{bc}{cd}]<br>
	...
	/**
	 * Returns true if none of the characters or strings in this UnicodeSet appears in the string. For example, for the Unicode set
	 * [a{bc}{cd}]<br>
	...
	/**
	 * Adds all of the elements in the specified set to this set if they're not already present. This operation effectively modifies this
	 * set so that its value is the <i>union</i> of the two sets. The behavior of this operation is unspecified if the specified collection
	...
	/**
	 * Retains only the elements in this set that are contained in the specified set. In other words, removes from this set all of its
	 * elements that are not contained in the specified set. This operation effectively modifies this set so that its value is the
	...
	/**
	 * Removes from this set all of its elements that are contained in the specified set. This operation effectively modifies this set so
	...
	/**
	 * Complements in this set all elements contained in the specified set. Any character in the other set will be removed if it is in this
	 * set, or will be added if it is not in this set.
	 * 
	 * @param c
	 ...
	 *                if index is outside the range <code>0..getRangeCount()-1</code>
	 * @see #getRangeCount
	 * @see #getRangeEnd
	 * @stable ICU 2.0
	 */
	...
	/**
	 * Compares the specified object with this set for equality. Returns <tt>true</tt> if the specified object is also a set, the two sets
	 * have the same size, and every member of the specified set is contained in this set (or equivalently, every member of this set is
	...
	/**
	 * Parses the given pattern, starting at the given position. The character at pattern.charAt(pos.getIndex()) must be '[', or the parse
	 * fails. Parsing continues until the corresponding closing ']'. If a syntax error is encountered between the opening and closing brace,
	 * the parse fails. Upon return from a successful parse, the ParsePosition is updated to point to the character following the closing
	 * ']', and an inversion list for the parsed pattern is returned. This method calls itself recursively to parse embedded subpatterns.
	 * 
	 * @param pattern
	 *            the string containing the pattern to be parsed. The portion of the string from pos.getIndex(), which must be a '[', to the
	 *            corresponding closing ']', is parsed.
	 * @param pos
	 *            upon entry, the position at which to being parsing. The character at pattern.charAt(pos.getIndex()) must be a '['. Upon
	 *            return from a successful parse, pos.getIndex() is either the character after the closing ']' of the parsed pattern, or
	 ...
	 * 
	 * @param chars
	 *            iterator over the pattern characters. Upon return it will be advanced to the first character after the parsed pattern, or
	 *            the end of the iteration if all characters are parsed.
	 * @param symbols
	 *            symbol table to use to parse and dereference variables, or null if none.
	 * @param rebuiltPat
	 *            the pattern that was parsed, rebuilt or copied from the input pattern, as appropriate.
	 * @param options
	 *            a bit mask of zero or more of the following: IGNORE_SPACE, CASE.
	 */
	void applyPattern(final RuleCharacterIterator chars, final SymbolTable symbols, final StringBuffer rebuiltPat, final int options) {
		...
		/**
		 * Handle global flags (invert, case insensitivity). If this pattern should be compiled case-insensitive, then we need to close over
		 * case BEFORE COMPLEMENTING. This makes patterns like /[^abc]/i work.
		 */
		if ((options & CASE) != 0) {
			closeOver(CASE);
		}
	...
	/**
	 * Modifies this set to contain those code points which have the given value for the given binary or enumerated property, as returned by
	 * UCharacter.getIntPropertyValue. Prior contents of this set are lost.
	 * 
	 * @param prop
	 *            a property in the range UProperty.BIN_START..UProperty.BIN_LIMIT-1 or UProperty.INT_START..UProperty.INT_LIMIT-1 or.
	 *            UProperty.MASK_START..UProperty.MASK_LIMIT-1.
	 * 
	 * @param value
	 *            a value in the range UCharacter.getIntPropertyMinValue(prop).. UCharacter.getIntPropertyMaxValue(prop), with one
	 *            exception. If prop is UProperty.GENERAL_CATEGORY_MASK, then value should not be a UCharacter.getType() result, but rather
	 *            a mask value produced by logically ORing (1 << UCharacter.getType()) values together. This allows grouped categories such
	 *            as [:L:] to be represented.
	 * 
	...
	/**
	 * Modifies this set to contain those code points which have the given value for the given property. Prior contents of this set are
	 * lost.
	 * 
	 * @param propertyAlias
	 *            a property alias, either short or long. The name is matched loosely. See PropertyAliases.txt for names and a description
	 *            of loose matching. If the value string is empty, then this string is interpreted as either a General_Category value alias,
	 *            a Script value alias, a binary property alias, or a special ID. Special IDs are matched loosely and correspond to the
	 *            following sets:
	 * 
	 *            "ANY" = [\u0000-\U0010FFFF], "ASCII" = [\u0000-\u007F].
	 * 
	 * @param valueAlias
	 *            a value alias, either short or long. The name is matched loosely. See PropertyValueAliases.txt for names and a description
	 *            of loose matching. In addition to aliases listed, numeric values and canonical combining classes may be expressed
	...
	public UnicodeSet applyPropertyAlias(final String propertyAlias, final String valueAlias, final SymbolTable symbols) {
		...
		if (symbols != null && (symbols instanceof XSymbolTable)
		...
		if (valueAlias.length() > 0) {
			...
			if ((p >= UProperty.BINARY_START && p < UProperty.BINARY_LIMIT) || (p >= UProperty.INT_START && p < UProperty.INT_LIMIT)
					|| (p >= UProperty.MASK_START && p < UProperty.MASK_LIMIT)) {
				try {
					v = UCharacter.getPropertyValueEnum(p, valueAlias);
				} catch (IllegalArgumentException e) {
					// Handle numeric CCC
					if (p == UProperty.CANONICAL_COMBINING_CLASS || p == UProperty.LEAD_CANONICAL_COMBINING_CLASS
	...
	/**
	 * Return true if the given iterator appears to point at a property pattern. Regardless of the result, return with the iterator
	 * unchanged.
	 * 
	 * @param chars
	 *            iterator over the pattern characters. Upon return it will be unchanged.
	 * @param iterOpts
	 *            RuleCharacterIterator options
	 */
	...
	/**
	 * Bitmask for constructor and applyPattern() indicating that white space should be ignored. If set, ignore Unicode Pattern_White_Space
	 * characters, unless they are quoted or escaped. This may be ORed together with other selectors.
	 * 
	 * @stable ICU 3.8
	 */
	...
	/**
	 * Bitmask for constructor, applyPattern(), and closeOver() indicating letter case. This may be ORed together with other selectors.
	 * 
	 * Enable case insensitive matching. E.g., "[ab]" with this flag will match 'a', 'A', 'b', and 'B'. "[^ab]" with this flag will match
	 * all except 'a', 'A', 'b', and 'B'. This performs a full closure over case mappings, e.g. U+017F for s.
	 * 
	 * The resulting set is a superset of the input for the code points but not for the strings. It performs a case mapping closure of the
	 * code points and adds full case folding strings for the code points, and reduces strings of the original set to their full case
	 * folding equivalents.
	 * 
	 * This is designed for case-insensitive matches, for example in regular expressions. The full code point case closure allows checking
	 * of an input character directly against the closure set. Strings are matched by comparing the case-folded form from the closure set
	 * with an incremental case folding of the string in question.
	 * 
	 * The closure set will also contain single code points if the original set contained case-equivalent strings (like U+00DF for "ss" or
	 * "Ss" etc.). This is not necessary (that is, redundant) for the above matching method but results in the same closure sets regardless
	 * of whether the original set contained the code point or a string.
	 * 
	 * @stable ICU 3.8
	 */
	 ...
	 * 
	 * Enable case insensitive matching. E.g., "[ab]" with this flag will match 'a', 'A', 'b', and 'B'. "[^ab]" with this flag will match
	 * all except 'a', 'A', 'b', and 'B'. This adds the lower-, title-, and uppercase mappings as well as the case folding of each existing
	 * element in the set.
	 * 
	 * @stable ICU 3.4
	 */
	...
	/**
	 * Close this set over the given attribute. For the attribute CASE, the result is to modify this set so that:
	 * 
	 * 1. For each character or string 'a' in this set, all strings 'b' such that foldCase(a) == foldCase(b) are added to this set. (For
	 * most 'a' that are single characters, 'b' will have b.length() == 1.)
	 * 
	 * 2. For each string 'e' in the resulting set, if e != foldCase(e), 'e' will be removed.
	 * 
	 * Example: [aq\u00DF{Bc}{bC}{Fi}] => [aAqQ\u00DF\uFB01{ss}{bc}{fi}]
	 * 
	 * (Here foldCase(x) refers to the operation UCharacter.foldCase(x, true), and a == b actually denotes a.equals(b), not pointer
	 * comparison.)
	 * 
	 * @param attribute
	 *            bitmask for attributes to close over. Currently only the CASE bit is supported. Any undefined bits are ignored.
	 * @return a reference to this set.
	 * @stable ICU 3.8
	 */
	...
	/**
	 * Span a string using this UnicodeSet. If the start index is less than 0, span will start from 0. If the start index is greater than
	 * the string length, span returns the string length.
	 * 
	 * @param s
	 *            The string to be spanned
	 * @param start
	...
	/**
	 * Span a string backwards (from the fromIndex) using this UnicodeSet. If the fromIndex is less than 0, spanBack will return 0. If
	 * fromIndex is greater than the string length, spanBack will start from the string length.
	 * 
	 * @param s
	 *            The string to be spanned
	 * @param fromIndex
	...
	/**
	 * Compares UnicodeSets, where shorter come first, and otherwise lexigraphically (according to the comparison of the first characters
	 * that differ).
	 * 
	...
	/**
	 * Utility to compare a string to a code point. Same results as turning the code point into a string (with the [ugly] new
	 * StringBuilder().appendCodePoint(codepoint).toString()) and comparing, but much faster (no object creation). Actually, there is one
	 * difference; a null compares as less. Note that this (=String) order is UTF-16 order -- *not* code point order.
	 * 
	 * @stable ICU 4.4
	 */
	...
	/**
	 * Utility to compare a string to a code point. Same results as turning the code point into a string and comparing, but much faster (no
	 * object creation). Actually, there is one difference; a null compares as less. Note that this (=String) order is UTF-16 order -- *not*
	 * code point order.
	 * 
	 * @stable ICU 4.4
	 */
	...
	/**
	 * Utility to compare two iterables. Warning: the ordering in iterables is important. For Collections that are ordered, like Lists, that
	 * is expected. However, Sets in Java violate Leibniz's law when it comes to iteration. That means that sets can't be compared directly
	 * with this method, unless they are TreeSets without (or with the same) comparator. Unfortunately, it is impossible to reliably detect
	 * in Java whether subclass of Collection satisfies the right criteria, so it is left to the user to avoid those circumstances.
	 * 
	 * @stable ICU 4.4
	 */
	...
	/**
	 * Utility to compare two iterators. Warning: the ordering in iterables is important. For Collections that are ordered, like Lists, that
	 * is expected. However, Sets in Java violate Leibniz's law when it comes to iteration. That means that sets can't be compared directly
	 * with this method, unless they are TreeSets without (or with the same) comparator. Unfortunately, it is impossible to reliably detect
	 * in Java whether subclass of Collection satisfies the right criteria, so it is left to the user to avoid those circumstances.
	 * 
	 ...
	 * @stable ICU 4.4
	 */
	public static <T extends Comparable<T>> int compare(final Collection<T> collection1, final Collection<T> collection2,
	...
	/**
	 * Simplify the ranges in a Unicode set by merging any ranges that are only separated by characters in the dontCare set. For example,
	 * the ranges: \\u2E80-\\u2E99\\u2E9B-\\u2EF3\\u2F00-\\u2FD5\\u2FF0-\\u2FFB\\u3000-\\u303E change to \\u2E80-\\u303E if the dontCare set
	 * includes unassigned characters (for a particular version of Unicode).
	 * 
	 * @param dontCare
	 *            Set with the don't-care characters for spanning
	...
	public UnicodeSet addBridges(final UnicodeSet dontCare) {
		...
		for (UnicodeSetIterator it = new UnicodeSetIterator(notInInput); it.nextRange();) {
			if (it.codepoint != 0 && it.codepoint != UnicodeSetIterator.IS_STRING && it.codepointEnd != 0x10FFFF
	...
	/**
	 * Find the first index at or after fromIndex where the UnicodeSet matches at that index. If findNot is true, then reverse the sense of
	 * the match: find the first place where the UnicodeSet doesn't match. If there is no match, length is returned.
	 * 
	 * @internal
	 * @deprecated This API is ICU internal only.
	 */
	...
	/**
	 * Find the last index before fromIndex where the UnicodeSet matches at that index. If findNot is true, then reverse the sense of the
	 * match: find the last place where the UnicodeSet doesn't match. If there is no match, -1 is returned. BEFORE index is not in the
	 * UnicodeSet.
	 * 
	 * @internal
	 * @deprecated This API is ICU internal only.
	...
	/**
	 * Strips code points from source. If matches is true, script all that match <i>this</i>. If matches is false, then strip all that
	 * <i>don't</i> match.
	 * 
	 * @param source
	 *            The source of the CharSequence to strip from.
	 * @param matches
	 *            A boolean to either strip all that matches or don't match with the current UnicodeSet object.
	...
	/**
	 * Argument values for whether span() and similar functions continue while the current character is contained vs. not contained in the
	 * set.
	 * <p>
	 ...
	 * <li>span() and spanBack() partition any string the same way when alternating between span(NOT_CONTAINED) and span(either "contained"
	 * condition).
	 * <li>Using a complemented (inverted) set and the opposite span conditions yields the same results.
	 * </ul>
	 * When a set contains multi-code point strings, then these statements may not be true, depending on the strings in the set (for
	 * example, whether they overlap with each other) and the string that is processed. For a set with strings:
	 * <ul>
	 * <li>The complement of the set contains the opposite set of code points, but the same set of strings. Therefore, complementing both
	 * the set and the span conditions may yield different results.
	 * <li>When starting spans at different positions in a string (span(s, ...) vs. span(s+1, ...)) the ends of the spans may be different
	 * because a set string may start before the later position.
	 * <li>span(SIMPLE) may be shorter than span(CONTAINED) because it will not recursively try all possible paths. For example, with a set
	 * which contains the three strings "xy", "xya" and "ax", span("xyax", CONTAINED) will return 4 but span("xyax", SIMPLE) will return 3.
	 * span(SIMPLE) will never be longer than span(CONTAINED).
	 * <li>With either "contained" condition, span() and spanBack() may partition a string in different ways. For example, with a set which
	 * contains the two strings "ab" and "ba", and when processing the string "aba", span() will yield contained/not-contained boundaries of
	 * { 0, 2, 3 } while spanBack() will yield boundaries of { 0, 1, 3 }.
	 * </ul>
	 * Note: If it is important to get the same boundaries whether iterating forward or backward through a string, then either only span()
	 * should be used and the boundaries cached for backward operation, or an ICU BreakIterator could be used.
	 * <p>
	 * Note: Unpaired surrogates are treated like surrogate code points. Similarly, set strings match only on code point boundaries, never
	...
	public enum SpanCondition {
		/**
		 * Continue a span() while there is no set element at the current position. Stops before the first set element (character or
		 * string). (For code points only, this is like while contains(current)==FALSE).
		 * <p>
		 * When span() returns, the substring between where it started and the position it returned consists only of characters that are not
		 * in the set, and none of its strings overlap with the span.
		 * 
		 * @stable ICU 4.4
		 */
		NOT_CONTAINED,
		...
		/**
		 * Continue a span() while there is a set element at the current position. (For characters only, this is like while
		 * contains(current)==TRUE).
		 * <p>
		 * When span() returns, the substring between where it started and the position it returned consists only of set elements
		 * (characters or strings) that are in the set.
		 * <p>
		 * If a set contains strings, then the span will be the longest substring matching any of the possible concatenations of set
		 * elements (characters or strings). (There must be a single, non-overlapping concatenation of characters or strings.) This is
		 * equivalent to a POSIX regular expression for (OR of each set element)*.
		 * 
		 * @stable ICU 4.4
		 */
		 ...
		 * <p>
		 * If a set contains strings, then the span will be the longest substring with a match at each position with the longest single set
	...
	/**
	 * Set the default symbol table. Null means ordinary processing. For internal use only. Will affect all subsequent parsing of
	 * UnicodeSets.
	 * <p>
	 * WARNING: If this function is used with a UnicodeProperty, and the Unassigned characters (gc=Cn) are different than in ICU other than
	 * in ICU, you MUST call {@code UnicodeProperty.ResetCacheProperties} afterwards. If you then call
	 * {@code UnicodeSet.setDefaultXSymbolTable} with null to clear the value, you MUST also call
	 * {@code UnicodeProperty.ResetCacheProperties}.
	 * 
	 * @param xSymbolTable
	 *            the new default symbol table.
	 * @internal
	 */
