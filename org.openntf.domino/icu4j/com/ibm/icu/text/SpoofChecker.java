 ...
 * 
 * <p>
 * This class is intended to check strings, typically identifiers of some type, such as URLs, for the presence of characters that are likely
 * to be visually confusing - for cases where the displayed form of an identifier may not be what it appears to be.
 * 
 * <p>
 * Unicode Technical Report #36, <a href="http://unicode.org/reports/tr36">http://unicode.org/reports/tr36</a> and Unicode Technical
 * Standard #39, <a href="http://unicode.org/reports/tr39">http://unicode.org/reports/tr39</a> "Unicode security considerations", give more
 * background on security and spoofing issues with Unicode identifiers. The tests and checks provided by this module implement the
 * recommendations from these Unicode documents.
 * 
 * <p>
 * The tests available on identifiers fall into two general categories:
 * <ul>
 * <li>Single identifier tests. Check whether an identifier is potentially confusable with any other string, or is suspicious for other
 * reasons.</li>
 * <li>Two identifier tests. Check whether two specific identifiers are confusable. This does not consider whether either of strings is
 * potentially confusable with any string other than the exact one specified.</li>
 * </ul>
 * 
 * <p>
 ...
 * <li>Configure the Builder for the desired set of tests. The tests that will be performed are specified by a set of SpoofCheck flags.</li>
 * <li>Build a <code>SpoofChecker</code> from the Builder.</li>
 * <li>Perform the checks using the pre-configured <code>SpoofChecker</code>. The results indicate which (if any) of the selected tests have
 * identified possible problems with the identifier. Results are reported as a set of SpoofCheck flags; this mirrors the form in which the
 * set of tests to perform was originally specified to the SpoofChecker.</li>
 * </ul>
 * 
 * <p>
 ...
 * 
 * <p>
 * Thread Safety: The methods on SpoofChecker objects are thread safe. The test functions for checking a single identifier, or for testing
 * whether two identifiers are potentially confusable, may called concurrently from multiple threads using the same SpoofChecker instance.
 * 
 * 
 * <p>
 ...
 * 
 * <p>
 * When testing whether pairs of identifiers are confusable, with <code>areConfusable()</code> the relevant tests are
 * 
 * <ul>
 * <li> <code>SINGLE_SCRIPT_CONFUSABLE</code>: All of the characters from the two identifiers are from a single script, and the two
 * identifiers are visually confusable.</li>
 * <li> <code>MIXED_SCRIPT_CONFUSABLE</code>: At least one of the identifiers contains characters from more than one script, and the two
 * identifiers are visually confusable.</li>
 * <li> <code>WHOLE_SCRIPT_CONFUSABLE</code>: Each of the two identifiers is of a single script, but the the two identifiers are from
 * different scripts, and they are visually confusable.</li>
 * </ul>
 * 
 * <p>
 ...
 * 
 * <p>
 * <code>ANY_CASE</code> is a modifier for the above tests. If the identifiers being checked can be of mixed case and are used in a
 * case-sensitive manner, this option should be specified.
 * 
 * <p>
 * If the identifiers being checked are used in a case-insensitive manner, and if they are displayed to users in lower-case form only, the
 * <code>ANY_CASE</code> option should not be specified. Confusabality issues involving upper case letters will not be reported.
 * 
 * <p>
 * When performing tests on a single identifier, with the check() family of functions, the relevant tests are:
 * 
 * <ul>
 * <li><code>MIXED_SCRIPT_CONFUSABLE</code>: the identifier contains characters from multiple scripts, and there exists an identifier of a
 * single script that is visually confusable.</li>
 * <li><code>WHOLE_SCRIPT_CONFUSABLE</code>: the identifier consists of characters from a single script, and there exists a visually
 * confusable identifier. The visually confusable identifier also consists of characters from a single script. but not the same script as
 * the identifier being checked.</li>
 * <li><code>ANY_CASE</code>: modifies the mixed script and whole script confusables tests. If specified, the checks will find confusable
 * characters of any case. If this flag is not set, the test is performed assuming case folded identifiers.</li>
 * <li><code>SINGLE_SCRIPT</code>: check that the identifier contains only characters from a single script. (Characters from the
 * <em>common</em> and <em>inherited</em> scripts are ignored.) This is not a test for confusable identifiers</li>
 * <li><code>INVISIBLE</code>: check an identifier for the presence of invisible characters, such as zero-width spaces, or character
 * sequences that are likely not to display, such as multiple occurrences of the same non-spacing mark. This check does not test the input
 * string as a whole for conformance to any particular syntax for identifiers.</li>
 * <li><code>CHAR_LIMIT</code>: check that an identifier contains only characters from a specified set of acceptable characters. See
 * <code>Builder.setAllowedChars()</code> and <code>Builder.setAllowedLocales()</code>.</li>
 * </ul>
 * 
 * <p>
 * Note on Scripts: <blockquote>Characters from the Unicode Scripts "Common" and "Inherited" are ignored when considering the script of an
 * identifier. Common characters include digits and symbols that are normally used with text from many different scripts. </blockquote>
 * 
 * @stable ICU 4.6
 */
...
public class SpoofChecker {
	...
	public enum RestrictionLevel {
		...
		ASCII,
		/**
		 * All characters in each identifier must be from a single script, or from the combinations: Latin + Han + Hiragana + Katakana;
		 * Latin + Han + Bopomofo; or Latin + Han + Hangul. Note that this level will satisfy the vast majority of Latin-script users; also
		 * that TR36 has ASCII instead of Latin.
		 * 
		 * @internal
		 */
		HIGHLY_RESTRICTIVE,
	 ...
	 * @internal
	 */
	public static final UnicodeSet RECOMMENDED = new UnicodeSet("[" + "[0-z\\u00C0-\\u017E\\u01A0\\u01A1\\u01AF\\u01B0\\u01CD-"
			+ "\\u01DC\\u01DE-\\u01E3\\u01E6-\\u01F5\\u01F8-\\u021B\\u021E" + "\\u021F\\u0226-\\u0233\\u02BB\\u02BC\\u02EC\\u0300-\\u0304"
			+ "\\u0306-\\u030C\\u030F-\\u0311\\u0313\\u0314\\u031B\\u0323-" + "\\u0328\\u032D\\u032E\\u0330\\u0331\\u0335\\u0338\\u0339"
			+ "\\u0342-\\u0345\\u037B-\\u03CE\\u03FC-\\u045F\\u048A-\\u0525"
			+ "\\u0531-\\u0586\\u05D0-\\u05F2\\u0621-\\u063F\\u0641-\\u0655"
			+ "\\u0660-\\u0669\\u0670-\\u068D\\u068F-\\u06D5\\u06E5\\u06E6"
			+ "\\u06EE-\\u06FF\\u0750-\\u07B1\\u0901-\\u0939\\u093C-\\u094D"
			...
			+ "\\u1E99\\u1F00-\\u1FFC\\u2D30-\\u2D65\\u2D80-\\u2DDE\\u3005-"
			+ "\\u3007\\u3041-\\u31B7\\u3400-\\u9FCB\\uA000-\\uA48C\\uA67F"
			+ "\\uA717-\\uA71F\\uA788\\uAA60-\\uAA7B\\uAC00-\\uD7A3\\uFA0E-" + "\\uFA29\\U00020000-"
			+ "\\U0002B734]-[[:Cn:][:nfkcqc=n:][:XIDC=n:]]]");

	/**
	 * Constants for the kinds of checks that USpoofChecker can perform. These values are used both to select the set of checks that will be
	 * performed, and to report results from the check function.
	 * 
	 */

	/**
	 * Single script confusable test. When testing whether two identifiers are confusable, report that they are if both are from the same
	 * script and they are visually confusable. Note: this test is not applicable to a check of a single identifier.
	 * 
	 * @stable ICU 4.6
	 */
	 ...
	 * 
	 * When checking a single identifier, report a problem if the identifier contains multiple scripts, and is also confusable with some
	 * other identifier in a single script.
	 * 
	 * When testing whether two identifiers are confusable, report that they are if the two IDs are visually confusable, and and at least
	 * one contains characters from more than one script.
	 * 
	 * @stable ICU 4.6
	 */
	 ...
	 * in another script.
	 * 
	 * When testing whether two Identifiers are confusable, report that they are if each is of a single script, the scripts of the two
	 * identifiers are different, and the identifiers are visually confusable.
	 * 
	 * @stable ICU 4.6
	 */
	 ...
	 * 
	 * When specified, consider all characters, of any case, when looking for confusables. If ANY_CASE is not specified, identifiers being
	 * checked are assumed to have been case folded, and upper case conusable characters will not be checked.
	 * 
	 * @stable ICU 4.6
	 */
	...
	/**
	 * Check that an identifer contains only characters from a single script (plus chars from the common and inherited scripts.) Applies to
	 * checks of a single identifier check only.
	 * 
	 * @stable ICU 4.6
	...
	/**
	 * Check an identifier for the presence of invisible characters, such as zero-width spaces, or character sequences that are likely not
	 * to display, such as multiple occurrences of the same non-spacing mark. This check does not test the input string as a whole for
	 * conformance to any particular syntax for identifiers.
	 * 
	 * @stable ICU 4.6
	 */
	...
	/**
	 * Check that an identifier contains only characters from a specified set of acceptable characters. See Builder.setAllowedChars() and
	...
	/**
	 * SpoofChecker Builder. To create a SpoofChecker, first instantiate a SpoofChecker.Builder, set the desired checking options on the
	 * builder, then call the build() function to create a SpoofChecker instance.
	 * 
	 * @stable ICU 4.6
	 */
	public static class Builder {
		...
		/**
		 * Constructor: Create a default Unicode Spoof Checker Builder, configured to perform all checks except for LOCALE_LIMIT and
		 * CHAR_LIMIT. Note that additional checks may be added in the future, resulting in the changes to the default checking behavior.
		 * 
		 * @stable ICU 4.6
		 */
		public Builder() {
		...
		/**
		 * Specify the source form of the spoof data Spoof Checker. The inputs correspond to the Unicode data files confusables.txt and
		 * confusablesWholeScript.txt as described in Unicode UAX 39. The syntax of the source data is as described in UAX 39 for these
		 * files, and the content of these files is acceptable input.
		 * 
		 * @param confusables
		 *            the Reader of confusable characters definitions, as found in file confusables.txt from unicode.org.
		 * @param confusablesWholeScript
		 *            the Reader of whole script confusables definitions, as found in the file xonfusablesWholeScript.txt from unicode.org.
		 * @throws ParseException
		 *             To report syntax errors in the input.
		 * @stable ICU 4.6
		 */
		public Builder setData(final Reader confusables, final Reader confusablesWholeScript) throws ParseException, java.io.IOException {
		 ...
		 * 
		 * @param checks
		 *            The set of checks that this spoof checker will perform. The value is an 'or' of the desired checks.
		 * @return self
		 * @stable ICU 4.6
		 */
		...
		/**
		 * Limit characters that are acceptable in identifiers being checked to those normally used with the languages associated with the
		 * specified locales. Any previously specified list of locales is replaced by the new settings.
		 * 
		 * A set of languages is determined from the locale(s), and from those a set of acceptable Unicode scripts is determined. Characters
		 * from this set of scripts, along with characters from the "common" and "inherited" Unicode Script categories will be permitted.
		 * 
		 * Supplying an empty string removes all restrictions; characters from any script will be allowed.
		 * 
		 * The CHAR_LIMIT test is automatically enabled for this SpoofChecker when calling this function with a non-empty list of locales.
		 * 
		 * The Unicode Set of characters that will be allowed is accessible via the getAllowedChars() function. setAllowedLocales() will
		 * <i>replace</i> any previously applied set of allowed characters.
		 * 
		 * Adjustments, such as additions or deletions of certain classes of characters, can be made to the result of setAllowedLocales() by
		 * fetching the resulting set with getAllowedChars(), manipulating it with the Unicode Set API, then resetting the spoof detectors
		 * limits with setAllowedChars()
		 * 
		 * @param locales
		 *            A Set of ULocales, from which the language and associated script are extracted. If the locales Set is null, no
		 *            restrictions will be placed on the allowed characters.
		 * 
		 * @return self
		 * @stable ICU 4.6
		 */
		...
		/**
		 * Limit the acceptable characters to those specified by a Unicode Set. Any previously specified character limit is is replaced by
		 * the new settings. This includes limits on characters that were set with the setAllowedLocales() function. Note that the
		 * RESTRICTED set is useful;
		 * 
		 * The CHAR_LIMIT test is automatically enabled for this SpoofChecker by this function.
		 * 
		 * @param chars
		 *            A Unicode Set containing the list of characters that are permitted. The incoming set is cloned by this function, so
		 *            there are no restrictions on modifying or deleting the UnicodeSet after calling this function. Note that this clears
		 *            the allowedLocales set.
		 * @return self
		 * @stable ICU 4.6
		 */
		...
		/**
		 * Set the loosest restriction level allowed. The default if this is not called is HIGHLY_RESTRICTIVE. This method also sets
		 * RESTRICTION_LEVEL.
		 * 
		...
		private static class WSConfusableDataBuilder {
			...
			static String parseExp = "(?m)" +                        // Multi-line mode
			...
			static void readWholeFileToString(final Reader reader, final StringBuffer buffer) throws java.io.IOException {
			...
			//
			static void buildWSConfusableData(final SpoofData fSpoofData, final DataOutputStream os, final Reader confusablesWS)
					...
					for (cp = startCodePoint; cp <= endCodePoint; cp++) {
						...
						if (cpScript != srcScript) {
							...
							throw new ParseException("ConfusablesWholeScript, line " + lineNum
									+ ": Mismatch between source script and code point " + Integer.toString(cp, 16), matcher.start(5));
						}
					}
				}
		...
		private static class ConfusabledataBuilder {
			...
			void build(final Reader confusables) throws ParseException, java.io.IOException {
				...
				while (matcher.find()) {
					...
					if (matcher.start(8) >= 0) {
						...
						throw new ParseException("Confusables, line " + fLineNum + ": Unrecognized Line: " + matcher.group(8),
								matcher.start(8));
					}
					...
					if (keyChar > 0x10ffff) {
						throw new ParseException("Confusables, line " + fLineNum + ": Bad code point: " + matcher.group(1),
								matcher.start(1));
					}
					...
					while (m.find()) {
						...
						if (keyChar > 0x10ffff) {
							throw new ParseException("Confusables, line " + fLineNum + ": Bad code point: " + Integer.toString(c, 16),
									matcher.start(2));
						}
					...
					Hashtable<Integer, SPUString> table = matcher.start(3) >= 0 ? fSLTable : matcher.start(4) >= 0 ? fSATable : matcher
							.start(5) >= 0 ? fMLTable : matcher.start(6) >= 0 ? fMATable : null;
					assert (table != null);
			...
			void addKeyEntry(final int keyChar, // The key character
					final Hashtable<Integer, SPUString> table, // The table, one of SATable,
					// MATable, etc.
					final int tableFlag) { // One of SA_TABLE_FLAG, etc.
	...
	/**
	 * Get a list of locales for the scripts that are acceptable in strings to be checked. If no limitations on scripts have been specified,
	 * an empty set will be returned.
	 * 
	 * setAllowedChars() will reset the list of allowed locales to be empty.
	 * 
	 * The returned set may not be identical to the originally specified set that is supplied to setAllowedLocales(); the information other
	 * than languages from the originally specified locales may be omitted.
	 * 
	...
	/**
	 * Get a UnicodeSet for the characters permitted in an identifier. This corresponds to the limits imposed by the Set Allowed Characters
	 * functions. Limitations imposed by other checks will not be reflected in the set returned by this function.
	 * 
	...
	public static class CheckResult {
		/**
		 * Indicate which of the spoof check(s) has failed. The value is a bitwise OR of the constants for the tests in question,
		 * SINGLE_SCRIPT_CONFUSABLE, MIXED_SCRIPT_CONFUSABLE, WHOLE_SCRIPT_CONFUSABLE, and so on.
		 * 
		 * @stable ICU 4.6
		 */
		public int checks;
	...
	/**
	 * Check the specified string for possible security issues. The text to be checked will typically be an identifier of some sort. The set
	 * of checks to be performed was specified when building the SpoofChecker.
	 * 
	 * @param text
	...
	/**
	 * Check the whether two specified strings are visually confusable. The types of confusability to be tested - single script, mixed
	 * script, or whole script - are determined by the check options set for the SpoofChecker.
	 * 
	 * The tests to be performed are controlled by the flags SINGLE_SCRIPT_CONFUSABLE MIXED_SCRIPT_CONFUSABLE WHOLE_SCRIPT_CONFUSABLE At
	 * least one of these tests must be selected.
	 * 
	 * ANY_CASE is a modifier for the tests. Select it if the identifiers may be of mixed case. If identifiers are case folded for
	 * comparison and display to the user, do not select the ANY_CASE option.
	 * 
	 * 
	 * @param s1
	 ...
	 * @return Non-zero if s1 and s1 are confusable. If not 0, the value will indicate the type(s) of confusability found, as defined by
	 *         spoof check test constants.
	 * @stable ICU 4.6
	 */
	...
	/**
	 * Get the "skeleton" for an identifier string. Skeletons are a transformation of the input string; Two strings are confusable if their
	 * skeletons are identical. See Unicode UAX 39 for additional information.
	 * 
	 * Using skeletons directly makes it possible to quickly check whether an identifier is confusable with any of some large set of
	 * existing identifiers, by creating an efficiently searchable collection of the skeletons.
	 * 
	 * @param type
	 *            The type of skeleton, corresponding to which of the Unicode confusable data tables to use. The default is Mixed-Script,
	 *            Lowercase. Allowed options are SINGLE_SCRIPT_CONFUSABLE and ANY_CASE_CONFUSABLE. The two flags may be ORed.
	 * @param id
	...
	private static class SpoofDataHeader {
		...
		public void output(final DataOutputStream os) throws java.io.IOException {
			int i;
			os.writeInt(fMagic);
			for (i = 0; i < fFormatVersion.length; i++) {
	...
	private static class SpoofData {
		...
		public static SpoofData getDefault() throws java.io.IOException {
			...
			InputStream is = com.ibm.icu.impl.ICUData.getRequiredStream(com.ibm.icu.impl.ICUResourceBundle.ICU_BUNDLE + "/confusables.cfu");
		...
		static boolean validateDataVersion(final SpoofDataHeader rawData) {
			if (rawData == null || rawData.fMagic != SpoofChecker.MAGIC || rawData.fFormatVersion[0] > 1 || rawData.fFormatVersion[1] > 0) {
				return false;
			}
			return true;
		}
		...
		// 0) are set to null.
		void initPtrs(final DataInputStream dis) throws java.io.IOException {
			int i;
			fCFUKeys = null;
			fCFUValues = null;
	...
	private static class ScriptSet {
		...
		public ScriptSet(final DataInputStream dis) throws java.io.IOException {
