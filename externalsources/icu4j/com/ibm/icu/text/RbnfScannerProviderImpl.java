...
public class RbnfScannerProviderImpl implements RbnfLenientScannerProvider {
	 ...
	 * 
	 * Only primary differences are treated as significant. This means that case differences, accent differences, alternate spellings of the
	 * same letter (e.g., ae and a-umlaut in German), ignorable characters, etc. are ignored in matching the text. In many cases, numerals
	 * will be accepted in place of words or phrases as well.
	 * 
	 * For example, all of the following will correctly parse as 255 in English in lenient-parse mode: <br>
	 ...
	 * "2 hundred fifty-5"
	 * 
	 * The Collator used is determined by the locale that was passed to this object on construction. The description passed to this object
	 * on construction may supply additional collation rules that are appended to the end of the default collator for the locale, enabling
	 * additional equivalences (such as adding more ignorable characters or permitting spelled-out version of symbols; see the demo program
	 * for examples).
	 * 
	 * It's important to emphasize that even strict parsing is relatively lenient: it will accept some text that it won't produce as output.
	 * In English, for example, it will correctly parse "two hundred zero" and "fifteen hundred".
	 * 
	 * @internal
	 * @deprecated This API is ICU internal only.
	 */
	...
	private static class RbnfLenientScannerImpl implements RbnfLenientScanner {
		...
		public boolean allIgnorable(final String s) {
			...
			int o = iter.next();
			while (o != CollationElementIterator.NULLORDER && CollationElementIterator.primaryOrder(o) == 0) {
				o = iter.next();
			}
		...
		public int[] findText2(final String str, final String key, final int startingAt) {
			...
			while (oKey != CollationElementIterator.NULLORDER) {
				while (oStr != CollationElementIterator.NULLORDER && CollationElementIterator.primaryOrder(oStr) == 0)
					oStr = strIter.next();

				while (oKey != CollationElementIterator.NULLORDER && CollationElementIterator.primaryOrder(oKey) == 0)
					oKey = keyIter.next();
		...
		public int prefixLength(final String str, final String prefix) {
			...
			while (oPrefix != CollationElementIterator.NULLORDER) {
				...
				while (CollationElementIterator.primaryOrder(oStr) == 0 && oStr != CollationElementIterator.NULLORDER) {
					oStr = strIter.next();
				}

				// skip over ignorable characters in the prefix
				while (CollationElementIterator.primaryOrder(oPrefix) == 0 && oPrefix != CollationElementIterator.NULLORDER) {
