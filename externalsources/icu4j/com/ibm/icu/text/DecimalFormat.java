 ...
 * 
 * <code>DecimalFormat</code> is a concrete subclass of {@link NumberFormat} that formats decimal numbers. It has a variety of features
 * designed to make it possible to parse and format numbers in any locale, including support for Western, Arabic, or Indic digits. It also
 * supports different flavors of numbers, including integers ("123"), fixed-point numbers ("123.4"), scientific notation ("1.23E4"),
 * percentages ("12%"), and currency amounts ("$123.00", "USD123.00", "123.00 US dollars"). All of these flavors can be easily localized.
 * 
 * <p>
 * To obtain a {@link NumberFormat} for a specific locale (including the default locale) call one of <code>NumberFormat</code>'s factory
 * methods such as {@link NumberFormat#getInstance}. Do not call the <code>DecimalFormat</code> constructors directly, unless you know what
 * you are doing, since the {@link NumberFormat} factory methods may return subclasses other than <code>DecimalFormat</code>. If you need to
 ...
 * 
 * Print out a number using the localized number, currency, and percent format for each locale.
 * 
 * <blockquote>
 * 
 * <pre>
 ...
 * 			System.out.print(&quot;: &quot; + ((DecimalFormat) format).toPattern() + &quot; -&gt; &quot; + form.format(myNumber));
 * 		} catch (Exception e) {
 * 		}
 * 		try {
 ...
 * 
 * <p>
 * Another example use getInstance(style).<br/>
 * Print out a number using the localized number, currency, percent, scientific, integer, iso currency, and plural currency format for each
 * locale.
 * 
 * <blockquote>
 * 
 ...
 * 
 * <p>
 * A <code>DecimalFormat</code> consists of a <em>pattern</em> and a set of <em>symbols</em>. The pattern may be set directly using
 * {@link #applyPattern}, or indirectly using other API methods which manipulate aspects of the pattern, such as the minimum number of
 * integer digits. The symbols are stored in a {@link DecimalFormatSymbols} object. When using the {@link NumberFormat} factory methods, the
 * pattern and symbols are read from ICU's locale data.
 * 
 * <h4>Special Pattern Characters</h4>
 * 
 * <p>
 * Many characters in a pattern are taken literally; they are matched during parsing and output unchanged during formatting. Special
 * characters, on the other hand, stand for other characters, strings, or classes of characters. For example, the '#' character is replaced
 * by a localized digit. Often the replacement character is the same as the pattern character; in the U.S. locale, the ',' grouping
 * character is replaced by ','. However, the replacement is still happening, and if the symbols are modified, the grouping character
 * changes. Some special characters affect the behavior of the formatter by their presence; for example, if the percent character is seen,
 * then the value is multiplied by 100 before being displayed.
 * 
 * <p>
 * To insert a special character in a pattern as a literal, that is, without any special meaning, the character must be quoted. There are
 * some exceptions to this which are noted below.
 * 
 * <p>
 * The characters listed here are used in non-localized patterns. Localized patterns use the corresponding characters taken from this
 * formatter's {@link DecimalFormatSymbols} object instead, and these characters lose their special status. Two exceptions are the currency
 ...
 * <td>Exponent
 * <td>Yes
 * <td>Prefix positive exponents with localized plus sign. <em>Need not be quoted in prefix or suffix.</em>
 * <tr valign=top bgcolor="#eeeeff">
 * <td><code>;</code>
 ...
 * <td>Prefix or suffix
 * <td>No
 * <td>Currency sign, replaced by currency symbol. If doubled, replaced by international currency symbol. If tripled, replaced by currency
 * plural names, for example, "US dollar" or "US dollars" for America. If present in a pattern, the monetary decimal separator is used
 ...
 * 
 * <p>
 * A <code>DecimalFormat</code> pattern contains a postive and negative subpattern, for example, "#,##0.00;(#,##0.00)". Each subpattern has
 * a prefix, a numeric part, and a suffix. If there is no explicit negative subpattern, the negative subpattern is the localized minus sign
 * prefixed to the positive subpattern. That is, "0.00" alone is equivalent to "0.00;-0.00". If there is an explicit negative subpattern, it
 * serves only to specify the negative prefix and suffix; the number of digits, minimal digits, and other characteristics are ignored in the
 * negative subpattern. That means that "#,##0.0#;(#)" has precisely the same result as "#,##0.0#;(#,##0.0#)".
 * 
 * <p>
 * The prefixes, suffixes, and various symbols used for infinity, digits, thousands separators, decimal separators, etc. may be set to
 * arbitrary values, and they will appear properly during formatting. However, care must be taken that the symbols and strings do not
 * conflict, or parsing will be unreliable. For example, either the positive and negative prefixes or the suffixes must be distinct for
 * {@link #parse} to be able to distinguish positive from negative values. Another example is that the decimal separator and thousands
 * separator should be distinct characters, or parsing will be impossible.
 * 
 * <p>
 * The <em>grouping separator</em> is a character that separates clusters of integer digits to make large numbers more legible. It commonly
 * used for thousands, but in some locales it separates ten-thousands. The <em>grouping size</em> is the number of digits between the
 * grouping separators, such as 3 for "100,000,000" or 4 for "1 0000 0000". There are actually two different grouping sizes: One used for
 * the least significant integer digits, the <em>primary grouping size</em>, and one used for all others, the
 * <em>secondary grouping size</em>. In most locales these are the same, but sometimes they are different. For example, if the primary
 * grouping interval is 3, and the secondary is 2, then this corresponds to the pattern "#,##,##0", and the number 123456789 is formatted as
 * "12,34,56,789". If a pattern contains multiple grouping separators, the interval between the last one and the end of the integer defines
 * the primary grouping size, and the interval between the last two defines the secondary grouping size. All others are ignored, so
 * "#,##,###,####" == "###,###,####" == "##,#,###,####".
 * 
 * <p>
 * Illegal patterns, such as "#.#.#" or "#.###,###", will cause <code>DecimalFormat</code> to throw an {@link IllegalArgumentException} with
 ...
 * <ul>
 * 
 * <li>The grouping separator ',' can occur inside the integer and sigDigits elements, between any two pattern characters of that element,
 * as long as the integer or sigDigits element is not followed by the exponent element.
 * 
 * <li>Two grouping intervals are recognized: That between the decimal point and the first grouping symbol, and that between the first and
 * second grouping symbols. These intervals are identical in most locales, but in some locales they differ. For example, the pattern
 * &quot;#,##,###&quot; formats the number 123456789 as &quot;12,34,56,789&quot;.
 * 
 * <li>The pad specifier <code>padSpec</code> may appear before the prefix, after the prefix, before the suffix, after the suffix, or not at
 * all.
 * 
 ...
 * 
 * <p>
 * <code>DecimalFormat</code> parses all Unicode characters that represent decimal digits, as defined by {@link UCharacter#digit}. In
 * addition, <code>DecimalFormat</code> also recognizes as digits the ten consecutive characters starting with the localized zero digit
 * defined in the {@link DecimalFormatSymbols} object. During formatting, the {@link DecimalFormatSymbols}-based digits are output.
 * 
 * <p>
 * During parsing, grouping separators are ignored.
 * 
 * <p>
 * For currency parsing, the formatter is able to parse every currency style formats no matter which style the formatter is constructed
 * with. For example, a formatter instance gotten from NumberFormat.getInstance(ULocale, NumberFormat.CURRENCYSTYLE) can parse formats such
 * as "USD1.00" and "3.00 US dollars".
 * 
 * <p>
 * If {@link #parse(String, ParsePosition)} fails to parse a string, it returns <code>null</code> and leaves the parse position unchanged.
 * The convenience method {@link #parse(String)} indicates parse failure by throwing a {@link java.text.ParseException}.
 * 
 * <p>
 * Parsing an extremely large or small absolute value (such as 1.0E10000 or 1.0E-10000) requires huge memory allocation for representing the
 * parsed number. Such input may expose a risk of DoS attacks. To prevent huge memory allocation triggered by such inputs,
 * <code>DecimalFormat</code> internally limits of maximum decimal digits to be 1000. Thus, an input string resulting more than 1000 digits
 * in plain decimal representation (non-exponent) will be treated as either overflow (positive/negative infinite) or underflow (+0.0/-0.0).
 * 
 * <h4>Formatting</h4>
 * 
 * <p>
 * Formatting is guided by several parameters, all of which can be specified either using a pattern or using the API. The following
 * description applies to formats that do not use <a href="#sci">scientific notation</a> or <a href="#sigdig">significant digits</a>.
 * 
 * <ul>
 * <li>If the number of actual integer digits exceeds the <em>maximum integer
 * digits</em>, then only the least significant digits are shown. For example, 1997 is formatted as "97" if the maximum integer digits is
 * set to 2.
 * 
 * <li>If the number of actual integer digits is less than the <em>minimum integer
 * digits</em>, then leading zeros are added. For example, 1997 is formatted as "01997" if the minimum integer digits is set to 5.
 * 
 * <li>If the number of actual fraction digits exceeds the <em>maximum fraction
 * digits</em>, then half-even rounding it performed to the maximum fraction digits. For example, 0.125 is formatted as "0.12" if the
 * maximum fraction digits is 2. This behavior can be changed by specifying a rounding increment and a rounding mode.
 * 
 * <li>If the number of actual fraction digits is less than the <em>minimum fraction
 * digits</em>, then trailing zeros are added. For example, 0.125 is formatted as "0.1250" if the mimimum fraction digits is set to 4.
 * 
 * <li>Trailing fractional zeros are not displayed if they occur <em>j</em> positions after the decimal, where <em>j</em> is less than the
 * maximum fraction digits. For example, 0.10004 is formatted as "0.1" if the maximum fraction digits is four or less.
 * </ul>
 * 
 * <p>
 ...
 * 
 * <p>
 * <code>NaN</code> is represented as a single character, typically <code>&#92;uFFFD</code>. This character is determined by the
 * {@link DecimalFormatSymbols} object. This is the only value for which the prefixes and suffixes are not used.
 * 
 * <p>
 * Infinity is represented as a single character, typically <code>&#92;u221E</code>, with the positive or negative prefixes and suffixes
 * applied. The infinity character is determined by the {@link DecimalFormatSymbols} object.
 * 
 * <a name="sci">
 * <h4>Scientific Notation</h4></a>
 * 
 * <p>
 * Numbers in scientific notation are expressed as the product of a mantissa and a power of ten, for example, 1234 can be expressed as 1.234
 * x 10<sup>3</sup>. The mantissa is typically in the half-open interval [1.0, 10.0) or sometimes [0.0, 1.0), but it need not be.
 * <code>DecimalFormat</code> supports arbitrary mantissas. <code>DecimalFormat</code> can be instructed to use scientific notation through
 * the API or through the pattern. In a pattern, the exponent character immediately followed by one or more digit characters indicates
 * scientific notation. Example: "0.###E0" formats the number 1234 as "1.234E3".
 * 
 * <ul>
 * 
 * <li>The number of digit characters after the exponent character gives the minimum exponent digit count. There is no maximum. Negative
 * exponents are formatted using the localized minus sign, <em>not</em> the prefix and suffix from the pattern. This allows patterns such as
 * "0.###E0 m/s". To prefix positive exponents with a localized plus sign, specify '+' between the exponent and the digits: "0.###E+0" will
 * produce formats "1E+1", "1E+0", "1E-1", etc. (In localized patterns, use the localized plus sign rather than '+'.)
 * 
 * <li>The minimum number of integer digits is achieved by adjusting the exponent. Example: 0.00123 formatted with "00.###E0" yields
 * "12.3E-4". This only happens if there is no maximum number of integer digits. If there is a maximum, then the minimum number of integer
 * digits is fixed at one.
 * 
 * <li>The maximum number of integer digits, if present, specifies the exponent grouping. The most common use of this is to generate
 * <em>engineering notation</em>, in which the exponent is a multiple of three, e.g., "##0.###E0". The number 12345 is formatted using
 * "##0.####E0" as "12.345E3".
 * 
 * <li>When using scientific notation, the formatter controls the digit counts using significant digits logic. The maximum number of
 * significant digits limits the total number of integer and fraction digits that will be shown in the mantissa; it does not affect parsing.
 * For example, 12345 formatted with "##0.##E0" is "12.3E3". See the section on significant digits for more details.
 * 
 * <li>The number of significant digits shown is determined as follows: If areSignificantDigitsUsed() returns false, then the minimum number
 * of significant digits shown is one, and the maximum number of significant digits shown is the sum of the <em>minimum integer</em> and
 * <em>maximum fraction</em> digits, and is unaffected by the maximum integer digits. If this sum is zero, then all significant digits are
 * shown. If areSignificantDigitsUsed() returns true, then the significant digit counts are specified by getMinimumSignificantDigits() and
 * getMaximumSignificantDigits(). In this case, the number of integer digits is fixed at one, and there is no exponent grouping.
 * 
 ...
 * 
 * <a name="sigdig">
 * <h4>Significant Digits</h4></a>
 * 
 * <code>DecimalFormat</code> has two ways of controlling how many digits are shows: (a) significant digits counts, or (b) integer and
 * fraction digit counts. Integer and fraction digit counts are described above. When a formatter is using significant digits counts, the
 * number of integer and fraction digits is not specified directly, and the formatter settings for these counts are ignored. Instead, the
 * formatter uses however many integer and fraction digits are required to display the specified number of significant digits. Examples:
 * 
 * <blockquote>
 ...
 * <ul>
 * 
 * <li>Significant digit counts may be expressed using patterns that specify a minimum and maximum number of significant digits. These are
 * indicated by the <code>'@'</code> and <code>'#'</code> characters. The minimum number of significant digits is the number of
 * <code>'@'</code> characters. The maximum number of significant digits is the number of <code>'@'</code> characters plus the number of
 * <code>'#'</code> characters following on the right. For example, the pattern <code>"@@@"</code> indicates exactly 3 significant digits.
 * The pattern <code>"@##"</code> indicates from 1 to 3 significant digits. Trailing zero digits to the right of the decimal separator are
 * suppressed after the minimum number of significant digits have been shown. For example, the pattern <code>"@##"</code> formats the number
 * 0.1203 as <code>"0.12"</code>.
 * 
 * <li>If a pattern uses significant digits, it may not contain a decimal separator, nor the <code>'0'</code> pattern character. Patterns
 * such as <code>"@00"</code> or <code>"@.###"</code> are disallowed.
 * 
 * <li>Any number of <code>'#'</code> characters may be prepended to the left of the leftmost <code>'@'</code> character. These have no
 * effect on the minimum and maximum significant digits counts, but may be used to position grouping separators. For example,
 * <code>"#,#@#"</code> indicates a minimum of one significant digits, a maximum of two significant digits, and a grouping size of three.
 * 
 * <li>In order to enable significant digits formatting, use a pattern containing the <code>'@'</code> pattern character. Alternatively,
 * call {@link #setSignificantDigitsUsed setSignificantDigitsUsed(true)}.
 * 
 * <li>In order to disable significant digits formatting, use a pattern that does not contain the <code>'@'</code> pattern character.
 * Alternatively, call {@link #setSignificantDigitsUsed setSignificantDigitsUsed(false)}.
 * 
 * <li>The number of significant digits has no effect on parsing.
 * 
 * <li>Significant digits may be used together with exponential notation. Such patterns are equivalent to a normal exponential pattern with
 * a minimum and maximum integer digit count of one, a minimum fraction digit count of <code>getMinimumSignificantDigits() -
 * 1</code>, and a maximum fraction digit count of <code>getMaximumSignificantDigits() -
 * 1</code>. For example, the pattern <code>"@@###E0"</code> is equivalent to <code>"0.0###E0"</code>.
 * 
 * <li>If signficant digits are in use, then the integer and fraction digit counts, as set via the API, are ignored. If significant digits
 * are not in use, then the signficant digit counts, as set via the API, are ignored.
 * 
 * </ul>
 * 
 * <h4>Padding</h4>
 * 
 * <p>
 * <code>DecimalFormat</code> supports padding the result of {@link #format} to a specific width. Padding may be specified either through
 * the API or through the pattern syntax. In a pattern the pad escape character, followed by a single pad character, causes padding to be
 * parsed and formatted. The pad escape character is '*' in unlocalized patterns, and can be localized using
 * {@link DecimalFormatSymbols#setPadEscape}. For example, <code>"$*x#,##0.00"</code> formats 123 to <code>"$xx123.00"</code>, and 1234 to
 * <code>"$1,234.00"</code>.
 * 
 * <ul>
 * 
 * <li>When padding is in effect, the width of the positive subpattern, including prefix and suffix, determines the format width. For
 * example, in the pattern <code>"* #0
 * o''clock"</code>, the format width is 10.
 * 
 * <li>The width is counted in 16-bit code units (Java <code>char</code>s).
 * 
 * <li>Some parameters which usually do not matter have meaning when padding is used, because the pattern width is significant with padding.
 * In the pattern "* ##,##,#,##0.##", the format width is 14.  The initial characters "##,##," do not affect the grouping size or maximum
 * integer digits, but they do affect the format width.
 * 
 * <li>Padding may be inserted at one of four locations: before the prefix, after the prefix, before the suffix, or after the suffix. If
 * padding is specified in any other location, {@link #applyPattern} throws an {@link IllegalArgumentException}. If there is no prefix,
 * before the prefix and after the prefix are equivalent, likewise for the suffix.
 * 
 * <li>When specified in a pattern, the 16-bit <code>char</code> immediately following the pad escape is the pad character. This may be any
 * character, including a special pattern character. That is, the pad escape <em>escapes</em> the following character. If there is no
 ...
 * 
 * <p>
 * <code>DecimalFormat</code> supports rounding to a specific increment. For example, 1230 rounded to the nearest 50 is 1250. 1.234 rounded
 * to the nearest 0.65 is 1.3. The rounding increment may be specified through the API or in a pattern. To specify a rounding increment in a
 * pattern, include the increment in the pattern itself. "#,#50" specifies a rounding increment of 50. "#,##0.05" specifies a rounding
 * increment of 0.05.
 * 
 * <ul>
 * 
 * <li>Rounding only affects the string produced by formatting. It does not affect parsing or change any numerical values.
 * 
 * <li>A <em>rounding mode</em> determines how values are rounded; see the {@link com.ibm.icu.math.BigDecimal} documentation for a
 * description of the modes. Rounding increments specified in patterns use the default mode,
 * {@link com.ibm.icu.math.BigDecimal#ROUND_HALF_EVEN}.
 * 
 * <li>Some locales use rounding in their currency formats to reflect the smallest currency denomination.
 * 
 * <li>In a pattern, digits '1' through '9' specify rounding, but otherwise behave identically to digit '0'.
 * 
 * </ul>
 * 
 ...
 * 
 * <p>
 * <code>DecimalFormat</code> objects are not synchronized. Multiple threads should not access one formatter concurrently.
 * 
...
public class DecimalFormat extends NumberFormat {
	...
	/**
	 * Creates a DecimalFormat using the default pattern and symbols for the default <code>FORMAT</code> locale. This is a convenient way to
	 * obtain a DecimalFormat when internationalization is not the main concern.
	 * 
	 * <p>
	 * To obtain standard formats for a given locale, use the factory methods on NumberFormat such as getNumberInstance. These factories
	 * will return the most appropriate sub-class of NumberFormat for a given locale.
	 * 
	...
	/**
	 * Creates a DecimalFormat from the given pattern and the symbols for the default <code>FORMAT</code> locale. This is a convenient way
	 * to obtain a DecimalFormat when internationalization is not the main concern.
	 * 
	 * <p>
	 * To obtain standard formats for a given locale, use the factory methods on NumberFormat such as getNumberInstance. These factories
	 * will return the most appropriate sub-class of NumberFormat for a given locale.
	...
	/**
	 * Creates a DecimalFormat from the given pattern and symbols. Use this constructor when you need to completely customize the behavior
	 * of the format.
	 * 
	 * <p>
	 * To obtain standard formats for a given locale, use the factory methods on NumberFormat such as getInstance or getCurrencyInstance. If
	 * you need only minor adjustments to a standard format, you can modify the format returned by a NumberFormat factory method.
	 * 
	 * @param pattern
	 *            a non-localized pattern string
	 * @param symbols
	...
	/**
	 * Creates a DecimalFormat from the given pattern, symbols, information used for currency plural format, and format style. Use this
	 * constructor when you need to completely customize the behavior of the format.
	 * 
	 * <p>
	 ...
	 * 
	 * <p>
	 * If you need only minor adjustments to a standard format, you can modify the format returned by a NumberFormat factory method using
	 * the setters.
	 * 
	 * <p>
	 * If you want to completely customize a decimal format, using your own DecimalFormatSymbols (such as group separators) and your own
	 * information for currency plural formatting (such as plural rule and currency plural patterns), you can use this constructor.
	 * 
	 * @param pattern
	 *            a non-localized pattern string
	 * @param symbols
	 ...
	 * @param style
	 *            the decimal formatting style, it is one of the following values: NumberFormat.NUMBERSTYLE; NumberFormat.CURRENCYSTYLE;
	 *            NumberFormat.PERCENTSTYLE; NumberFormat.SCIENTIFICSTYLE; NumberFormat.INTEGERSTYLE; NumberFormat.ISOCURRENCYSTYLE;
	...
	/**
	 * Creates a DecimalFormat for currency plural format from the given pattern, symbols, and style.
	 */
	...
	private double round(double number) {
		...
		if (roundingDouble > 0.0) {
			...
			return round(number, roundingDouble, roundingDoubleReciprocal, roundingMode, isNegative);
		}
		return number;
	}
	...
	/**
	 * This is a special function used by the CompactDecimalFormat subclass. It completes only the rounding portion of the formatting and
	 * returns the resulting double. CompactDecimalFormat uses the result to compute the plural form to use.
	 * 
	 * @param number
	 *            The number to format.
	 * @return The number rounded to the correct number of significant digits with negative sign stripped off.
	 * @internal
	 * @deprecated
	 */
	@Deprecated
	...
	/**
	 * This is a special function used by the CompactDecimalFormat subclass to determine if the number to be formatted is negative.
	 * 
	 * @param number
	 *            The number to format.
	...
	/**
	 * Round a double value to the nearest multiple of the given rounding increment, according to the given mode. This is equivalent to
	 * rounding value/roundingInc to the nearest integer, according to the given mode, and returning that integer * roundingInc. Note this
	 * is changed from the version in 2.4, since division of doubles have inaccuracies. jitterbug 1871.
	 * 
	 * @param number
	 ...
	 */
	private static double round(double number, final double roundingInc, final double roundingIncReciprocal, final int mode,
	 ...
	 */
	@Override
	public StringBuffer format(final java.math.BigDecimal number, final StringBuffer result, final FieldPosition fieldPosition) {
		return format(number, result, fieldPosition, false);
	}

	private StringBuffer format(java.math.BigDecimal number, final StringBuffer result, final FieldPosition fieldPosition,
			final boolean parseAttr) {
		if (multiplier != 1) {
	 ...
	 */
	@Override
	public StringBuffer format(BigDecimal number, final StringBuffer result, final FieldPosition fieldPosition) {
	...
	/**
	 * Returns true if a grouping separator belongs at the given position, based on whether grouping is in use and the values of the primary
	 * and secondary grouping interval.
	 * 
	 * @param pos
	 *            the number of integer digits to the right of the current position. Zero indicates the position after the rightmost integer
	 *            digit.
	...
	/**
	 * Return the number of fraction digits to display, or the total number of digits for significant digit formats and exponential formats.
	 */
	...
	private StringBuffer subformat(final int number, final StringBuffer result, final FieldPosition fieldPosition,
			final boolean isNegative, final boolean isInteger, final boolean parseAttr) {
		if (currencySignCount == CURRENCY_SIGN_COUNT_IN_PLURAL_FORMAT) {
	...
	private StringBuffer subformat(final double number, final StringBuffer result, final FieldPosition fieldPosition,
			final boolean isNegative, final boolean isInteger, final boolean parseAttr) {
		if (currencySignCount == CURRENCY_SIGN_COUNT_IN_PLURAL_FORMAT) {
			return subformat(currencyPluralInfo.select(number), result, fieldPosition, isNegative, isInteger, parseAttr);
		} else {
	...
	private StringBuffer subformat(final String pluralCount, final StringBuffer result, final FieldPosition fieldPosition,
			final boolean isNegative, final boolean isInteger, final boolean parseAttr) {
	 ...
	 */
	private StringBuffer subformat(final StringBuffer result, final FieldPosition fieldPosition, final boolean isNegative,
			final boolean isInteger, final boolean parseAttr) {
	...
	private void subformatFixed(final StringBuffer result, final FieldPosition fieldPosition, final boolean isInteger,
			final boolean parseAttr) {
	...
	private void subformatExponential(final StringBuffer result, final FieldPosition fieldPosition, final boolean parseAttr) {
		...
		for (i = 0; i < totalDigits; ++i) {
			...
			result.append((i < digitList.count) ? digits[digitList.getDigitValue(i)] : digits[0]);
		}
		...
		if (parseAttr) {
			addAttribute(Field.EXPONENT_SYMBOL, result.length() - symbols.getExponentSeparator().length(), result.length());
		}
	...
	/**
	 * Parses the given string, returning a <code>Number</code> object to represent the parsed value. <code>Double</code> objects are
	 * returned to represent non-integral values which cannot be stored in a <code>BigDecimal</code>. These are <code>NaN</code>, infinity,
	 * -infinity, and -0.0. If {@link #isParseBigDecimal()} is false (the default), all other values are returned as <code>Long</code>,
	 * <code>BigInteger</code>, or <code>BigDecimal</code> values, in that order of preference. If {@link #isParseBigDecimal()} is true, all
	 * other values are returned as <code>BigDecimal</code> valuse. If the parse fails, null is returned.
	 * 
	 * @param text
	 *            the string to be parsed
	 * @param parsePosition
	 *            defines the position where parsing is to begin, and upon return, the position where parsing left off. If the position has
	 *            not changed upon return, then parsing failed.
	...
	/**
	 * Parses text from the given string as a CurrencyAmount. Unlike the parse() method, this method will attempt to parse a generic
	 * currency name, searching for a match of this object's locale's currency display names, or for a 3-letter ISO currency code. This
	 * method will fail if this format is not a currency format, that is, if it does not contain the currency pattern symbol (U+00A4) in its
	 * prefix or suffix.
	 * 
	 * @param text
	 *            the text to parse
	 ...
	 *            output, the position after the last matched character. If the parse fails, the position in unchanged upon output.
	 * @param currency
	 *            if non-null, a CurrencyAmount is parsed and returned; otherwise a Number is parsed and returned
	...
	private Object parse(final String text, final ParsePosition parsePosition, final Currency[] currency) {
		...
		if (currencySignCount > 0) {
			...
			if (!subparse(text, parsePosition, digitList, status, currency, negPrefixPattern, negSuffixPattern, posPrefixPattern,
	...
	private boolean parseForCurrency(final String text, final ParsePosition parsePosition, final Currency[] currency, final boolean[] status) {
		...
		boolean found;
		if (style == NumberFormat.PLURALCURRENCYSTYLE) {
			found = subparse(text, tmpPos, tmpDigitList, tmpStatus, currency, negPrefixPattern, negSuffixPattern, posPrefixPattern,
		...
		for (AffixForCurrency affix : affixPatternsForCurrency) {
			...
			boolean result = subparse(text, tmpPos, tmpDigitList, tmpStatus, currency, affix.getNegPrefix(), affix.getNegSuffix(),
					affix.getPosPrefix(), affix.getPosSuffix(), affix.getPatternType());
			if (result) {
				found = true;
				if (tmpPos.getIndex() > maxPosIndex) {
					maxPosIndex = tmpPos.getIndex();
		...
		currencySignCount = -1;
		boolean result = subparse(text, tmpPos, tmpDigitList, tmpStatus, currency, negativePrefix, negativeSuffix, positivePrefix,
				positiveSuffix, Currency.SYMBOL_NAME);
	...
	private void setupCurrencyAffixForAllPatterns() {
		...
		applyPatternWithoutExpandAffix(getPattern(symbols.getULocale(), NumberFormat.CURRENCYSTYLE), false);
		AffixForCurrency affixes = new AffixForCurrency(negPrefixPattern, negSuffixPattern, posPrefixPattern, posSuffixPattern,
		...
		while (iter.hasNext()) {
			...
			if (currencyPattern != null && currencyUnitPatternSet.contains(currencyPattern) == false) {
				...
				affixes = new AffixForCurrency(negPrefixPattern, negSuffixPattern, posPrefixPattern, posSuffixPattern, Currency.LONG_NAME);
				affixPatternsForCurrency.add(affixes);
			}
		}
		// reset pattern back
	...
	//"[.\u2024\u3002\uFE12\uFE52\uFF0E\uFF61]"
			0x002E, 0x002E, 0x2024, 0x2024, 0x3002, 0x3002, 0xFE12, 0xFE12, 0xFE52, 0xFE52, 0xFF0E, 0xFF0E, 0xFF61, 0xFF61).freeze();

	private static final UnicodeSet commaEquivalents = new UnicodeSet(
	//"[,\u060C\u066B\u3001\uFE10\uFE11\uFE50\uFE51\uFF0C\uFF64]"
			0x002C, 0x002C, 0x060C, 0x060C, 0x066B, 0x066B, 0x3001, 0x3001, 0xFE10, 0xFE11, 0xFE50, 0xFE51, 0xFF0C, 0xFF0C, 0xFF64, 0xFF64)
			.freeze();
	...
	//"[.\u2024\uFE52\uFF0E\uFF61]"
			0x002E, 0x002E, 0x2024, 0x2024, 0xFE52, 0xFE52, 0xFF0E, 0xFF0E, 0xFF61, 0xFF61).freeze();

	private static final UnicodeSet strictCommaEquivalents = new UnicodeSet(
	//"[,\u066B\uFE10\uFE50\uFF0C]"
			0x002C, 0x002C, 0x066B, 0x066B, 0xFE10, 0xFE10, 0xFE50, 0xFE50, 0xFF0C, 0xFF0C).freeze();
	...
	new UnicodeSet(0x0020, 0x0020, 0x0027, 0x0027, 0x002C, 0x002C, 0x002E, 0x002E, 0x00A0, 0x00A0, 0x060C, 0x060C, 0x066B, 0x066C, 0x2000,
			0x200A, 0x2018, 0x2019, 0x2024, 0x2024, 0x202F, 0x202F, 0x205F, 0x205F, 0x3000, 0x3002, 0xFE10, 0xFE12, 0xFE50, 0xFE52, 0xFF07,
			0xFF07, 0xFF0C, 0xFF0C, 0xFF0E, 0xFF0E, 0xFF61, 0xFF61, 0xFF64, 0xFF64).freeze();
	...
	new UnicodeSet(0x0020, 0x0020, 0x0027, 0x0027, 0x002C, 0x002C, 0x002E, 0x002E, 0x00A0, 0x00A0, 0x066B, 0x066C, 0x2000, 0x200A, 0x2018,
			0x2019, 0x2024, 0x2024, 0x202F, 0x202F, 0x205F, 0x205F, 0x3000, 0x3000, 0xFE10, 0xFE10, 0xFE50, 0xFE50, 0xFE52, 0xFE52, 0xFF07,
			0xFF07, 0xFF0C, 0xFF0C, 0xFF0E, 0xFF0E, 0xFF61, 0xFF61).freeze();
	...
	/**
	 * Parses the given text into a number. The text is parsed beginning at parsePosition, until an unparseable character is seen.
	 * 
	 * @param text
	 *            the string to parse.
	 * @param parsePosition
	 ...
	 * @param currency
	 *            return value for parsed currency, for generic currency parsing mode, or null for normal parsing. In generic currency
	 *            parsing mode, any currency is parsed, not just the currency that this formatter is set to.
	 * @param negPrefix
	 ...
	 */
	private final boolean subparse(final String text, final ParsePosition parsePosition, final DigitList digits, final boolean status[],
			final Currency currency[], final String negPrefix, final String negSuffix, final String posPrefix, final String posSuffix,
			final int type) {
		...
		if (text.regionMatches(position, symbols.getInfinity(), 0, symbols.getInfinity().length())) {
			...
			char decimal = currencySignCount == 0 ? symbols.getDecimalSeparator() : symbols.getMonetaryDecimalSeparator();
			...
			UnicodeSet groupEquiv = skipExtendedSeparatorParsing ? UnicodeSet.EMPTY : (strictParse ? strictDefaultGroupingSeparators
			...
			for (; position < text.length(); position += UTF16.getCharCount(ch)) {
				...
				} else if (digit > 0 && digit <= 9) // [sic] digit==0 handled above
				{
					...
					if (strictParse) {
						if (backup != -1 || (lastGroup != -1 && countCodePoints(text, lastGroup, position) != groupingSize + 1)) {
							strictFail = true;
							break;
						}
					}
	...
	/**
	 * Starting at position, advance past a run of pad characters, if any. Return the index of the first character after position that is
	 * not a pad character. Result is >= position.
	 */
	...
	/**
	 * Returns the length matched by the given affix, or -1 if none. Runs of white space in the affix, match runs of white space in the
	 * input. Pattern white space and input white space are determined differently; see code.
	 * 
	 * @param text
	 *            input text
	 * @param pos
	 ...
	 */
	private int compareAffix(final String text, final int pos, final boolean isNegative, final boolean isPrefix, final String affixPat,
			final int type, final Currency[] currency) {
	...
	/**
	 * Return the length matched by the given affix, or -1 if none. Runs of white space in the affix, match runs of white space in the
	 * input. Pattern white space and input white space are determined differently; see code.
	 * 
	 * @param affix
	...
	/**
	 * Matches a single character at text[pos] and return the index of the next character upon success. Return -1 on failure. If ch is a
	 * Pattern_White_Space then match a run of white space in text.
	 */
	...
	/**
	 * Matches a string at text[pos] and return the index of the next character upon success. Return -1 on failure. Match a run of white
	 * space in str with a run of white space in text.
	 */
	...
	/**
	 * Update the currency object to match the symbols. This method is used only when the caller has passed in a symbols object that may not
	 * be the default object for its locale.
	 */
	...
	/**
	 * Returns the multiplier for use in percent, permill, etc. For a percentage, set the suffixes to have "%" and the multiplier to be 100.
	...
	/**
	 * Sets the multiplier for use in percent, permill, etc. For a percentage, set the suffixes to have "%" and the multiplier to be 100.
	 * (For Arabic, use arabic percent symbol). For a permill, set the suffixes to have "\u2031" and the multiplier to be 1000.
	 * 
	 * <p>
	 * Examples: with 100, 1.23 -> "123", and "123" -> 1.23
	 ...
	 * 
	 * @param newValue
	 *            A positive rounding increment, or <code>null</code> or <code>BigDecimal(0.0)</code> to disable rounding.
	 * @throws IllegalArgumentException
	 ...
	 * 
	 * @param roundingMode
	 *            A rounding mode, between <code>BigDecimal.ROUND_UP</code> and <code>BigDecimal.ROUND_UNNECESSARY</code>.
	 * @exception IllegalArgumentException
	...
	/**
	 * Returns the width to which the output of <code>format()</code> is padded. The width is counted in 16-bit code units.
	 * 
	...
	/**
	 * Sets the width to which the output of <code>format()</code> is padded. The width is counted in 16-bit code units. This method also
	...
	/**
	 * {@icu} Sets the character used to pad to the format width. If padding is not enabled, then this will take effect if padding is later
	 * enabled.
	 * 
	 * @param padChar
	 *            the pad character
	...
	/**
	 * {@icu} Returns the position at which padding will take place. This is the location at which padding will be inserted if the result of
	 * <code>format()</code> is shorter than the format width.
	 * 
	 * @return the pad position, one of <code>PAD_BEFORE_PREFIX</code>, <code>PAD_AFTER_PREFIX</code>, <code>PAD_BEFORE_SUFFIX</code>, or
	...
	/**
	 * {@icu} Sets the position at which padding will take place. This is the location at which padding will be inserted if the result of
	 * <code>format()</code> is shorter than the format width. This has no effect unless padding is enabled.
	 * 
	 * @param padPos
	 *            the pad position, one of <code>PAD_BEFORE_PREFIX</code>, <code>PAD_AFTER_PREFIX</code>, <code>PAD_BEFORE_SUFFIX</code>, or
	 *            <code>PAD_AFTER_SUFFIX</code>.
	 * @exception IllegalArgumentException
	...
	/**
	 * {@icu} Sets whether or not scientific notation is used. When scientific notation is used, the effective maximum number of integer
	 * digits is <= 8. If the maximum number of integer digits is set to more than 8, the effective maximum will be 1. This allows this call
	 * to generate a 'default' scientific number format without additional changes.
	 * 
	 * @param useScientific
	 ...
	 * 
	 * @return true if the exponent is always prefixed with either the localized minus sign or the localized plus sign, false if only
	 ...
	 * 
	 * @param expSignAlways
	 *            true if the exponent is always prefixed with either the localized minus sign or the localized plus sign, false if only
	 *            negative exponents are prefixed with the localized minus sign.
	 * @see #setScientificNotation
	 * @see #isScientificNotation
	 * @see #setMinimumExponentDigits
	...
	/**
	 * Returns the grouping size. Grouping size is the number of digits between grouping separators in the integer portion of a number. For
	 * example, in the number "123,456.78", the grouping size is 3.
	 * 
	 * @see #setGroupingSize
	...
	/**
	 * Sets the grouping size. Grouping size is the number of digits between grouping separators in the integer portion of a number. For
	 * example, in the number "123,456.78", the grouping size is 3.
	 * 
	 * @see #getGroupingSize
	...
	/**
	 * {@icu} Returns the secondary grouping size. In some locales one grouping interval is used for the least significant integer digits
	 * (the primary grouping size), and another is used for all others (the secondary grouping size). A formatter supporting a secondary
	 * grouping size will return a positive integer unequal to the primary grouping size returned by <code>getGroupingSize()</code>. For
	 * example, if the primary grouping size is 4, and the secondary grouping size is 2, then the number 123456789 formats as
	 * "1,23,45,6789", and the pattern appears as "#,##,###0".
	 * 
	...
	/**
	 * {@icu} Sets the secondary grouping size. If set to a value less than 1, then secondary grouping is turned off, and the primary
	 * grouping size is used for all intervals, not just the least significant.
	 * 
	 ...
	 * 
	 * <p>
	 * This only affects formatting, and only where there might be no digits after the decimal point, e.g., if true, 3456.00 -> "3,456." if
	 * false, 3456.00 -> "3456" This is independent of parsing. If you want parsing to stop at the decimal point, use setParseIntegerOnly.
	 * 
	 * <p>
	 * Example: Decimal ON: 12345 -> 12345.; OFF: 12345 -> 12345
	 * 
	...
	/**
	 * {@icu} Returns a copy of the CurrencyPluralInfo used by this format. It might return null if the decimal format is not a plural type
	 * currency decimal format. Plural type currency decimal format means either the pattern in the decimal format contains 3 currency
	 * signs, or the decimal format is initialized with PLURALCURRENCYSTYLE.
	 * 
	...
	public boolean equals(final Object obj) {
		...
		return currencySignCount == other.currencySignCount
				&& (style != NumberFormat.PLURALCURRENCYSTYLE || equals(posPrefixPattern, other.posPrefixPattern)
						&& equals(posSuffixPattern, other.posSuffixPattern) && equals(negPrefixPattern, other.negPrefixPattern)
	...
	/**
	 * Expands the affix pattern strings into the expanded affix strings. If any affix pattern string is null, do not expand it. This method
	 * should be called any time the symbols or the affix patterns change in order to keep the expanded affix strings up to date. This
	 * method also will be called before formatting if format currency plural names, since the plural name is not a static one, it is based
	 * on the currency plural count, the affix will be known only after the currency plural count is know. In which case, the parameter
	 * 'pluralCount' will be a non-null currency plural count. In all other cases, the 'pluralCount' is null, which means it is not needed.
	 */
	...
	/**
	 * Expands an affix pattern into an affix string. All characters in the pattern are literal unless bracketed by QUOTEs. The following
	 * characters outside QUOTE are recognized: PATTERN_PERCENT, PATTERN_PER_MILLE, PATTERN_MINUS, and CURRENCY_SIGN. If CURRENCY_SIGN is
	 * doubled, it is interpreted as an international currency sign. If CURRENCY_SIGN is tripled, it is interpreted as currency plural long
	 * names, such as "US Dollars". Any other character outside QUOTE represents itself. Quoted text must be well-formed.
	 * 
	 * This method is used in two distinct ways. First, it is used to expand the stored affix patterns into actual affixes. For this usage,
	 * doFormat must be false. Second, it is used to expand the stored affix patterns given a specific number (doFormat == true), for those
	 * rare cases in which a currency format references a ChoiceFormat (e.g., en_IN display name for INR). The number itself is taken from
	 * digitList.
	 * 
	 * When used in the first way, this method has a side effect: It sets currencyChoice to a ChoiceFormat object, if the currency's display
	 ...
	 * @param pluralCount
	 *            the plural count. It is only used for currency plural format. In which case, it is the plural count of the currency
	 *            amount. For example, in en_US, it is the singular "one", or the plural "other". For all other cases, it is null, and is
	 *            not being used.
	 * @param buffer
	 *            a scratch StringBuffer; its contents will be lost
	 * @param doFormat
	 *            if false, then the pattern will be expanded, and if a currency symbol is encountered that expands to a ChoiceFormat, the
	 *            currencyChoice member variable will be initialized if it is null. If doFormat is true, then it is assumed that the
	 *            currencyChoice has been created, and it will be used to format the value in digitList.
	 */
	// Bug 4212072 [Richard/GCL]
	...
	private String toPattern(final boolean localized) {
		...
		String padSpec = (formatWidth > 0) ? new StringBuffer(2).append(localized ? symbols.getPadEscape() : PATTERN_PAD_ESCAPE)
				.append(pad).toString() : null;
		if (roundingIncrementICU != null) {
	...
	/**
	 * Applies the given pattern to this Format object. A pattern is a short-hand specification for the various formatting properties. These
	 * properties can also be changed individually through the various setter methods.
	 * 
	 * <p>
	 * There is no limit to integer digits are set by this routine, since that is the typical end-user desire; use setMaximumInteger if you
	 * want to set a real value. For negative numbers, use a second pattern, separated by a semicolon
	 * 
	 * <p>
	...
	/**
	 * Applies the given pattern to this Format object. The pattern is assumed to be in a localized notation. A pattern is a short-hand
	 * specification for the various formatting properties. These properties can also be changed individually through the various setter
	 * methods.
	 * 
	 * <p>
	 * There is no limit to integer digits are set by this routine, since that is the typical end-user desire; use setMaximumInteger if you
	...
	private void applyPatternWithoutExpandAffix(final String pattern, final boolean localized) {
		...
		for (int part = 0; part < 2 && pos < pattern.length(); ++part) {
			...
			if ((decimalPos < 0 && digitRightCount > 0 && sigDigitCount == 0)
					...
					|| groupingCount == 0 || groupingCount2 == 0 || (sigDigitCount > 0 && zeroDigitCount > 0) || subpart > 2) { // subpart > 2 == unmatched quote
	...
	/**
	 * {@icu} Sets the minimum number of significant digits that will be displayed. If <code>min</code> is less than one then it is set to
	 * one. If the maximum significant digits count is less than <code>min</code>, then it is set to <code>min</code>. This value has no
	 * effect unless {@link #areSignificantDigitsUsed()} returns true.
	 * 
	 * @param min
	...
	/**
	 * {@icu} Sets the maximum number of significant digits that will be displayed. If <code>max</code> is less than one then it is set to
	 * one. If the minimum significant digits count is greater than <code>max</code>, then it is set to <code>max</code>. This value has no
	 * effect unless {@link #areSignificantDigitsUsed()} returns true.
	 * 
	 * @param max
	...
	/**
	 * Sets the <tt>Currency</tt> object used to display currency amounts. This takes effect immediately, if this format is a currency
	 * format. If this format is not a currency format, then the currency object is used if and when this object becomes a currency format
	 * through the application of a new pattern.
	 * 
	 * @param theCurrency
	...
	/**
	 * Returns the currency in effect for this formatter. Subclasses should override this method as needed. Unlike getCurrency(), this
	...
	/**
	 * Set the maximum number of exponent digits when parsing a number. If the limit is set too high, an OutOfMemoryException may be
	 * triggered. The default value is 1000.
	 * 
	 * @param newValue
	 *            the new limit
	 * @draft ICU 51
	...
	/**
	 * First, read the default serializable fields from the stream. Then if <code>serialVersionOnStream</code> is less than 1, indicating
	 * that the stream was written by JDK 1.1, initialize <code>useExponentialNotation</code> to false, since it was not present in JDK 1.1.
	 * Finally, set serialVersionOnStream back to the maximum allowed value so that default serialization will work properly if this object
	...
	private void setInternalRoundingIncrement(final BigDecimal value) {
	 ...
	 * 
	 * <p>
	 * This pattern is expanded by the method <code>expandAffix()</code> to <code>positivePrefix</code> to update the latter to reflect
	 * changes in <code>symbols</code>. If this variable is <code>null</code> then <code>positivePrefix</code> is taken as a literal value
	 * that does not change when <code>symbols</code> changes. This variable is always <code>null</code> for <code>DecimalFormat</code>
	...
	/**
	 * The suffix pattern for non-negative numbers. This variable corresponds to <code>positiveSuffix</code>. This variable is analogous to
	...
	/**
	 * The prefix pattern for negative numbers. This variable corresponds to <code>negativePrefix</code>. This variable is analogous to
	 * <code>posPrefixPattern</code>; see that variable for further documentation.
	 * 
	 * @serial
	 */
	...
	/**
	 * The suffix pattern for negative numbers. This variable corresponds to <code>negativeSuffix</code>. This variable is analogous to
	 * <code>posPrefixPattern</code>; see that variable for further documentation.
	 * 
	 * @serial
	 */
	...
	/**
	 * The secondary grouping size. This is only used for Hindi numerals, which use a primary grouping of 3 and a secondary grouping of 2,
	 * e.g., "12,34,567". If this value is less than 1, then secondary grouping is equal to the primary grouping.
	 * 
	 */
	...
	/**
	 * The <code>DecimalFormatSymbols</code> object used by this format. It contains the symbols used to format numbers, e.g. the grouping
	 * separator, decimal separator, and so on.
	 * 
	 * @serial
	...
	/**
	 * The minimum number of significant digits to show. Must be >= 1 and <= maxSignificantDigits. Ignored unless useSignificantDigits ==
	 * true.
	 * 
	 * @serial
	 * @since ICU 3.0
	 ...
	 * 
	 * <p>
	 * Note that the JDK 1.2 public API provides no way to set this field, even though it is supported by the implementation and the stream
	 * format. The intent is that this will be added to the API in the future.
	 * 
	 * @serial
	 */
	...
	/**
	 * The minimum number of digits used to display the exponent when a number is formatted in exponential notation. This field is ignored
	 * if <code>useExponentialNotation</code> is not true.
	 * 
	 * <p>
	 * Note that the JDK 1.2 public API provides no way to set this field, even though it is supported by the implementation and the stream
	 * format. The intent is that this will be added to the API in the future.
	...
	/**
	 * If true, the exponent is always prefixed with either the plus sign or the minus sign. Otherwise, only negative exponents are prefixed
	 * with the minus sign. This has no effect unless <code>useExponentialNotation</code> is true.
	 * 
	 * @serial
	...
	/**
	 * The value to which numbers are rounded during formatting. For example, if the rounding increment is 0.05, then 13.371 would be
	 * formatted as 13.350, assuming 3 fraction digits. Has the value <code>null</code> if rounding is not in effect, or a positive value if
	 * rounding is in effect. Default value <code>null</code>.
	 * 
	 * @serial
	 * @since AlphaWorks NumberFormat
	 */
	 ...
	 * The value to which numbers are rounded during formatting. For example, if the rounding increment is 0.05, then 13.371 would be
	 * formatted as 13.350, assuming 3 fraction digits. Has the value <code>null</code> if rounding is not in effect, or a positive value if
	 * rounding is in effect. Default value <code>null</code>. WARNING: the roundingIncrement value is the one serialized.
	 * 
	 * @serial
	 * @since AlphaWorks NumberFormat
	 */
	...
	/**
	 * If the roundingDouble is the reciprocal of an integer (the most common case!), this is set to be that integer. Otherwise it is 0.0.
	 */
	private transient double roundingDoubleReciprocal = 0.0;

	/**
	 * The rounding mode. This value controls any rounding operations which occur when applying a rounding increment or when reducing the
	 * number of fraction digits to satisfy a maximum fraction digits limit. The value may assume any of the <code>BigDecimal</code>
	 * rounding mode values. Default value <code>BigDecimal.ROUND_HALF_EVEN</code>.
	 * 
	 * @serial
	 * @since AlphaWorks NumberFormat
	 */
	...
	/**
	 * Operations on <code>BigDecimal</code> numbers are controlled by a {@link MathContext} object, which provides the context (precision
	 * and other information) for the operation. The default <code>MathContext</code> settings are
	 * <code>digits=0, form=PLAIN, lostDigits=false, roundingMode=ROUND_HALF_UP</code>; these settings perform fixed point arithmetic with
	...
	/**
	 * The character used to pad the result of format to <code>formatWidth</code>, if padding is in effect. Default value ' '.
	 * 
	 * @serial
	 * @since AlphaWorks NumberFormat
	 */
	...
	/**
	 * The position in the string at which the <code>pad</code> character will be inserted, if padding is in effect. Must have a value from
	 * <code>PAD_BEFORE_PREFIX</code> to <code>PAD_AFTER_SUFFIX</code>. Default value <code>PAD_BEFORE_PREFIX</code>.
	 * 
	 * @serial
	 * @since AlphaWorks NumberFormat
	 */
	...
	/**
	 * True if {@link #parse(String, ParsePosition)} to return BigDecimal rather than Long, Double or BigDecimal except special values. This
	 * property is introduced for J2SE 5 compatibility support.
	 * 
	 * @serial
	 * @since ICU 3.6
	 ...
	 * 
	 * <li><b>1</b>: version from JDK 1.2 and later, which includes the two new fields <code>useExponentialNotation</code> and
	 * <code>minExponentDigits</code>.
	 * 
	 * <li><b>2</b>: version on AlphaWorks, which adds roundingMode, formatWidth, pad, padPosition, exponentSignAlwaysShown,
	 * roundingIncrement.
	 * 
	...
	/**
	 * {@icu} Constant for {@link #getPadPosition()} and {@link #setPadPosition(int)} to specify pad characters inserted before the prefix.
	 * 
	 * @see #setPadPosition
	 * @see #getPadPosition
	 * @see #PAD_AFTER_PREFIX
	...
	/**
	 * {@icu} Constant for {@link #getPadPosition()} and {@link #setPadPosition(int)} to specify pad characters inserted after the prefix.
	 * 
	 * @see #setPadPosition
	 * @see #getPadPosition
	 * @see #PAD_BEFORE_PREFIX
	...
	/**
	 * {@icu} Constant for {@link #getPadPosition()} and {@link #setPadPosition(int)} to specify pad characters inserted before the suffix.
	 * 
	 * @see #setPadPosition
	 * @see #getPadPosition
	 * @see #PAD_BEFORE_PREFIX
	...
	/**
	 * {@icu} Constant for {@link #getPadPosition()} and {@link #setPadPosition(int)} to specify pad characters inserted after the suffix.
	 * 
	 * @see #setPadPosition
	 * @see #getPadPosition
	 * @see #PAD_BEFORE_PREFIX
	...
	/**
	 * The CURRENCY_SIGN is the standard Unicode symbol for currency. It is used in patterns and substitued with either the currency symbol,
	 * or if it is doubled, with the international currency symbol. If the CURRENCY_SIGN is seen in a pattern, then the decimal separator is
	...
	/**
	 * When someone turns on scientific mode, we assume that more than this number of digits is due to flipping from some other mode that
	 * didn't restrict the maximum, and so we force 1 integer digit. We don't bother to track and see if someone is using exponential
	 * notation with more than this number, it wouldn't make sense anyway, and this is just to make sure that someone turning on scientific
	 * mode with default settings doesn't end up with lots of zeroes.
	 */
	...
	/**
	 * Represents whether this is a currency format, and which currency format style. 0: not currency format type; 1: currency style --
	 * symbol name, such as "$" for US dollar. 2: currency style -- ISO name, such as USD for US dollar. 3: currency style -- plural long
	 * name, such as "US Dollar" for "1.00 US Dollar", or "US Dollars" for "3.00 US Dollars".
	 */
	private int currencySignCount = 0;

	/**
	 * For parsing purposes, we need to remember all prefix patterns and suffix patterns of every currency format pattern, including the
	 * pattern of the default currency style, ISO currency style, and plural currency style. The patterns are set through applyPattern. The
	 * following are used to represent the affix patterns in currency plural formats.
	 */
	private static final class AffixForCurrency {
		...
		public AffixForCurrency(final String negPrefix, final String negSuffix, final String posPrefix, final String posSuffix,
				final int type) {
