...
/**
 * A replacer that produces static text as its output. The text may contain transliterator stand-in characters that represent nested
 * UnicodeReplacer objects, making it possible to encode a tree of replacers in a StringReplacer. A StringReplacer that contains such
 * stand-ins is called a <em>complex</em> StringReplacer. A complex StringReplacer has a slower processing loop than a non-complex one.
 * 
 * @author Alan Liu
 */
...
class StringReplacer implements UnicodeReplacer {
	...
	/**
	 * A complex object contains nested replacers and requires more complex processing. StringReplacers are initially assumed to be complex.
	 * If no nested replacers are seen during processing, then isComplex is set to false, and future replacements are short circuited for
	 * better performance.
	 */
	 ...
	 * 
	 * @param theOutput
	 *            text that will replace input text when the replace() method is called. May contain stand-in characters that represent
	 *            nested replacers.
	 * @param theCursorPos
	 *            cursor position that will be returned by the replace() method
	 * @param theData
