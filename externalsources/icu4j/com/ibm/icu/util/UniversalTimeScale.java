...
/**
 * There are quite a few different conventions for binary datetime, depending on different platforms and protocols. Some of these have
 * severe drawbacks. For example, people using Unix time (seconds since Jan 1, 1970, usually in a 32-bit integer) think that they are safe
 * until near the year 2038. But cases can and do arise where arithmetic manipulations causes serious problems. Consider the computation of
 * the average of two datetimes, for example: if one calculates them with <code>averageTime = (time1 + time2)/2</code>, there will be
 * overflow even with dates beginning in 2004. Moreover, even if these problems don't occur, there is the issue of conversion back and forth
 ...
 * 
 * <p>
 * ICU implements a universal time scale that is similar to the .NET framework's System.DateTime. The universal time scale is a 64-bit
 * integer that holds ticks since midnight, January 1st, 0001. (One tick is 100 nanoseconds.) Negative values are supported. This has enough
 * range to guarantee that calculations involving dates around the present are safe.
 * </p>
 * 
 * <p>
 * The universal time scale always measures time according to the proleptic Gregorian calendar. That is, the Gregorian calendar's leap year
 * rules are used for all times, even before 1582 when it was introduced. (This is different from the default ICU calendar which switches
 * from the Julian to the Gregorian calendar in 1582. See GregorianCalendar.setGregorianChange() and ucal_setGregorianChange().)
 * </p>
 * 
 * ICU provides conversion functions to and from all other major time scales, allowing datetimes in any time scale to be converted to the
 * universal time scale, safely manipulated, and converted back to any other datetime time scale.</p>
 * 
 * <p>
 * For more details and background, see the <a href="http://www.icu-project.org/userguide/universalTimeScale.html">Universal Time Scale</a>
...
public final class UniversalTimeScale {
	...
	/**
	 * Used in Unix systems. Data is an <code>int</code> or a <code>long</code>. Value is seconds since January 1, 1970.
	 * 
	 * @stable ICU 3.2
	 */
	...
	/**
	 * Used in the ICU4C. Data is a <code>double</code>. Value is milliseconds since January 1, 1970.
	 * 
	 * @stable ICU 3.2
	 */
	...
	/**
	 * Used in Windows for file times. Data is a <code>long</code>. Value is ticks (1 tick == 100 nanoseconds) since January 1, 1601.
	 * 
	 * @stable ICU 3.2
	 */
	...
	/**
	 * Used in the .NET framework's <code>System.DateTime</code> structure. Data is a <code>long</code>. Value is ticks (1 tick == 100
	 * nanoseconds) since January 1, 0001.
	 * 
	 * @stable ICU 3.2
	 */
	...
	/**
	 * Used in older Macintosh systems. Data is an <code>int</code>. Value is seconds since January 1, 1904.
	 * 
	 * @stable ICU 3.2
	 */
	...
	/**
	 * Data is a <code>long</code>. Value is microseconds since January 1, 1970. Similar to Unix time (linear value from 1970) and struct
	 * timeval (microseconds resolution).
	 * 
	 * @stable ICU 3.8
	 */
	 ...
	 * 
	 * NOTE: This is an internal value. DO NOT USE IT. May not actually be equal to the epoch offset value minus one.
	 * 
	 * @see #getTimeScaleValue
	 * 
	 * @internal
	...
	private static final TimeScaleData[] timeScaleTable = {
			new TimeScaleData(milliseconds, 621355968000000000L, -9223372036854774999L, 9223372036854774999L, -984472800485477L,
			...
			new TimeScaleData(ticks, 504911232000000000L, -8718460804854775808L, 9223372036854775807L, -9223372036854775808L,
					8718460804854775807L), // WINDOWS_FILE_TIME
			new TimeScaleData(ticks, 000000000000000000L, -9223372036854775808L, 9223372036854775807L, -9223372036854775808L,
			...
			new TimeScaleData(microseconds, 621355968000000000L, -9223372036854775804L, 9223372036854775804L, -984472800485477580L,
	...
	/**
	 * Convert a <code>double</code> datetime from the given time scale to the universal time scale. All calculations are done using
	 * <code>BigDecimal</code> to guarantee that the value does not go out of range.
	 * 
	 * @param otherTime
	 *            The <code>double</code> datetime
	 * @param timeScale
	 *            The time scale to convert from
	 * 
	 * @return The datetime converted to the universal time scale
	...
	/**
	 * Convert a <code>long</code> datetime from the given time scale to the universal time scale. All calculations are done using
	 * <code>BigDecimal</code> to guarantee that the value does not go out of range.
	 * 
	 * @param otherTime
	 *            The <code>long</code> datetime
	...
	/**
	 * Convert a <code>BigDecimal</code> datetime from the given time scale to the universal time scale. All calculations are done using
	 * <code>BigDecimal</code> to guarantee that the value does not go out of range.
	 * 
	 * @param otherTime
	 *            The <code>BigDecimal</code> datetime
	 * @param timeScale
	 *            The time scale to convert from
	 * 
	 * @return The datetime converted to the universal time scale
	 ...
	 * 
	 * Since this calculation requires a divide, we must round. The straight forward way to round by adding half of the divisor will push
	 * the sum out of range for values within have the divisor of the limits of the precision of a <code>long</code>. To get around this, we
	 * do the rounding like this:
	 * 
	 * <p>
	 * <code>
	 ...
	 * 
	 * <p>
	 * (i.e. we subtract units first to guarantee that we'll still be in range when we add <code>units/2</code>. We then need to add one to
	 * the quotent to make up for the extra subtraction. This simplifies to:
	 * 
	 * <p>
	 * <code>
	 ...
	 * 
	 * <p>
	 * Since we also need to subtract the epochOffset, we fold the <code>+/- 1</code> into the offset value. (i.e.
	 * <code>epochOffsetP1</code>, <code>epochOffsetM1</code>.)
	 * 
	 * @param universalTime
