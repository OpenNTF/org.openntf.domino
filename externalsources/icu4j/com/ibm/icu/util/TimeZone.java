 ...
 * 
 * <p>
 * <code>TimeZone</code> represents a time zone offset, and also computes daylight savings.
 * 
 * <p>
 * Typically, you get a <code>TimeZone</code> using {@link #getDefault()} which creates a <code>TimeZone</code> based on the time zone where
 * the program is running. For example, for a program running in Japan, <code>getDefault</code> creates a <code>TimeZone</code> object based
 * on Japanese Standard Time.
 * 
 * <p>
 * You can also get a <code>TimeZone</code> using {@link #getTimeZone(String)} along with a time zone ID. For instance, the time zone ID for
 * the U.S. Pacific Time zone is "America/Los_Angeles". So, you can get a U.S. Pacific Time <code>TimeZone</code> object with:
 * 
 * <blockquote>
 * 
 * <pre>
 ...
 * </pre>
 * 
 * </blockquote> You can use the {@link #getAvailableIDs()} method to iterate through all the supported time zone IDs, or getCanonicalID
 * method to check if a time zone ID is supported or not. You can then choose a supported ID to get a <code>TimeZone</code>. If the time
 * zone you want is not represented by one of the supported IDs, then you can create a custom time zone ID with the following syntax:
 * 
 * <blockquote>
 * 
 * <pre>
 ...
 * </blockquote>
 * 
 * For example, you might specify GMT+14:00 as a custom time zone ID. The <code>TimeZone</code> that is returned when you specify a custom
 * time zone ID uses the specified offset from GMT(=UTC) and does not observe daylight saving time. For example, you might specify GMT+14:00
 * as a custom time zone ID to create a TimeZone representing 14 hours ahead of GMT (with no daylight saving time). In addition,
 * <code>getCanonicalID</code> can also be used to normalize a custom time zone ID.
 * 
 * <p>
 * For compatibility with JDK 1.1.x, some other three-letter time zone IDs (such as "PST", "CTT", "AST") are also supported. However,
 * <strong>their use is deprecated</strong> because the same abbreviation is often used for multiple time zones (for example, "CST" could be
 * U.S. "Central Standard Time" and "China Standard Time"), and the Java platform can then only recognize one of them.
 * 
 * <p>
 * <strong>Note:</strong> Starting from ICU4J 4.0, you can optionally choose JDK <code>TimeZone</code> as the time zone implementation. The
 * TimeZone factory method <code>getTimeZone</code> creates an instance of ICU's own <code>TimeZone</code> subclass by default. If you want
 * to use the JDK implementation always, you can set the default time zone implementation type by the new method
 * <code>setDefaultTimeZoneType</code>. Alternatively, you can change the initial default implementation type by setting a property below.
 * 
 * <blockquote>
 * 
 * <pre>
 ...
 * 
 * <p>
 * This property is included in ICUConfig.properties in com.ibm.icu package. When the <code>TimeZone</code> class is loaded, the
 * initialization code checks if the property <code>com.ibm.icu.util.TimeZone.DefaultTimeZoneType=xxx</code> is defined by the system
 * properties. If not available, then it loads ICUConfig.properties to get the default time zone implementation type. The property setting
 * is only used for the initial default value and you can change the default type by calling <code>setDefaultTimeZoneType</code> at runtime.
 * 
 * @see Calendar
...
abstract public class TimeZone implements Serializable, Cloneable, Freezable<TimeZone> {
	...
	/**
	 * {@icu} A time zone implementation type indicating ICU's own TimeZone used by <code>getTimeZone</code>,
	 * <code>setDefaultTimeZoneType</code> and <code>getDefaultTimeZoneType</code>.
	 * 
	 * @stable ICU 4.0
	 */
	public static final int TIMEZONE_ICU = 0;
	/**
	 * {@icu} A time zone implementation type indicating JDK TimeZone used by <code>getTimeZone</code>, <code>setDefaultTimeZoneType</code>
	...
	/**
	 * A style specifier for <code>getDisplayName()</code> indicating a long name, such as "Pacific Standard Time."
	 * 
	 * @see #SHORT
	 * @stable ICU 2.0
	 */
	...
	/**
	 * {@icu} A style specifier for <code>getDisplayName()</code> indicating a short generic name, such as "PT."
	 * 
	 * @see #LONG_GENERIC
	 * @stable ICU 4.4
	 */
	...
	/**
	 * {@icu} A style specifier for <code>getDisplayName()</code> indicating a long generic name, such as "Pacific Time."
	 * 
	 * @see #SHORT_GENERIC
	 * @stable ICU 4.4
	 */
	...
	/**
	 * {@icu} A style specifier for <code>getDisplayName()</code> indicating a short name derived from the timezone's offset, such as
	 * "-0800."
	 * 
	 * @see #LONG_GMT
	 * @stable ICU 4.4
	...
	/**
	 * {@icu} A style specifier for <code>getDisplayName()</code> indicating a long name derived from the timezone's offset, such as
	 * "GMT-08:00."
	 * 
	 * @see #SHORT_GMT
	 * @stable ICU 4.4
	...
	/**
	 * {@icu} A style specifier for <code>getDisplayName()</code> indicating a short name derived from the timezone's short standard or
	 * daylight timezone name ignoring commonlyUsed, such as "PDT."
	 * 
	 * @stable ICU 4.4
	 */
	...
	/**
	 * {@icu} A style specifier for <code>getDisplayName()</code> indicating a long name derived from the timezone's fallback name, such as
